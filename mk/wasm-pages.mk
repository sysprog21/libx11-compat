...
$(WASM_PAGES_STAMP): $(WASM_PAGES_HTML) mk/wasm-pages.mk
	...
	$(Q){ \
	    printf '%s\n' '<!doctype html>' '<meta charset="utf-8">' \
	        ...
	        '<style>body{font:16px system-ui,sans-serif;max-width:760px;margin:40px auto;padding:0 20px;line-height:1.5}h1{font-size:28px}h2{font-size:20px;margin-top:28px}a{color:#0645ad}</style>' \
	        '<h1>libx11-compat WebAssembly showcases</h1>' '<h2>Examples</h2>' '<ul>'; \
	    for item in \
	    ...
	    for item in \
	        'xnedit:XNEdit' 'xwpe:xwpe' 'mosaic:Mosaic' 'xcircuit:XCircuit'; do \
	        app="$${item%%:*}"; title="$${item#*:}"; \
	        printf '<li><a href="%s.html">%s</a></li>\n' "$$app" "$$title"; \
	    done; \
	    printf '%s\n' '</ul>'; \
...
check-wasm-pages:
	...
	for app in $(WASM_PAGES_APPS); do \
	    if [ ! -s "$(OUT)/pages/$$app.html" ]; then \
	        echo "check-wasm-pages: missing $$app.html" >&2; exit 1; \
	    fi; \
	done; \
