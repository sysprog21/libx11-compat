...
$(LIBXT_OBJ_DIR)/%.o: $(UPSTREAM_HEADERS_STAMP) $(LIBXT_GEN_HEADERS) $(LIBXT_STAGED_H) | \
	...
	$(Q)$(CC) $(LIBXT_CPPFLAGS) $(CFLAGS) $(LIBXT_CFLAGS) $(CFLAGS_EXTRA) \
	    -MMD -MP -MF $(@:.o=.d) -c $(LIBXT_SRC_DIR)/$*.c -o $@
...
#
# rpath=$$ORIGIN tells the Linux dynamic linker to look in the same
# directory as libXt-compat.so for its DT_NEEDED entries -- without it,
# libXt-compat.so's reference to libX11-compat.so (recorded by basename
# from the -lX11-compat link) is unresolvable at runtime because the
# loader does not search the original link directory. macOS encodes a
# relative install_name (build/libX11-compat.so) so cwd-from-repo-root
# already works there; gate the flag on Linux only.
LIBXT_LDFLAGS :=
ifeq ($(UNAME_S),Linux)
  LIBXT_LDFLAGS += -Wl,-rpath,'$$ORIGIN'
endif
...
# Include the per-object header-dependency files generated by -MMD above.
# Without this, edits to staged libXt headers or the synthesized
# config.h / StringDefs.h do not trigger rebuilds of the libXt .o files
# and incremental builds can ship stale objects.
