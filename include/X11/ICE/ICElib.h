 ...
 * Provide an opaque IceConn type plus the small set of helpers and constants
 * the in-tree compatibility surface references so the header resolves without
 * dragging libICE into the build closure.
 */
...
/* Real libICE defines IceProcessMessagesStatus as an enum too, but the values
 * themselves are commonly probed via macros in some downstream code. Mirror
 * with #ifndef so any later #include of the real header (e.g. through a Motif
 * build pulling system X11 headers) does not collide on the enum tag.
 */
