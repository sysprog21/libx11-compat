 ...
 * Shell.c registers so an external editres client can introspect a live widget
 * tree. libx11-compat ships no Xmu, no editres client targets this in-process
 * display, and Shell.c's only use is the function-pointer registration. A no-op
 * _XEditResCheckMessages keeps Shell.c compilable and link-safe without
 * dragging libXmu into the build.
 */
