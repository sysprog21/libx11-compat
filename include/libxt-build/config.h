 ...
 * platform knobs. libx11-compat does not run autoconf, so the knobs
 * are pinned to values that match the modern macOS / Linux
 * environments libx11-compat targets: poll(2) is always available,
 * threads are enabled, and the session-management surface is disabled
 * so libICE/libSM stay out of the dependency closure (see
 * mk/libxt.mk).
 *
 ...
 * include path with anything else. mk/libxt.mk adds the parent
 * directory to the include path *only* for the libXt translation
 * units.
 */
...
/* poll() is on every libx11-compat-supported platform. The select()
 * fallback path in NextEvent.c is bit-set-sized and predates large fd
 * numbers; libx11-compat never selects it.
 */
#define USE_POLL 1
