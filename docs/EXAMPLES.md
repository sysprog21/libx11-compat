...
| `moire` | [`examples/moire.c`](../examples/moire.c) | Adopted from `x-tests` `moire`. Animated moire pattern with batched `XDrawSegments`, pixmap back-buffer redraw, timed frames, and `WM_DELETE_WINDOW`. Pass `--once` for a single-frame smoke. |
...
`gradient` from `x-tests` is intentionally not bundled yet: it depends on `XRenderCreateLinearGradient`, which the current XRender shim does not implement.
