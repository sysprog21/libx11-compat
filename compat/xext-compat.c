...
typedef void (*XShapeCombineRegionFn)(Display*, Window, int, int, int, Region, int);
typedef void (*XShapeCombineRectanglesFn)(Display*, XID, int, int, int, XRectangle*, int, int, int);
typedef void (*XShapeCombineMaskFn)(Display*, XID, int, int, int, Pixmap, int);
typedef void (*XShapeCombineShapeFn)(Display*, XID, int, int, int, Pixmap, int, int);
typedef void (*XShapeOffsetShapeFn)(Display*, XID, int, int, int);
typedef Status (*XShapeQueryExtentsFn)(Display*, Window, Bool*, int*, int*, unsigned int*, unsigned int*, Bool*, int*, int*, unsigned int*, unsigned int*);
typedef void (*XShapeSelectInputFn)(Display*, Window, unsigned long);
typedef unsigned long (*XShapeInputSelectedFn)(Display*, Window);
typedef XRectangle* (*XShapeGetRectanglesFn)(Display*, Window, int, int*, int*);
...
void XShapeCombineRegion(Display* dpy, Window dest, int destKind, int xOff, int yOff, Region r, int op)
{
...
Status XShapeQueryExtents(Display* dpy, Window w, Bool* bShaped, int* xbs, int* ybs, unsigned int* wbs, unsigned int* hbs, Bool* cShaped, int* xcs, int* ycs, unsigned int* wcs, unsigned int* hcs)
{
    XShapeQueryExtentsFn fn = RESOLVE(XShapeQueryExtents);
    return fn ? fn(dpy, w, bShaped, xbs, ybs, wbs, hbs, cShaped, xcs, ycs, wcs, hcs) : 0;
}
