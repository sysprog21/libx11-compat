...
#define GOM(T, t)           \
    ...
    switch (type) {
        ...
        GOV(GL_INT, GLint);
        GOV(GL_BOOL, /*GLboolean*/ GLint);  // GLboolean is an unsigned char and
                                         ...
                                         GLint *params)
{
    ...
    if (k != kh_end(glprogram->uniform)) {
        ...
        for (int i = 0; i < n; i++)
            params[i] =
                fl[i];  // is this correct? float -> int without adjustment?
        noerrorShim();
        return;
    }
    errorShim(GL_INVALID_VALUE);
...
void GoUniformfv(program_t *glprogram,
                 GLint location,
                 int size,
                 int count,
                 const GLfloat *value)
...
void GoUniformiv(program_t *glprogram,
                 GLint location,
                 int size,
                 int count,
                 const GLint *value)
...
void GoUniformMatrix2fv(program_t *glprogram,
                        GLint location,
                        GLsizei count,
                        GLboolean transpose,
                        const GLfloat *value)
{
    ...
    if (memcmp((char *) glprogram->cache.cache + m->cache_offs, v, rsize) ==
        0) {
        noerrorShim();
        return;  // nothing to do, same value already there
    }
...
void GoUniformMatrix3fv(program_t *glprogram,
                        GLint location,
                        GLsizei count,
                        GLboolean transpose,
                        const GLfloat *value)
...
{
    DBG(printf(
            "glUniformMatrix4fv(%d, %d, %d, %p) p=>(%f, %f, %f, %f, %f...)\n",
...
void GoUniformMatrix4fv(program_t *glprogram,
                        GLint location,
                        GLsizei count,
                        GLboolean transpose,
                        const GLfloat *value)
...
int GetUniformi(program_t *glprogram, GLint location)
{
    ...
    GLint ret;
    memcpy(&ret, (char *) glprogram->cache.cache + m->cache_offs,
           sizeof(GLint));
    return ret;
}
