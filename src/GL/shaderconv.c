...
#define ShadAppend(S) Tmp = gl4es_append(Tmp, &tmpsize, S)
    ...
    int notexarray =
        globals4es.notexarray || need->need_notexarray || fpeShader;
    ...
    if (fragdepth) {
        /* If #extension is used, it should be placed before the second line of
         * the header.
         */
        if (hardext.fragdepth)
            Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 1), GLESUseFragDepth,
                                       Tmp, &tmpsize);
        else
    ...
    if (derivatives) {
        ...
        if (hardext.derivatives)
            Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 1), GLESUseDerivative,
                                       Tmp, &tmpsize);
        else
            Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline - 1),
    ...
    if (hardext.maxdrawbuffers > 1 && strstr(pBuffer, "gl_FragData[")) {
        Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 1), useEXTDrawBuffers,
                                   Tmp, &tmpsize);
    }
    ...
    if (!fpeShader && !globals4es.nointovlhack) {
        if (strstr(Tmp, "pow(") || strstr(Tmp, "pow (")) {
            Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), HackAltPow,
                                       Tmp, &tmpsize);
        }
        if (strstr(Tmp, "max(") || strstr(Tmp, "max (")) {
            Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), HackAltMax,
                                       Tmp, &tmpsize);
        }
        if (strstr(Tmp, "min(") || strstr(Tmp, "min (")) {
            Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), HackAltMin,
                                       Tmp, &tmpsize);
        }
        if (strstr(Tmp, "clamp(") || strstr(Tmp, "clamp (")) {
            Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                                       HackAltClamp, Tmp, &tmpsize);
        }
        if (strstr(Tmp, "mod(") || strstr(Tmp, "mod (")) {
            Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), HackAltMod,
                                       Tmp, &tmpsize);
        }
    }
    ...
    if (isVertex) {
        ...
        if (strstr(Tmp, "ftransform(")) {
            ...
            // don't increment headline count, as all variying and attributes
        ...
        for (int i = 0; i < n; i++) {
            if (strstr(Tmp, builtin_attrib[i].glname)) {
                // ok, this attribute is used replace gl_name by _gl4es_ one
                ...
                Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline++), def,
                                           Tmp, &tmpsize);
            }
        }
        if (strstr(Tmp, gl_VertexAttrib)) {
            ...
            for (int i = 0; i < MAX_VATTRIB; ++i) {
                ...
                if (gl4es_find_string(Tmp, gl_VA[i])) {
                    ...
                    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, gl_VA[i],
                                                gl4es_VA[i]);
                    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline++),
                                               A, Tmp, &tmpsize);
                }
    ...
    if (strstr(Tmp, "gl_SecondaryColor") || need->need_secondary) {
        ...
        Tmp = gl4es_inplace_replace(
            ...
            (need->need_secondary == 1)
                ...
                : "(gl_FrontFacing?gl_FrontSecondaryColor:gl_"
                  "BackSecondaryColor)");
    }
    if (strstr(Tmp, "gl_FrontSecondaryColor") || need->need_secondary) {
        ...
        nvarying += 1;
        Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                                   gl4es_frontSecondaryColorSource, Tmp,
                                   &tmpsize);
        ...
        if (notexarray) {
            for (int k = 0; k < ntex + 1; k++) {
                ...
                if (strstr(Tmp, d2)) {
                    ...
                    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), d,
                                               Tmp, &tmpsize);
    ...
    // builtin matrices work
    {
        if (strstr(Tmp, "transpose(") || strstr(Tmp, "transpose ") ||
            ...
            Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                                       gl4es_transpose, Tmp, &tmpsize);
            Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "transpose",
                                        "gl4es_transpose");
            // don't increment headline count, as all variying and attributes
            // should be created before
        }
        // check for builtin matrix uniform...
        {
            ...
            int ntex = -1;
            // Try to determine max Texture matrice used, for each transposed
            // inverse or regular...
            for (int i = 0; i < 4; ++i) {
                char *p = Tmp;
            ...
            for (int i = 0; i < n; i++) {
                if (strstr(Tmp, builtin_matrix[i].glname)) {
                    // ok, this matrix is used replace gl_name by _gl4es_ one
                    Tmp = gl4es_inplace_replace(Tmp, &tmpsize,
    ...
    // FragShader ? cleaning up the "centroid" keyword...
    ...
    if (strstr(Tmp, "gl_LightSourceParameters") ||
        ...
        Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                                   gl4es_LightSourceParametersSource, Tmp,
                                   &tmpsize);
    ...
    if (strstr(Tmp, "gl_LightModelParameters") ||
        strstr(Tmp, "gl_LightModel")) {
        Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                                   gl4es_LightModelParametersSource, Tmp,
                                   &tmpsize);
    ...
    if (strstr(Tmp, "gl_LightProducts") ||
        strstr(Tmp, "gl_FrontLightProduct") ||
    ...
    if (strstr(Tmp, "gl_MaxClipPlanes")) {
        Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 2),
                                   gl4es_MaxClipPlanesSource, Tmp, &tmpsize);
    ...
    if (strstr(Tmp, "gl_ClipVertex")) {
        ...
        if (p) {
            ...
            Tmp = gl4es_inplace_insert(p + 1, gl4es_ClipVertex_clip, Tmp,
                                       &tmpsize);
        }
    }
    // oldprogram uniforms...
    if (gl4es_find_string(Tmp, gl_ProgramEnv)) {
        ...
        if (noarray_ok) {
            ...
            for (int i = 0; i <= maxind; ++i) {
                ...
                sprintf(T, "_gl4es_%s_ProgramEnv_%d",
                ...
                if (gl4es_find_string(Tmp, T)) {
                    ...
                    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), U,
                                               Tmp, &tmpsize);
                    headline += 1;
                }
            }
            ...
            // need the full array...
            char T[60], U[300];
            sprintf(T, "_gl4es_%s_ProgramEnv",
    ...
    if (gl4es_find_string(Tmp, gl_ProgramLocal)) {
        ...
        if (noarray_ok) {
            ...
            // need the full array...
            char T[60], U[300];
            sprintf(T, "_gl4es_%s_ProgramLocal",
                    isVertex ? "Vertex" : "Fragment");
            sprintf(
                U, "uniform vec4 %s[%d];\n", T,
...
#undef GO
    ...
    // non-square matrix handling the square one first
...
int isBuiltinMatrix(const char *name)
{
    ...
    for (int i = 0; i < n && ret == -1; i++) {
        if (strncmp(builtin_matrix[i].name, name,
                    strlen(builtin_matrix[i].name)) == 0) {
