...
void APIENTRY_GL4ES gl4es_glPushAttrib(GLbitfield mask)
{
    ...
    if (mask & GL_ENABLE_BIT) {
        ...
        for (i = 0; i < hardext.maxplanes; i++) {
            *(cur->clip_planes_enabled + i) =
                gl4es_glIsEnabled(GL_CLIP_PLANE0 + i);
        }
        ...
        cur->color_logic_op = gl4es_glIsEnabled(GL_COLOR_LOGIC_OP);
        // TODO: GL_INDEX_LOGIC_OP TODO: GL_MAP1_x TODO: GL_MAP2_x
        ...
        // TODO: GL_POLYGON_OFFSET_POINT TODO: GL_POLYGON_SMOOTH TODO:
        // GL_POLYGON_STIPPLE
    ...
    if (mask & GL_HINT_BIT) {
        gl4es_glGetIntegerv(GL_PERSPECTIVE_CORRECTION_HINT,
                            &cur->perspective_hint);
    ...
    if (mask & GL_LIGHTING_BIT) {
        ...
        gl4es_glGetIntegerv(GL_LIGHT_MODEL_TWO_SIDE,
                            &cur->light_model_two_side);
...
#undef M
    ...
    if (mask & GL_PIXEL_MODE_BIT) {
        ...
        // TODO: GL_DEPTH_BIAS & GL_DEPTH_SCALE (probably difficult) TODO:
        // GL_INDEX_OFFEST & GL_INDEX_SHIFT TODO: GL_MAP_COLOR & GL_MAP_STENCIL
        // (probably difficult too)
    ...
    // TODO: GL_POLYGON_BIT TODO: GL_POLYGON_STIPPLE_BIT
    ...
    if (mask & GL_STENCIL_BUFFER_BIT) {
        ...
        gl4es_glGetIntegerv(GL_STENCIL_VALUE_MASK,
                            (GLint *) &cur->stencil_mask);
    ...
    if (mask & GL_TEXTURE_BIT) {
        ...
        for (a = 0; a < hardext.maxtex; a++) {
            ...
            cur->texgen[a] =
                glstate
                    ->texgen[a];  // all mode and planes per texture in 1 line
...
void APIENTRY_GL4ES gl4es_glPopAttrib(void)
{
    ...
    if (cur->mask & GL_ENABLE_BIT) {
        ...
        // TODO: GL_POLYGON_OFFSET_POINT TODO: GL_POLYGON_SMOOTH TODO:
        // GL_POLYGON_STIPPLE
        enable_disable(GL_SAMPLE_ALPHA_TO_COVERAGE,
                       cur->sample_alpha_to_coverage);
    ...
    if (cur->mask & GL_LIGHTING_BIT) {
        ...
        // Light position / direction is transformed. So load identity in
        // modelview to restore correct stuff
...
#undef M
    ...
    if (cur->mask & GL_POINT_BIT) {
        ...
        if (hardext.pointsprite) {
            ...
            int a;
            for (a = 0; a < hardext.maxtex; a++) {
                if (glstate->texture.pscoordreplace[a] !=
    ...
    if (cur->mask & GL_STENCIL_BUFFER_BIT) {
        enable_disable(GL_STENCIL_TEST, cur->stencil_test);
        gl4es_glStencilFunc(cur->stencil_func, cur->stencil_ref,
                            cur->stencil_mask);
        // TODO: Stencil value mask
    ...
    if (cur->mask & GL_TEXTURE_BIT) {
        ...
        for (a = 0; a < hardext.maxtex; a++) {
            ...
            for (int j = 0; j < ENABLED_TEXTURE_LAST; j++)
                if (cur->texture[a][j] !=
                    glstate->texture.bound[a][j]->texture) {
                    if (glstate->texture.active != a)
                        gl4es_glActiveTexture(GL_TEXTURE0 + a);
    ...
    if (cur->mask & GL_TRANSFORM_BIT) {
        if (!(cur->mask & GL_ENABLE_BIT)) {
            int i;
            for (i = 0; i < hardext.maxplanes; i++) {
                enable_disable(GL_CLIP_PLANE0 + i,
                               *(cur->clip_planes_enabled + i));
            }
        }
