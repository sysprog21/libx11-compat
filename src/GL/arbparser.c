...
int compareTokenWith(const sCurStatus *curStatus,
                     const char *compWith,
                     const int len)
{
...
eToken readNextToken(sCurStatus *curStatus)
{
    ...
    case '9':
        ...
        while ((*curStatus->endOfToken >= '0') &&
            ...
            curStatus->tokInt =
                curStatus->tokInt * 10 + *curStatus->endOfToken - '0';
            ++curStatus->endOfToken;
        }
        ...
        if ((*curStatus->endOfToken == 'e') ||
            (*curStatus->endOfToken == 'E')) {
            curStatus->curToken = TOK_FLOATCONST;
            ++curStatus->endOfToken;
    ...
    case '$':
        ...
        while (((*curStatus->endOfToken >= '0') &&
                (*curStatus->endOfToken <= '9')) ||
               ((*curStatus->endOfToken >= 'A') &&
                (*curStatus->endOfToken <= 'Z')) ||
               ((*curStatus->endOfToken >= 'a') &&
                (*curStatus->endOfToken <= 'z')) ||
               (*curStatus->endOfToken == '_') ||
               (*curStatus->endOfToken == '$')) {
            ++curStatus->endOfToken;
        }
...
int resolveAttrib(sCurStatus_NewVar *newVar, int vertex)
{
    ...
    if (!tok) {
        ...
        if (!tok) {
            ...
            if (!tok) {
                ARBCONV_DBG_RE(
                    "Failed to get attrib: vertex.attrib(tok NULL)\n")
                return 1;
        ...
        if (!tok) {
            ...
            if (!IS_NONE_OR_SWIZZLE) {
                ...
                if ((tok[0] == '[') && (newVar->strLen >= 2) &&
                    ...
                    ARBCONV_DBG_RE(
                        "Failed to get attrib: fragment.texcoord.%s\n", tok)
                    free(tok);
                    return 1;
                }
            } else {
                pushArray((sArray *) &newVar->var->init,
                          strdup("gl_TexCoord[0]"));
    ...
    /* TODO: (* todo, V done, X unsupported) V "vertex" "." "position" X
     * "vertex" "." "weight" <vtxOptWeightNum> V "vertex" "." "normal" V
     * "vertex" "." "color" V "vertex" "." "color" "." "primary" V "vertex" "."
     * "color" "." "secondary" V "vertex" "." "fogcoord" V "vertex" "."
     * "texcoord" V "vertex" "." "texcoord" "[" <texCoordNum> "]" "vertex" "."
     * "matrixindex" "[" <vtxWeightNum> "]" V "vertex" "." "attrib" "["
     * <vtxAttribNum> "]"
     *
     * V "fragment" "." "color" V "fragment" "." "color" "." "primary" V
     * "fragment" "." "color" "." "secondary" V "fragment" "." "texcoord" V
     * "fragment" "." "texcoord" "[" <texCoordNum> "]" V "fragment" "."
     * "fogcoord" V "fragment" "." "position"
     */

    return 0;
}
int resolveOutput(sCurStatus_NewVar *newVar,
                  int vertex,
                  struct sSpecialCases *specialCases)
{
        ...
        if (!tok) {
            ...
            if (!strcmp(tok, "front")) {
                ...
                if (!strcmp(tok, "primary")) {
                    ...
                    ARBCONV_DBG_RE(
                        "Failed to get output: result.color.front.%s\n", tok)
                    free(tok);
                    return 1;
                }
                ...
                if (!strcmp(tok, "primary")) {
                    ...
                    ARBCONV_DBG_RE(
                        "Failed to get output: result.color.back.%s\n", tok)
                    free(tok);
                    return 1;
                }
    ...
    /* TODO: (V done) V "result" "." "position" V "result" "." "color" V
     * "result" "." "color" "." "primary" V "result" "." "color" "." "secondary"
     * V "result" "." "color" "." "front" V "result" "." "color" "." "front" "."
     * "primary" V "result" "." "color" "." "front" "." "secondary" V "result"
     * "." "color" "." "back" V "result" "." "color" "." "back" "." "primary" V
     * "result" "." "color" "." "back" "." "secondary" V "result" "." "fogcoord"
     * V "result" "." "pointsize" V "result" "." "texcoord" V "result" "."
     * "texcoord" "[" <texCoordNum> "]"
     *
     * V "result" "." "color" V "result" "." "depth"
     */
...
char **resolveParam(sCurStatus_NewVar *newVar, int vertex, int type)
{
    ...
    if (!tok) {
        ...
        if (!tok) {
            ...
            if (!tok) {
                ARBCONV_DBG_RE(
                    "Failed to get param: state.material(tok NULL)\n")
                return NULL;
            ...
            if (!tok) {
                ...
                free(tok);
                matrixNameMallocd =
                    (char *) malloc((mtxNameLen + 17) * sizeof(char));
            ...
            if (newVar->strParts[0][0] == '[') {
                ...
                if ((sln[0] >= '0') && (sln[0] <= '9')) {
                    if (newVar->strParts[0][0] != ']') {
                        ARBCONV_DBG_RE(
                            "Failed to get param: state.light[%s(not ])\n", sln)
                        return NULL;
                    }
                    free(popFIFO((sArray *) newVar));
                    ...
                    if (!tok) {
                        ARBCONV_DBG_RE(
                            "Failed to get param: state.light[%s](tok NULL)\n",
                            sln)
                        return NULL;
                    } else if (!strcmp(tok, "ambient")) {
                        free(tok);
                        ...
                        if (!tok) {
                            ARBCONV_DBG_RE(
                                "Failed to get param: state.light[%s].spot(tok "
                                "NULL)\n",
                                sln)
                            return NULL;
                        ...
                        ARBCONV_DBG_RE(
                            "Failed to get param: state.light[%s].%s\n", sln,
                            tok)
                        free(tok);
                        return NULL;
                    }
                    ...
                    sprintf(matrixNameMallocd, "gl_LightSource[%s].%s", sln,
                            matrixName);
                    free(sln);
                    char **r = (char **) calloc(2, sizeof(char *));
                    r[0] = matrixNameMallocd;
            ...
            if (!tok) {
                ARBCONV_DBG_RE(
                    "Failed to get param: state.lightmodel(tok NULL)\n")
                return NULL;
            } else if (!strcmp(tok, "ambient")) {
                free(tok);
                char **r = (char **) calloc(2, sizeof(char *));
                ...
                if (!tok) {
                    ARBCONV_DBG_RE(
                        "Failed to get param: state.lightmodel.front(tok "
                        "NULL)\n")
                    return NULL;
                } else if (!strcmp(tok, "scenecolor")) {
                    free(tok);
                    ...
                    ARBCONV_DBG_RE(
                        "Failed to get param: state.lightmodel.front.%s\n", tok)
                    free(tok);
                    return NULL;
                }
            } else if (!strcmp(tok, "back")) {
                ...
                if (!tok) {
                    ARBCONV_DBG_RE(
                        "Failed to get param: state.lightmodel.back(tok "
                        "NULL)\n")
                    return NULL;
                } else if (!strcmp(tok, "scenecolor")) {
                    free(tok);
                    ...
                    ARBCONV_DBG_RE(
                        "Failed to get param: state.lightmodel.back.%s\n", tok)
                    free(tok);
                    return NULL;
                }
            } else {
            ...
            if (newVar->strParts[0][0] == '[') {
                ...
                if ((sln[0] >= '0') && (sln[0] <= '9')) {
                    ...
                    if (newVar->strParts[0][0] != ']') {
                        ARBCONV_DBG_RE(
                            "Failed to get param: state.lightprod[%s(not ])\n",
                            sln)
                        return NULL;
                    }
                    ...
                    if (!tok) {
                        ARBCONV_DBG_RE(
                            "Failed to get param: [%s][%s](tok NULL)\n",
                            matrixName, sln)
                        return NULL;
                    } else if (!strcmp(tok, "ambient")) {
                        free(tok);
                    ...
                    sprintf(matrixNameMallocd, "%s[%s].%s", matrixName, sln,
                            prop);
                    free(sln);
                    char **r = (char **) calloc(2, sizeof(char *));
                    ;
            ...
            if (!tok) {
                ...
                if (newVar->strLen &&
                    ...
                    if (newVar->strParts[0][0] == '[') {
                        ...
                        if (tok[0] != ']') {
                            ARBCONV_DBG_RE(
                                "Failed to get param: state.modelview[%d(not "
                                "])\n",
                                mvmtx)
                            free(tok);
                            return NULL;
                        ...
                        if (mvmtx != 0) {
                            ARBCONV_DBG_RE(
                                "Failed to get param: state.modelview[%d "
                                "(!=0)]\n",
                                mvmtx)
                            return NULL;
                        }
                    ...
                    /* if (mvmtxsz && mvmtx) { mtxNameLen += 2 + mvmtxsz;
                     * matrixNameMallocd = malloc((mtxNameLen + 1) *
                     * sizeof(char)); sprintf(matrixNameMallocd, "%s",
                     * matrixName); }
                     */
                } else {
                    matrixName = "gl_ModelViewMatrixTranspose";
                ...
                if (newVar->strLen &&
                    ...
                    if (newVar->strParts[0][0] == '[') {
                        ...
                        if (tok[0] != ']') {
                            ARBCONV_DBG_RE(
                                "Failed to get param: state.texture[%d(not "
                                "])\n",
                                mvmtx)
                            free(tok);
                            return NULL;
        ...
        if (!tok) {
            ...
            if (!tok) {
                ...
                if ((tok[0] >= '0') && (tok[0] <= '9')) {
                    ...
                    if (tok[0] != ']') {
                        ARBCONV_DBG_RE(
                            "Failed to get param: program.env[%d..%d(not ])\n",
                            start, end)
                        free(tok);
                        return NULL;
                    }
            ...
            if (!tok) {
                ...
                if ((tok[0] >= '0') && (tok[0] <= '9')) {
                    ...
                    if (tok[0] != ']') {
                        ARBCONV_DBG_RE(
                            "Failed to get param: program.local[%d..%d(not "
                            "])\n",
                            start, end)
                        free(tok);
                        return NULL;
    ...
    /* TODO: (* todo, V done, ! todo but only in vertex/fragment shaders, ?
     * todo?) <paramSingleItemDecl> \ V "state" "." "material" "." "ambient" \ V
     * "state" "." "material" "." "diffuse" \ V "state" "." "material" "."
     * "specular" \ V "state" "." "material" "." "emission" \ V "state" "."
     * "material" "." "shininess" \ V "state" "." "material" "." "front" "."
     * "ambient" \ V "state" "." "material" "." "front" "." "diffuse" \ V
     * "state" "." "material" "." "front" "." "specular" \ V "state" "."
     * "material" "." "front" "." "emission" \ V "state" "." "material" "."
     * "front" "." "shininess" \ V "state" "." "material" "." "back" "."
     * "ambient" \ V "state" "." "material" "." "back" "." "diffuse" \ V "state"
     * "." "material" "." "back" "." "specular" \ V "state" "." "material" "."
     * "back" "." "emission" \ V "state" "." "material" "." "back" "."
     * "shininess" \ V "state" "." "light" "[" <stateLightNumber> "]" "."
     * "ambient" \ V "state" "." "light" "[" <stateLightNumber> "]" "."
     * "diffuse" \ V "state" "." "light" "[" <stateLightNumber> "]" "."
     * "specular" \ V "state" "." "light" "[" <stateLightNumber> "]" "."
     * "position" \ V "state" "." "light" "[" <stateLightNumber> "]" "."
     * "attenuation" \ V "state" "." "light" "[" <stateLightNumber> "]" "."
     * "spot" "." "direction" \ V "state" "." "light" "[" <stateLightNumber> "]"
     * "." "half" \ V "state" "." "lightmodel" "." "ambient" \ V "state" "."
     * "lightmodel" "." "scenecolor" \ V "state" "." "lightmodel" "." "front"
     * "." "scenecolor" \ V "state" "." "lightmodel" "." "back" "." "scenecolor"
     * \ V "state" "." "lightprod" "[" <stateLightNumber> "]" "." "ambient" \ V
     * "state" "." "lightprod" "[" <stateLightNumber> "]" "." "diffuse" \ V
     * "state" "." "lightprod" "[" <stateLightNumber> "]" "." "specular" \ V
     * "state" "." "lightprod" "[" <stateLightNumber> "]" "." "front" "."
     * "ambient" \ V "state" "." "lightprod" "[" <stateLightNumber> "]" "."
     * "front" "." "diffuse" \ V "state" "." "lightprod" "[" <stateLightNumber>
     * "]" "." "front" "." "specular" \ V "state" "." "lightprod" "["
     * <stateLightNumber> "]" "." "back" "." "ambient" \ V "state" "."
     * "lightprod" "[" <stateLightNumber> "]" "." "back" "." "diffuse" \ V
     * "state" "." "lightprod" "[" <stateLightNumber> "]" "." "back" "."
     * "specular" \ ! "state" "." "texgen" "." "eye" "." "s" \ ! "state" "."
     * "texgen" "." "eye" "." "t" \ ! "state" "." "texgen" "." "eye" "." "r" \ !
     * "state" "." "texgen" "." "eye" "." "q" \ ! "state" "." "texgen" "."
     * "object" "." "s" \ ! "state" "." "texgen" "." "object" "." "t" \ !
     * "state" "." "texgen" "." "object" "." "r" \ ! "state" "." "texgen" "."
     * "object" "." "q" \ ! "state" "." "texgen" "[" <texCoordNum> "]" "." "eye"
     * "." "s" \ ! "state" "." "texgen" "[" <texCoordNum> "]" "." "eye" "." "t"
     * \ ! "state" "." "texgen" "[" <texCoordNum> "]" "." "eye" "." "r" \ !
     * "state" "." "texgen" "[" <texCoordNum> "]" "." "eye" "." "q" \ ! "state"
     * "." "texgen" "[" <texCoordNum> "]" "." "object" "." "s" \ ! "state" "."
     * "texgen" "[" <texCoordNum> "]" "." "object" "." "t" \ ! "state" "."
     * "texgen" "[" <texCoordNum> "]" "." "object" "." "r" \ ! "state" "."
     * "texgen" "[" <texCoordNum> "]" "." "object" "." "q" \ * "state" "." "fog"
     * "." "color" \ * "state" "." "fog" "." "params" \ ! "state" "." "clip" "["
     * <stateClipPlaneNum> "]" "." "plane" \ ! "state" "." "point" "." "size" \
     * ! "state" "." "point" "." "attenuation" \ V "state" "." "matrix" "."
     * "modelview" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "."
     * "matrix" "." "modelview" "." "inverse" "." "row" "[" <stateMatrixRowNum>
     * "]" \ V "state" "." "matrix" "." "modelview" "." "transpose" "." "row"
     * "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "modelview" "."
     * "invtrans" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix"
     * "." "modelview" "[" <stateModMatNum> "]" "." "row" "["
     * <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "modelview" "["
     * <stateModMatNum> "]" "." "inverse" "." "row" "[" <stateMatrixRowNum> "]"
     * \ V "state" "." "matrix" "." "modelview" "[" <stateModMatNum> "]" "."
     * "transpose" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "."
     * "matrix" "." "modelview" "[" <stateModMatNum> "]" "." "invtrans" "."
     * "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix" "."
     * "projection" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "."
     * "matrix" "." "projection" "." "inverse" "." "row" "[" <stateMatrixRowNum>
     * "]" \ V "state" "." "matrix" "." "projection" "." "transpose" "." "row"
     * "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "projection" "."
     * "invtrans" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix"
     * "." "mvp" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix"
     * "." "mvp" "." "inverse" "." "row" "[" <stateMatrixRowNum> "]" \ V "state"
     * "." "matrix" "." "mvp" "." "transpose" "." "row" "[" <stateMatrixRowNum>
     * "]" \ V "state" "." "matrix" "." "mvp" "." "invtrans" "." "row" "["
     * <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "texture" "." "row"
     * "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "texture" "."
     * "inverse" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix"
     * "." "texture" "." "transpose" "." "row" "[" <stateMatrixRowNum> "]" \ V
     * "state" "." "matrix" "." "texture" "." "invtrans" "." "row" "["
     * <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "texture" "["
     * <texCoordNum> "]" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "."
     * "matrix" "." "texture" "[" <texCoordNum> "]" "." "inverse" "." "row" "["
     * <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "texture" "["
     * <texCoordNum> "]" "." "transpose" "." "row" "[" <stateMatrixRowNum> "]" \
     * V "state" "." "matrix" "." "texture" "[" <texCoordNum> "]" "." "invtrans"
     * "." "row" "[" <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "."
     * "palette" "[" <statePaletteMatNum> "]" "." "row" "[" <stateMatrixRowNum>
     * "]" \ ? "state" "." "matrix" "." "palette" "[" <statePaletteMatNum> "]"
     * "." "inverse" "." "row" "[" <stateMatrixRowNum> "]" \ ? "state" "."
     * "matrix" "." "palette" "[" <statePaletteMatNum> "]" "." "transpose" "."
     * "row" "[" <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "." "palette"
     * "[" <statePaletteMatNum> "]" "." "invtrans" "." "row" "["
     * <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "." "program" "["
     * <stateProgramMatNum> "]" "." "row" "[" <stateMatrixRowNum> "]" \ ?
     ...
     * "]" \ V "program" "." "env" "[" <progEnvParamNum> "]" \ V "program" "."
     * "local" "[" <progLocalParamNum> "]" \ V <optionalSign> <floatConstant> \
     * V "{" <optionalSign> <floatConstant> "}" \ V "{" <optionalSign>
     * <floatConstant> "," <optionalSign> <floatConstant> "}" \ V "{"
     ...
     *
     * <paramSingleItemUse> \ V "state" "." "material" "." "ambient" \ V "state"
     * "." "material" "." "diffuse" \ V "state" "." "material" "." "specular" \
     * V "state" "." "material" "." "emission" \ V "state" "." "material" "."
     * "shininess" \ V "state" "." "material" "." "front" "." "ambient" \ V
     * "state" "." "material" "." "front" "." "diffuse" \ V "state" "."
     * "material" "." "front" "." "specular" \ V "state" "." "material" "."
     * "front" "." "emission" \ V "state" "." "material" "." "front" "."
     * "shininess" \ V "state" "." "material" "." "back" "." "ambient" \ V
     * "state" "." "material" "." "back" "." "diffuse" \ V "state" "."
     * "material" "." "back" "." "specular" \ V "state" "." "material" "."
     * "back" "." "emission" \ V "state" "." "material" "." "back" "."
     * "shininess" \ V "state" "." "light" "[" <stateLightNumber> "]" "."
     * "ambient" \ V "state" "." "light" "[" <stateLightNumber> "]" "."
     ...
     * <stateProgramMatNum> "]" "." "invtrans" "." "row" "[" <stateMatrixRowNum>
     * "]" \ V "program" "." "env" "[" <progEnvParamNum> "]" \ V "program" "."
     * "local" "[" <progLocalParamNum> "]" \ * <floatConstant> \ V "{"
     * <optionalSign> <floatConstant> "}" \ V "{" <optionalSign> <floatConstant>
     * "," <optionalSign> <floatConstant> "}" \ V "{" <optionalSign>
     * <floatConstant> "," <optionalSign> <floatConstant> "," <optionalSign>
     * <floatConstant> "}" \ V "{" <optionalSign> <floatConstant> ","
     * <optionalSign> <floatConstant> "," <optionalSign> <floatConstant> ","
     * <optionalSign> <floatConstant> "}"
     *
     * <paramMultipleItem> \ V "state" "." "material" "." "ambient" \ V "state"
     * "." "material" "." "diffuse" \ V "state" "." "material" "." "specular" \
     * V "state" "." "material" "." "emission" \ V "state" "." "material" "."
     * "shininess" \ V "state" "." "material" "." "front" "." "ambient" \ V
     * "state" "." "material" "." "front" "." "diffuse" \ V "state" "."
     ...
     * <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "." "program" "["
     * <stateProgramMatNum> "]" "." "invtrans" "." "row" "[" <stateMatrixRowNum>
     * "]" \ V "state" "." "matrix" "." "modelview" \ V "state" "." "matrix" "."
     * "modelview" "." "inverse" \ V "state" "." "matrix" "." "modelview" "."
     * "transpose" \ V "state" "." "matrix" "." "modelview" "." "invtrans" \ V
     * "state" "." "matrix" "." "modelview" "." "row" "[" <stateMatrixRowNum>
     * ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "modelview" "."
     * "inverse" "." "row" "[" <stateMatrixRowNum> ".." <stateMatrixRowNum> "]"
     * \ V "state" "." "matrix" "." "modelview" "." "transpose" "." "row" "["
     ...
     * <stateModMatNum> "]" "." "inverse" \ V "state" "." "matrix" "."
     * "modelview" "[" <stateModMatNum> "]" "." "transpose" \ V "state" "."
     * "matrix" "." "modelview" "[" <stateModMatNum> "]" "." "invtrans" \ V
     * "state" "." "matrix" "." "modelview" "[" <stateModMatNum> "]" "." "row"
     * "[" <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "."
     * "matrix" "." "modelview" "[" <stateModMatNum> "]" "." "inverse" "." "row"
     * "[" <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "."
     * "matrix" "." "modelview" "[" <stateModMatNum> "]" "." "transpose" "."
     * "row" "[" <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state"
     * "." "matrix" "." "modelview" "[" <stateModMatNum> "]" "." "invtrans" "."
     * "row" "[" <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state"
     * "." "matrix" "." "projection" \ V "state" "." "matrix" "." "projection"
     * "." "inverse" \ V "state" "." "matrix" "." "projection" "." "transpose" \
     * V "state" "." "matrix" "." "projection" "." "invtrans" \ V "state" "."
     * "matrix" "." "projection" "." "row" "[" <stateMatrixRowNum> ".."
     * <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "projection" "."
     * "inverse" "." "row" "[" <stateMatrixRowNum> ".." <stateMatrixRowNum> "]"
     * \ V "state" "." "matrix" "." "projection" "." "transpose" "." "row" "["
     * <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix"
     * "." "projection" "." "invtrans" "." "row" "[" <stateMatrixRowNum> ".."
     * <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "mvp" \ V "state"
     * "." "matrix" "." "mvp" "." "inverse" \ V "state" "." "matrix" "." "mvp"
     * "." "transpose" \ V "state" "." "matrix" "." "mvp" "." "invtrans" \ V
     * "state" "." "matrix" "." "mvp" "." "row" "[" <stateMatrixRowNum> ".."
     * <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "mvp" "." "inverse"
     * "." "row" "[" <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V
     * "state" "." "matrix" "." "mvp" "." "transpose" "." "row" "["
     * <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix"
     * "." "mvp" "." "invtrans" "." "row" "[" <stateMatrixRowNum> ".."
     * <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "texture" \ V
     * "state" "." "matrix" "." "texture" "." "inverse" \ V "state" "." "matrix"
     ...
     * <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix"
     * "." "texture" "[" <texCoordNum> "]" \ V "state" "." "matrix" "."
     * "texture" "[" <texCoordNum> "]" "." "inverse" \ V "state" "." "matrix"
     * "." "texture" "[" <texCoordNum> "]" "." "transpose" \ V "state" "."
     * "matrix" "." "texture" "[" <texCoordNum> "]" "." "invtrans" \ V "state"
     * "." "matrix" "." "texture" "[" <texCoordNum> "]" "." "row" "["
     * <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix"
     * "." "texture" "[" <texCoordNum> "]" "." "inverse" "." "row" "["
     * <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix"
     * "." "texture" "[" <texCoordNum> "]" "." "transpose" "." "row" "["
     * <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix"
     * "." "texture" "[" <texCoordNum> "]" "." "invtrans" "." "row" "["
     * <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix"
     * "." "palette" "[" <statePaletteMatNum> "]" \ ? "state" "." "matrix" "."
     * "palette" "[" <statePaletteMatNum> "]" "." "inverse" \ ? "state" "."
     * "matrix" "." "palette" "[" <statePaletteMatNum> "]" "." "transpose" \ ?
     * "state" "." "matrix" "." "palette" "[" <statePaletteMatNum> "]" "."
     * "invtrans" \ ? "state" "." "matrix" "." "palette" "["
     * <statePaletteMatNum> "]" "." "row" "[" <stateMatrixRowNum> ".."
     * <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "." "palette" "["
     * <statePaletteMatNum> "]" "." "inverse" "." "row" "[" <stateMatrixRowNum>
     * ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "." "palette" "["
     * <statePaletteMatNum> "]" "." "transpose" "." "row" "["
     * <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix"
     * "." "palette" "[" <statePaletteMatNum> "]" "." "invtrans" "." "row" "["
     * <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix"
     * "." "program" "[" <stateProgramMatNum> "]" \ ? "state" "." "matrix" "."
     * "program" "[" <stateProgramMatNum> "]" "." "inverse" \ ? "state" "."
     * "matrix" "." "program" "[" <stateProgramMatNum> "]" "." "transpose" \ ?
     * "state" "." "matrix" "." "program" "[" <stateProgramMatNum> "]" "."
     * "invtrans" \ ? "state" "." "matrix" "." "program" "["
     * <stateProgramMatNum> "]" "." "row" "[" <stateMatrixRowNum> ".."
     * <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "." "program" "["
     * <stateProgramMatNum> "]" "." "inverse" "." "row" "[" <stateMatrixRowNum>
     * ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "." "program" "["
     * <stateProgramMatNum> "]" "." "transpose" "." "row" "["
     * <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix"
     ...
     * ".." <progEnvParamNum> "]" \ V "program" "." "local" "["
     * <progLocalParamNum> "]" \ V "program" "." "local" "[" <progLocalParamNum>
     * ".." <progLocalParamNum> "]" \ V <optionalSign> <floatConstant> \ V "{"
     * <optionalSign> <floatConstant> "}" \ V "{" <optionalSign> <floatConstant>
     * "," <optionalSign> <floatConstant> "}" \ V "{" <optionalSign>
     * <floatConstant> "," <optionalSign> <floatConstant> "," <optionalSign>
     * <floatConstant> "}" \ V "{" <optionalSign> <floatConstant> ","
     ...
     *
     * <progEnvParamNum> \ * <integer> from 0 to MAX_PROGRAM_ENV_PARAMETERS_ARB
     * - 1
     *
     * <progLocalParamNum> \ * <integer> from 0 to
     * MAX_PROGRAM_LOCAL_PARAMETERS_ARB - 1
     *
     * <paramSingleItemDecl> \ V "state" "." "material" "." "ambient" \ V
     * "state" "." "material" "." "diffuse" \ V "state" "." "material" "."
     * "specular" \ V "state" "." "material" "." "emission" \ V "state" "."
     * "material" "." "shininess" \ V "state" "." "material" "." "front" "."
     * "ambient" \ V "state" "." "material" "." "front" "." "diffuse" \ V
     ...
     * "lightprod" "[" <stateLightNumber> "]" "." "back" "." "diffuse" \ V
     * "state" "." "lightprod" "[" <stateLightNumber> "]" "." "back" "."
     * "specular" \ ! "state" "." "texenv" "." "color" \ ! "state" "." "texenv"
     * "[" <legacyTexUnitNum> "]" "." "color" \ * "state" "." "fog" "." "color"
     * \ * "state" "." "fog" "." "params" \ ! "state" "." "depth" "." "range" \
     ...
     * "]" \ V "state" "." "matrix" "." "modelview" "[" <stateModMatNum> "]" "."
     * "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix" "."
     * "modelview" "[" <stateModMatNum> "]" "." "inverse" "." "row" "["
     * <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "modelview" "["
...
#define FAIL(str)                    \
    ...
    *error_msg = strdup(str);        \
    return
void parseToken(sCurStatus *curStatusPtr,
                int vertex,
                char **error_msg,
                struct sSpecialCases *specialCases)
{
    ...
    // TODO: replace '$' in variable names with something else that is also
    // valid in GLSL
    ...
    case ST_LINE_START:
        ...
        case TOK_IDENTIFIER: {
            ...
            eVariableType vtype;
            if ((inst = STR2INST(
                     tok, &curStatusPtr->curValue.newInst.inst.saturated)) !=
                INST_UNK) {
                if (INSTTEX(inst) && vertex) {
                    FAIL(
                        "Texture instructions are only valid in fragment "
                        "shaders");
                }
                if (vertex && curStatusPtr->curValue.newInst.inst.saturated) {
                    FAIL(
                        "Instruction cannot be saturated in ARB vertex "
                        "shaders");
                }
                ...
                for (int i = 0; i < MAX_OPERANDS; ++i) {
                    ...
                    curStatusPtr->curValue.newInst.inst.vars[i].floatArrAddr =
                        NULL;
                    curStatusPtr->curValue.newInst.inst.vars[i].sign = 0;
                    curStatusPtr->curValue.newInst.inst.vars[i].swizzle[0] =
                        SWIZ_NONE;
                    curStatusPtr->curValue.newInst.inst.vars[i].swizzle[1] =
                        SWIZ_NONE;
                    curStatusPtr->curValue.newInst.inst.vars[i].swizzle[2] =
                        SWIZ_NONE;
                    curStatusPtr->curValue.newInst.inst.vars[i].swizzle[3] =
                        SWIZ_NONE;
                }
        ...
        case VARTYPE_TEMP:
            ...
            case TOK_IDENTIFIER: {
                ...
                if (!strcmp(tok, "half")) {
                    ...
                    // Hopefully this doesn't make a free-after-free in case of
                    // error (though it shouldn't)
                    int ret;
                    khint_t varIdx =
                        kh_put(variables, curStatusPtr->varsMap, tok, &ret);
                    if (ret < 0) {
                        FAIL("Unknown error");
                    }
    ...
    case ST_VARIABLE_INIT:
        ...
        case VARTYPE_ATTRIB:
            ...
            case TOK_END_OF_INST: {
                ...
                if ((tok =
                         popFIFO((sArray *) &curStatusPtr->curValue.newVar))) {
                    free(tok);
                    FAIL("Not a valid attribute");
                }
        ...
        case VARTYPE_PARAM:
            ...
            case TOK_INTEGER:
                /* ... 0 works, too, in theory if
                 * (curStatusPtr->curValue.newVar.state != 2) { FAIL("Invalid
                 * state"); }
                 */
            ...
            case TOK_END_OF_INST: {
                ...
                char **param =
                    resolveParam(&curStatusPtr->curValue.newVar, vertex, 0);
                ...
                for (char **parm = param; *parm; ++parm) {
                    pushArray(
                        (sArray *) &curStatusPtr->curValue.newVar.var->init,
                        *parm);
                }
                free(param);
...
#define STATE_AFTER_SWIZZLE -1
            ...
            case STATE_START:
                if (texSampler) {
                    if ((curStatusPtr->tokInt < 1) ||
                        (curStatusPtr->tokInt > 3)) {
                        FAIL("Invalid texture sampler");
                    }
                    curStatusPtr->curValue.newInst.state =
                        STATE_AFTER_TEXSPLINT;
                    break;
                }
        ...
        case TOK_IDENTIFIER: {
            ...
            case STATE_AFTER_VALID_LSQBR_START: {
                ...
                if (!kh_truly_exist(curStatusPtr->varsMap, idx)) {
                    free(tok);
                    FAIL(
                        "Invalid relative addressing (not a declared address)");
                }
            ...
            case STATE_AFTER_TEXSPLINT:
                if (texSampler) {
                    if ((getTokenLength(curStatusPtr) != 1) ||
                        ...
                        default:
                            FAIL(
                                "Invalid texture sampler (shouldn't happen "
                                "here)");
                            break;
                        }
                    }
        ...
        case TOK_END_OF_INST:
            if (INSTTEX(curStatusPtr->curValue.newInst.inst.type)) {
                ...
                if (curStatusPtr->curValue.newInst.state !=
                    STATE_AFTER_TEXSAMPLER) {
            ...
            case STATE_RBRACE:        // directly to the resolveParam part...
                if (INSTTEX(curStatusPtr->curValue.newInst.inst.type) &&
                    ...
                    if ((curStatusPtr->_fixedNewVar.strLen != 4) ||
                        ...
                        strcmp(curStatusPtr->_fixedNewVar.strParts[0],
                               "texture")) {
                        FAIL("Invalid texture instruction");
                    }
                    ...
                    if (!curStatusPtr->texVars[id]->size) {
                        pushArray(
                            (sArray *) curStatusPtr->texVars[id],
                    ...
                    if ((vertex &&
                        ...
                        (!vertex &&
                         !strcmp(curStatusPtr->_fixedNewVar.strParts[0],
                                 "fragment"))) {
                        failure =
                            resolveAttrib(&curStatusPtr->_fixedNewVar, vertex);
                    } else if (!strcmp(curStatusPtr->_fixedNewVar.strParts[0],
                                       "result")) {
                        failure = resolveOutput(&curStatusPtr->_fixedNewVar,
                                                vertex, specialCases);
                    } else {
                        char **resolved = resolveParam(
                            &curStatusPtr->_fixedNewVar, vertex, 1);
                    ...
                    if (curStatusPtr->_fixedNewVar.strLen) {
                        if (!IS_SWIZZLE(
                                curStatusPtr->_fixedNewVar.strParts[0]) ||
                        ...
                        int e = 0;
                        char *swiz =
                            popArray((sArray *) &curStatusPtr->_fixedNewVar);
