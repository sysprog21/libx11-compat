...
void APIENTRY_GL4ES gl4es_glBlendColor(GLclampf red,
                                       GLclampf green,
                                       GLclampf blue,
                                       GLclampf alpha)
{
                                              ...
                                              GLenum dfactorAlpha)
{
    ...
    if (globals4es.shaderblend) {
        ...
        if (gles_glBlendFuncSeparate == NULL) {
            // some fallback function to have better rendering with SDL2, better
            // then nothing...
            if (sfactorRGB == GL_SRC_ALPHA &&
                dfactorRGB == GL_ONE_MINUS_SRC_ALPHA &&
                sfactorAlpha == GL_ONE &&
                dfactorAlpha == GL_ONE_MINUS_SRC_ALPHA)
...
void APIENTRY_GL4ES gl4es_glBlendFunc(GLenum sfactor, GLenum dfactor)
{
    ...
    if (globals4es.shaderblend) {
        ...
        if ((globals4es.blendhack) && (sfactor == GL_SRC_ALPHA) &&
            (dfactor == GL_ONE)) {
            // special case, as seen in Xash3D, but it breaks torus_trooper, so
            // behind a parameter
            sfactor = GL_ONE;
        }
