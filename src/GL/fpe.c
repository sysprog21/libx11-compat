...
void APIENTRY_GL4ES fpe_ReleventState(fpe_state_t *dest,
                                      fpe_state_t *src,
                                      int fixed)
{
    // filter out some non relevant state (like texture stuff if texture is
...
uniform_t *findUniform(khash_t(uniformlist) * uniforms, const char *name)
{
    ...
    kh_foreach(uniforms, k, m,
               if (!strcmp(m->name, name)) return m;) return NULL;
}
...
void APIENTRY_GL4ES fpe_oldprogram(fpe_state_t *state)
{
    ...
    if (state->vertex_prg_id) {
        ...
        if (status != GL_TRUE) {
            ...
            if (globals4es.logshader)
                printf(
                    "LIBGL: FPE ARB Vertex program compile failed: ARB source "
                    "is\n%s\n=======\nGLSL source is\n%s\nError is: %s\n",
        ...
        gl4es_glShaderSource(glstate->fpe->vert, 1,
                             fpe_VertexShader(&old_frg->shader->need, state),
                             NULL);
        gl4es_glCompileShader(glstate->fpe->vert);
        gl4es_glGetShaderiv(glstate->fpe->vert, GL_COMPILE_STATUS, &status);
        if (status != GL_TRUE) {
    ...
    if (state->fragment_prg_id) {
        ...
        if (status != GL_TRUE) {
            ...
            if (globals4es.logshader)
                printf(
                    "LIBGL: FPE ARB Fragment program compile failed: ARB "
                    "source is\n%s\n=======\nGLSL source is\n%s\nError is: "
                    "%s\n",
        ...
        if (status != GL_TRUE) {
            ...
            gles_glGetShaderInfoLog(glstate->fpe->frag, 1000, NULL, buff);
            printf(
                "LIBGL: FPE ARB Default Fragment program compile failed: %s\n",
                buff);
        }
    }
    ...
    if (status != GL_TRUE) {
        ...
        if (globals4es.logshader)
            printf(
                "LIBGL: FPE ARB Program link failed: %s\n with vertex "
                "%s%s%s%s%s and fragment %s%s%s%s%s\n",
    ...
    if (glstate->fpe->glprogram == NULL) {
        ...
        if (fpe_GetProgramPSA(glstate->fpe->prog, &state) == 0) {
            ...
            if (state.vertex_prg_id || state.fragment_prg_id) {
                ...
                glstate->fpe->vert = gl4es_glCreateShader(GL_VERTEX_SHADER);
                gl4es_glShaderSource(glstate->fpe->vert, 1,
                                     fpe_VertexShader(NULL, glstate->fpe_state),
                                     NULL);
                gl4es_glCompileShader(glstate->fpe->vert);
                gl4es_glGetShaderiv(glstate->fpe->vert, GL_COMPILE_STATUS,
                                    &status);
                if (status != GL_TRUE) {
                    char buff[1000];
                    gles_glGetShaderInfoLog(glstate->fpe->vert, 1000, NULL,
                                            buff);
                    if (globals4es.logshader)
                        printf(
                            "LIBGL: FPE Vertex shader compile failed: source "
                            "is\n%s\n\nError is: %s\n",
                ...
                gl4es_glCompileShader(glstate->fpe->frag);
                gl4es_glGetShaderiv(glstate->fpe->frag, GL_COMPILE_STATUS,
                                    &status);
                if (status != GL_TRUE) {
                    char buff[1000];
                    gles_glGetShaderInfoLog(glstate->fpe->frag, 1000, NULL,
                                            buff);
                    if (globals4es.logshader)
                        printf(
                            "LIBGL: FPE Fragment shader compile failed: source "
                            "is\n%s\n\nError is: %s\n",
                            fpe_FragmentShader(NULL, glstate->fpe_state)[0],
                            buff);
                    else
                        printf(
                            "LIBGL: FPE Fragment shader compile failed: %s\n",
                            buff);
                }
                ...
                gl4es_glAttachShader(glstate->fpe->prog, glstate->fpe->frag);
                gl4es_glLinkProgram(glstate->fpe->prog);
                gl4es_glGetProgramiv(glstate->fpe->prog, GL_LINK_STATUS,
                                     &status);
                if (status != GL_TRUE) {
                    char buff[1000];
                    gles_glGetProgramInfoLog(glstate->fpe->prog, 1000, NULL,
                                             buff);
                    if (globals4es.logshader) {
                        printf(
                            "LIBGL: FPE Program link failed: source of vertex "
                            "shader is\n%s\n\n",
                            fpe_VertexShader(NULL, glstate->fpe_state)[0]);
                        printf(
                                           ...
                                           fpe_state_t *state)
{
    ...
    // but kh'thingy is initialized) TODO: what if program is composed of more
    // then 1 vertex or fragment shader?
    fpe_fpe_t *fpe =
        fpe_GetCache((fpe_cache_t *) glprogram->fpe_cache, state, 0);
    if (fpe->glprogram == NULL) {
        ...
        gl4es_glShaderSource(
            fpe->frag, 1,
            fpe_CustomFragmentShader(glprogram->last_frag->source, state),
            NULL);
        ...
        {
            attribloc_t *al;
            LOAD_GLES2(glBindAttribLocation);  // using real one to avoid
                                               // overwriting of attribloc...
            kh_foreach_value(
...
program_t *APIENTRY_GL4ES
fpe_CustomShader_DefaultVertex(program_t *glprogram, fpe_state_t *state_vertex)
{
    // state is not empty and glprogram already has some cache (it may be empty,
    // but kh'thingy is initialized) TODO: what if program is composed of more
    // then 1 vertex or fragment shader?
    ...
    if (fpe->glprogram == NULL) {
        ...
        gl4es_glShaderSource(
            fpe->vert, 1,
            fpe_VertexShader(glprogram->default_need, state_vertex), NULL);
        gl4es_glCompileShader(fpe->vert);
        gl4es_glGetShaderiv(fpe->vert, GL_COMPILE_STATUS, &status);
        if (status != GL_TRUE) {
            char buff[1000];
        ...
        gl4es_glShaderSource(fpe->frag, 1,
                             fpe_CustomFragmentShader(
                                 glprogram->last_frag->source, state_vertex),
                             NULL);
        gl4es_glCompileShader(fpe->frag);
        gl4es_glGetShaderiv(fpe->frag, GL_COMPILE_STATUS, &status);
        if (status != GL_TRUE) {
        ...
        if (status != GL_TRUE) {
            ...
            gl4es_glGetProgramInfoLog(fpe->prog, 1000, NULL, buff);
            printf(
                "LIBGL: FPE Custom Program with Default Vertex link failed: "
                "%s\n",
                buff);
            return glprogram;  // fallback to non-customized custom program..
        }
                                     ...
                                     program_t *glprogram)
{
    ...
    kh_foreach(
        uniforms, k, m, if (m->parent_size) {
            ...
            case GL_FLOAT_VEC4:
                GoUniformfv(
                    glprogram, m->id, n_uniform(m->type), 1,
            ...
            default:
                printf(
                    "LIBGL: Warning, sync uniform on father/son program with "
...
void APIENTRY_GL4ES
fpe_glMultiTexCoord4f(GLenum target, GLfloat s, GLfloat t, GLfloat r, GLfloat q)
{
}

void APIENTRY_GL4ES fpe_glSecondaryColorPointer(GLint size,
                                                GLenum type,
                                                GLsizei stride,
...
void APIENTRY_GL4ES fpe_glVertexPointer(GLint size,
                                        GLenum type,
                                        GLsizei stride,
                                        const GLvoid *pointer)
{
...
void APIENTRY_GL4ES fpe_glColorPointer(GLint size,
                                       GLenum type,
                                       GLsizei stride,
                                       const GLvoid *pointer)
{
...
void APIENTRY_GL4ES fpe_glNormalPointer(GLenum type,
                                        GLsizei stride,
                                        const GLvoid *pointer)
{
...
void APIENTRY_GL4ES fpe_glTexCoordPointer(GLint size,
                                          GLenum type,
                                          GLsizei stride,
                                          const GLvoid *pointer)
{
...
void APIENTRY_GL4ES fpe_glTexCoordPointerTMU(GLint size,
                                             GLenum type,
                                             GLsizei stride,
                                             const GLvoid *pointer,
                                             int TMU)
...
void APIENTRY_GL4ES fpe_glFogCoordPointer(GLenum type,
                                          GLsizei stride,
                                          const GLvoid *pointer)
{
...
void APIENTRY_GL4ES fpe_glColor4f(GLfloat red,
                                  GLfloat green,
                                  GLfloat blue,
                                  GLfloat alpha)
{
...
void APIENTRY_GL4ES fpe_glDrawElements(GLenum mode,
                                       GLsizei count,
                                       GLenum type,
                                       const GLvoid *indices)
{
    DBG(printf(
            "fpe_glDrawElements(%s, %d, %s, %p), program=%d, instanceID=%u\n",
...
void APIENTRY_GL4ES fpe_glDrawArraysInstanced(GLenum mode,
                                              GLint first,
                                              GLsizei count,
                                              GLsizei primcount)
{
                ...
                if (w->divisor && w->enabled) {
                    char *current =
                        (char *) ((uintptr_t) w->pointer +
                                  ((w->buffer) ? (uintptr_t) w->buffer->data
                                               : 0));
                    int stride = w->stride;
                    if (!stride)
                    ...
                    if (memcmp(glstate->gleshard->vavalue[i], current,
                               4 * sizeof(GLfloat))) {
                        memcpy(glstate->gleshard->vavalue[i], current,
                               4 * sizeof(GLfloat));
                        gles_glVertexAttrib4fv(i,
...
void APIENTRY_GL4ES fpe_glDrawElementsInstanced(GLenum mode,
                                                GLsizei count,
                                                GLenum type,
                                                const GLvoid *indices,
                                                GLsizei primcount)
...
void APIENTRY_GL4ES fpe_glLightfv(GLenum light,
                                  GLenum pname,
                                  const GLfloat *params)
{
    noerrorShim();
}
void APIENTRY_GL4ES fpe_glMaterialfv(GLenum face,
                                     GLenum pname,
                                     const GLfloat *params)
{
    noerrorShim();
}
void APIENTRY_GL4ES fpe_glMaterialf(GLenum face,
                                    GLenum pname,
                                    const GLfloat param)
{
...
void realize_glenv(int ispoint,
                   ...
                   scratch_t *scratch)
{
    // the handling of GL_BGRA size of GL_DOUBLE using 1 scratch in not ideal,
    // and a waste when dealing with Buffers TODO: have the scratch buffer part
    // of the VBO, and tag it dirty when buffer is changed (or always dirty for
    // VBO 0)
    if (hardext.esversion == 1)
        return;
    ...
    if (glstate->fpe_bound_changed && !glstate->glsl->program) {
        for (int i = 0; i < glstate->fpe_bound_changed; i++) {
            ...
            // disable texture unit, in that case (binded texture iconsts not
            // valid)
    ...
    if (gl4es_glIsProgram(glstate->glsl->program)) {
        // but first, check if some fixedpipeline state (like GL_ALPHA_TEST)
        // need to alter the original program
        fpe_state_t state;
    ...
    if (globals4es.fbounbind && glstate->fbo.current_fb->id) {
        ...
        while (tu_idx < MAX_TEX && glprogram->texunits[tu_idx].type && !need) {
            ...
            if (tex && tex->binded_fbo == glstate->fbo.current_fb->id) {
                DBG(printf("Texture %d is used on Uniform %s (%d) and binded "
                           "on FBO %d Attachement=%s\n",
                           tex->glname,
                           GetUniformName(glprogram,
                                          glprogram->texunits[tu_idx].id),
                           glprogram->texunits[tu_idx].id,
    ...
    if (glprogram->has_builtin_matrix) {
        if (glprogram->builtin_matrix[MAT_MVP] != -1 ||
            ...
            GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_MVP], 1,
                               GL_FALSE, getMVPMat());
            GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_MVP_T],
                               1, GL_TRUE, getMVPMat());
        ...
        if (glprogram->builtin_matrix[MAT_MV] != -1 ||
            ...
            GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_MV], 1,
                               GL_FALSE, getMVMat());
            GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_MV_T],
                               1, GL_TRUE, getMVMat());
        ...
        if (glprogram->builtin_matrix[MAT_N] != -1 ||
            ...
            if (glprogram->builtin_normalrescale != -1 &&
                ...
                float tmp = 1.0f;
                GoUniformfv(glprogram, glprogram->builtin_normalrescale, 1, 1,
                            &tmp);
            }
            if (glprogram->builtin_matrix[MAT_N] != -1) {
                GoUniformMatrix3fv(glprogram, glprogram->builtin_matrix[MAT_N],
                                   1, GL_FALSE, getNormalMat());
            }
            if ((glprogram->builtin_normalrescale != -1 &&
                 glstate->fpe_state->rescaling)) {
                if (glprogram->builtin_normalrescale != -1) {
                    ...
                    float tmp =
                        1.0f / sqrtf(invmat[3 * 4 + 1] * invmat[3 * 4 + 1] +
                                     invmat[3 * 4 + 2] * invmat[3 * 4 + 2] +
                                     invmat[3 * 4 + 3] * invmat[3 * 4 + 3]);
                    GoUniformfv(glprogram, glprogram->builtin_normalrescale, 1,
                                1, &tmp);
                }
            }
        }
        ...
        for (int i = 0; i < MAX_TEX; i++) {
            if (glprogram->builtin_matrix[MAT_T0 + i * 4] != -1 ||
                ...
                GoUniformMatrix4fv(glprogram,
                                   glprogram->builtin_matrix[MAT_T0 + i * 4], 1,
                                   GL_FALSE, getTexMat(i));
                GoUniformMatrix4fv(glprogram,
                                   glprogram->builtin_matrix[MAT_T0_T + i * 4],
                                   1, GL_TRUE, getTexMat(i));
                if (glprogram->builtin_matrix[MAT_T0_I + i * 4] != -1 ||
                    glprogram->builtin_matrix[MAT_T0_IT + i * 4] != -1) {
                    GLfloat invmat[16];
                    matrix_inverse(getTexMat(i), invmat);
                    GoUniformMatrix4fv(
                        glprogram, glprogram->builtin_matrix[MAT_T0_I + i * 4],
                        1, GL_FALSE, invmat);
                    GoUniformMatrix4fv(
                        glprogram, glprogram->builtin_matrix[MAT_T0_IT + i * 4],
                        1, GL_TRUE, invmat);
                }
            }
        }
    ...
    if (glprogram->has_builtin_light) {
        for (int i = 0; i < MAX_LIGHT; i++) {
            if (glprogram->builtin_lights[i].has) {
                GoUniformfv(glprogram, glprogram->builtin_lights[i].ambient, 4,
                            1, glstate->light.lights[i].ambient);
                GoUniformfv(glprogram, glprogram->builtin_lights[i].diffuse, 4,
                            1, glstate->light.lights[i].diffuse);
                GoUniformfv(glprogram, glprogram->builtin_lights[i].specular, 4,
                            1, glstate->light.lights[i].specular);
                GoUniformfv(glprogram, glprogram->builtin_lights[i].position, 4,
                            1, glstate->light.lights[i].position);
                if (glprogram->builtin_lights[i].halfVector !=
                    -1) {  // not sure of this, isn't gl_Vertex need to be used
                           // at some point?
                    GLfloat tmp[4];
                    memcpy(tmp, glstate->light.lights[i].position,
                           4 * sizeof(GLfloat));
                    ...
                    GoUniformfv(glprogram,
                                glprogram->builtin_lights[i].halfVector, 1, 1,
                                tmp);
                }
                GoUniformfv(glprogram,
                            glprogram->builtin_lights[i].spotDirection, 3, 1,
                            glstate->light.lights[i].spotDirection);
                GoUniformfv(glprogram,
                            glprogram->builtin_lights[i].spotExponent, 1, 1,
                            &glstate->light.lights[i].spotExponent);
                GoUniformfv(glprogram, glprogram->builtin_lights[i].spotCutoff,
                            1, 1, &glstate->light.lights[i].spotCutoff);
                if (!memcmp(&glprogram->builtin_lights[i].oldspotCutoff,
                    ...
                    glprogram->builtin_lights[i].oldspotCosCutoff =
                        cosf(glstate->light.lights[i].spotCutoff *
                             3.1415926535f / 180.0f);
                }
                GoUniformfv(glprogram,
                            glprogram->builtin_lights[i].spotCosCutoff, 1, 1,
                            &glprogram->builtin_lights[i].oldspotCosCutoff);
                GoUniformfv(glprogram,
                            glprogram->builtin_lights[i].constantAttenuation, 1,
                            1, &glstate->light.lights[i].constantAttenuation);
                GoUniformfv(glprogram,
                            glprogram->builtin_lights[i].linearAttenuation, 1,
                            1, &glstate->light.lights[i].linearAttenuation);
                GoUniformfv(glprogram,
                            glprogram->builtin_lights[i].quadraticAttenuation,
                            1, 1,
            ...
            if (glprogram->builtin_lightprod[1][i].has) {
                ...
                vector4_mult(glstate->material.back.ambient,
                             ...
                             tmp);  // TODO: Check that
                GoUniformfv(glprogram,
                            glprogram->builtin_lightprod[1][i].ambient, 4, 1,
                            tmp);
                vector4_mult(glstate->material.back.diffuse,
                             glstate->light.lights[i].diffuse, tmp);
                GoUniformfv(glprogram,
                            glprogram->builtin_lightprod[1][i].diffuse, 4, 1,
                            tmp);
                vector4_mult(glstate->material.back.specular,
                             glstate->light.lights[i].specular, tmp);
                GoUniformfv(glprogram,
                            glprogram->builtin_lightprod[1][i].specular, 4, 1,
                            tmp);
            }
        }
        ...
        if (glprogram->builtin_material[0].has) {
            GoUniformfv(glprogram, glprogram->builtin_material[0].emission, 4,
                        1, glstate->material.front.emission);
            ...
            GoUniformfv(glprogram, glprogram->builtin_material[0].specular, 4,
                        1, glstate->material.front.specular);
            GoUniformfv(glprogram, glprogram->builtin_material[0].shininess, 1,
                        1, &glstate->material.front.shininess);
        ...
        if (glprogram->builtin_material[1].has) {
            GoUniformfv(glprogram, glprogram->builtin_material[1].emission, 4,
                        1, glstate->material.back.emission);
            ...
            GoUniformfv(glprogram, glprogram->builtin_material[1].specular, 4,
                        1, glstate->material.back.specular);
            GoUniformfv(glprogram, glprogram->builtin_material[1].shininess, 1,
                        1, &glstate->material.back.shininess);
        ...
        if (glprogram->builtin_lightmodelprod[0].sceneColor != -1) {
            GLfloat tmp[4];
            vector4_mult(glstate->material.front.ambient,
                         glstate->light.ambient, tmp);  // TODO: check that
            vector4_add(tmp, glstate->material.front.emission, tmp);
            GoUniformfv(glprogram,
                        glprogram->builtin_lightmodelprod[0].sceneColor, 4, 1,
                        tmp);
        }
        if (glprogram->builtin_lightmodelprod[1].sceneColor != -1) {
            ...
            GoUniformfv(glprogram,
                        glprogram->builtin_lightmodelprod[1].sceneColor, 4, 1,
                        tmp);
        }
    }
    // Instance ID
    ...
    if (glprogram->builtin_fog.has) {
        ...
        GoUniformfv(glprogram, glprogram->builtin_fog.start, 1, 1,
                    &glstate->fog.start);
        GoUniformfv(glprogram, glprogram->builtin_fog.end, 1, 1,
                    &glstate->fog.end);
    ...
    if (glprogram->builtin_pointsprite.has) {
        ...
        GoUniformfv(glprogram, glprogram->builtin_pointsprite.fadeThresholdSize,
                    1, 1, &glstate->pointsprite.fadeThresholdSize);
        GoUniformfv(glprogram,
                    glprogram->builtin_pointsprite.distanceConstantAttenuation,
                    1, 1, glstate->pointsprite.distance + 0);
        GoUniformfv(glprogram,
                    glprogram->builtin_pointsprite.distanceLinearAttenuation, 1,
                    1, glstate->pointsprite.distance + 1);
        GoUniformfv(glprogram,
                    glprogram->builtin_pointsprite.distanceQuadraticAttenuation,
                    1, 1, glstate->pointsprite.distance + 2);
    }
    // texenv
    ...
    if (glprogram->has_builtin_texsampler) {
        for (int i = 0; i < hardext.maxtex; i++)
            GoUniformiv(glprogram, glprogram->builtin_texsampler[i], 1, 1,
                        &i);  // very basic stuff here, but sampler needs to be
                              // a uniform...
    }
    if (glprogram->has_builtin_texadjust) {
        for (int i = 0; i < hardext.maxtex; i++) {
            ...
            gltexture_t *tex =
                (tt == -1) ? NULL : glstate->texture.bound[i][tt];
            if (tex && tex->valid)
    ...
    if (glprogram->last_vert && glprogram->last_vert->old) {
        ...
        if (glprogram->has_vtx_progloc) {
            for (int i = 0; i < MAX_VTX_PROG_LOC_PARAMS; ++i)
                GoUniformfv(
                    glprogram, glprogram->vtx_progloc[i], 4, 1,
    ...
    if (glprogram->last_frag && glprogram->last_frag->old) {
        ...
        if (glprogram->has_frg_progloc) {
            for (int i = 0; i < MAX_FRG_PROG_LOC_PARAMS; ++i)
                GoUniformfv(
                    glprogram, glprogram->frg_progloc[i], 4, 1,
    ...
    for (int i = 0; i < hardext.maxvattrib; i++)
        if (glprogram->va_size[i])  // only check used VA...
        {
            ...
            if (enabled && !w->buffer && !w->pointer) {
                DBG(
                    printf("Warning: VA %d Enabled with buffer:0 and NULL "
                           "pointer, disabling\n",
                           i));
                enabled = 0;
            }
            ...
            if (v->enabled) {
                ...
                if (dirty || v->size != w->size || v->type != w->type ||
                    ...
                    if ((w->size == GL_BGRA || w->type == GL_DOUBLE) &&
                        scratch->size < 8) {
                        // need to adjust, so first need the min/max (a shame as
                        // I already must have that somewhere)
                        int imin, imax;
                        if (type == 0) {
                            imin = first;
                            imax = count;
                            ...
                            if (type == GL_UNSIGNED_INT)
                                getminmax_indices_ui(indices, &imax, &imin,
                                                     count);
                            else
                                getminmax_indices_us(indices, &imax, &imin,
                                                     count);
                            ++imax;
                        }
                        if (w->size == GL_BGRA) {
                            ...
                            if (warn) {
                                printf(
                                    "LIBGL: VertexAttribArray using GL_DOUBLE "
                                    "unimplemented!\n");
                                warn = 0;
                            }
                        }
                    ...
                    if (v->integer) {
                        if (gles_glVertexAttribIPointer)
                            gles_glVertexAttribIPointer(i, v->size, v->type,
                                                        v->stride, v->pointer);
                        else
                            gles_glVertexAttribPointer(i, v->size, v->type, 0,
                                                       v->stride, v->pointer);
                        DBG(printf(
                                "glVertexAttribIPointer(%d, %d, %s, %d, %p)\n",
                                i, v->size, PrintEnum(v->type), v->stride,
                                v->pointer);)
                    } else {
                        gles_glVertexAttribPointer(i, v->size, v->type,
                                                   v->normalized, v->stride,
                                                   v->pointer);
                        DBG(printf("glVertexAttribPointer(%d, %d, %s, %d, %d, "
                                   "%p)\n",
    ...
    for (int i = 0; i < hardext.maxvattrib; i++) {
        ...
        if (i < 2) {
            ...
            if (v->size != 2 || v->type != GL_FLOAT || v->normalized != 0 ||
                v->stride != 0 ||
                v->pointer !=
                    ((i == 0) ? glstate->blit->vert : glstate->blit->tex) ||
                v->buffer != 0) {
                v->size = 2;
                v->type = GL_FLOAT;
                v->normalized = 0;
...
int builtin_CheckUniform(program_t *glprogram, char *name, GLint id, int size)
{
    ...
    if (strncmp(name, lightsource_code, strlen(lightsource_code)) == 0 ||
        strncmp(name, lightsource_fpe_code, strlen(lightsource_fpe_code)) ==
            0) {
        // it's a light! grab it's number - also, fpe or not fpe is the same
        // length. The fpe version avoids the array...
        int n = name[strlen(lightsource_code)] -
    ...
    if (strncmp(name, frontmaterial_code, strlen(frontmaterial_code)) == 0 ||
        ...
        // it's a material
        int n =
            (strncmp(name, frontmaterial_code, strlen(frontmaterial_code)) == 0)
                ? 0
                : 1;
    ...
    if (strncmp(name, frontlightmodelprod_code,
        ...
        strncmp(name, backlightmodelprod_code,
                strlen(backlightmodelprod_code)) == 0) {
    ...
    if (strncmp(name, frontlightprod_code, strlen(frontlightprod_code)) == 0 ||
        ...
        strncmp(name, backlightprod_fpe_code, strlen(backlightprod_fpe_code)) ==
            0) {
        // it's a material
        int i = (strncmp(name, frontlightprod_code,
                         strlen(frontlightprod_code)) == 0 ||
                 strncmp(name, frontlightprod_fpe_code,
                         strlen(frontlightprod_fpe_code)) == 0)
                    ? 0
                    : 1;
    ...
    // texture env color
    if (strncmp(name, texenvcolor_fpe_code, strlen(texenvcolor_fpe_code)) ==
        0) {
