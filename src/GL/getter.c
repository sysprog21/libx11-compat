...
void BuildExtensionsList()
{
    if (!glstate->extensions) {
        ...
        // and now split in array of individual extensions TODO: is all this
        // better be moved in glstate?
        ...
        for (int i = 0; i < glstate->num_extensions; i++) {
            char *p2 = strchr(p, ' ');
            int sz = p2 - p;
            glstate->extensions_list[i] =
                (GLubyte *) calloc(sz + 1, sizeof(GLubyte));
...
const GLubyte *APIENTRY_GL4ES gl4es_glGetString(GLenum name)
{
    ...
    case GL_VENDOR:
        if (!vendor_string[0]) {
            snprintf(vendor_string, 127, "GL4ES wrapping %s",
                     gl4es_original_vendor ? gl4es_original_vendor
                                           : "an unknown hardware");
        }
        return (GLubyte *) vendor_string;
    case GL_RENDERER:
        if (!renderer_string[0]) {
            snprintf(renderer_string, 127, "GL4ES using %s",
                     gl4es_original_renderer ? gl4es_original_renderer
                                             : "an unknown renderer");
        }
...
int gl4es_commonGet(GLenum pname, GLfloat *params)
{
    ...
    case GL_ALPHA_SCALE:
        *params =
            glstate->raster.raster_scale[(pname - GL_GREEN_SCALE) / 2 + 1];
        break;
    case GL_GREEN_BIAS:
    case GL_BLUE_BIAS:
    case GL_ALPHA_BIAS:
    ...
    case GL_MODELVIEW_STACK_DEPTH:
        *params = (glstate->modelview_matrix)
                      ? (glstate->modelview_matrix->top + 1)
                      : 1;
        break;
    ...
    case GL_TEXTURE_BINDING_1D:
        *params = glstate->texture.bound[glstate->texture.active][ENABLED_TEX1D]
                      ? glstate->texture
                            .bound[glstate->texture.active][ENABLED_TEX1D]
                            ->glname
                      : 0;
        break;
    case GL_TEXTURE_BINDING_2D:
        *params = glstate->texture.bound[glstate->texture.active][ENABLED_TEX2D]
                      ? glstate->texture
                            .bound[glstate->texture.active][ENABLED_TEX2D]
                            ->glname
                      : 0;
        break;
    case GL_TEXTURE_BINDING_3D:
        *params = glstate->texture.bound[glstate->texture.active][ENABLED_TEX3D]
                      ? glstate->texture
                            .bound[glstate->texture.active][ENABLED_TEX3D]
                            ->glname
                      : 0;
        break;
    ...
    case GL_TEXTURE_BINDING_CUBE_MAP:
        *params =
            glstate->texture.bound[glstate->texture.active][ENABLED_CUBE_MAP]
                ? glstate->texture
                      .bound[glstate->texture.active][ENABLED_CUBE_MAP]
                      ->glname
                : 0;
        break;
...
#undef GO
    ...
    case GL_SAMPLER_BINDING:
        if (glstate->samplers.sampler[glstate->texture.active])
            *params =
                glstate->samplers.sampler[glstate->texture.active]->glname;
        else
            *params = 0;
        break;
    // GL4ES special hints
...
void APIENTRY_GL4ES gl4es_glGetLightfv(GLenum light,
                                       GLenum pname,
                                       GLfloat *params)
{
...
void APIENTRY_GL4ES gl4es_glGetMaterialfv(GLenum face,
                                          GLenum pname,
                                          GLfloat *params)
{
    ...
    case GL_AMBIENT:
        if (face == GL_FRONT)
            memcpy(params, glstate->material.front.ambient,
                   4 * sizeof(GLfloat));
        if (face == GL_BACK)
    ...
    case GL_DIFFUSE:
        if (face == GL_FRONT)
            memcpy(params, glstate->material.front.diffuse,
                   4 * sizeof(GLfloat));
        if (face == GL_BACK)
    ...
    case GL_SPECULAR:
        if (face == GL_FRONT)
            memcpy(params, glstate->material.front.specular,
                   4 * sizeof(GLfloat));
        if (face == GL_BACK)
            memcpy(params, glstate->material.back.specular,
                   4 * sizeof(GLfloat));
        break;
    case GL_EMISSION:
        if (face == GL_FRONT)
            memcpy(params, glstate->material.front.emission,
                   4 * sizeof(GLfloat));
        if (face == GL_BACK)
            memcpy(params, glstate->material.back.emission,
                   4 * sizeof(GLfloat));
        break;
    case GL_SHININESS:
        if (face == GL_FRONT)
