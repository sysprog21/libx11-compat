...
#define env(name, global, message) \
    ...
    case 10:
        SHUT_LOGD(
            "Texture shrink, mode 10 selected (advertise 8192 max texture "
            "size, but >2048 are quadshrinked and > 512 are shrinked), but not "
            "for empty textures\n");
        break;
    case 11:
        SHUT_LOGD(
            "Texture shrink, mode 11 selected (advertise a max texture size "
            "*2, but every texture with one dimension > max texture size will "
            "get shrinked to max texture size), but not for empty textures\n");
        break;
    case 1:
    ...
    case 4:
        SHUT_LOGD(
            "Texture shrink, mode 4 selected (only > 256 /2, >=1024 /4 )\n");
        break;
    case 5:
        SHUT_LOGD(
            "Texture shrink, mode 5 selected (every > 256 is downscaled to 256 "
            "), but not for empty textures\n");
        break;
    case 6:
        SHUT_LOGD(
            "Texture shrink, mode 6 selected (only > 128 /2, >=512 is "
            "downscaled to 256 ), but not for empty textures\n");
        break;
    case 7:
        SHUT_LOGD(
            "Texture shrink, mode 7 selected (only > 512 /2 ), but not for "
            "empty textures\n");
        break;
    case 8:
        SHUT_LOGD(
            "Texture shrink, mode 8 selected (advertise 8192 max texture size, "
            "but >2048 are shrinked to 2048)\n");
        break;
    case 9:
        SHUT_LOGD(
            "Texture shrink, mode 9 selected (advertise 8192 max texture size, "
            "but >4096 are quadshrinked and > 512 are shrinked), but not for "
            "empty textures\n");
        break;
    default:
    ...
    env(LIBGL_BLENDCOLOR, globals4es.blendcolor,
    ...
    if (env_version) {
        SHUT_LOGD(
            "Override version string with \"%s\" (should be in the form of "
            "\"1.x\")\n",
            env_version);
    }
    if (env_version) {
        ...
        snprintf(globals4es.version, 49, "%d.%d gl4es wrapper %d.%d.%d",
                 globals4es.gl / 10, globals4es.gl % 10, MAJOR, MINOR,
                 REVISION);
    ...
    if (IsEnvVarTrue("LIBGL_NOINTOVLHACK")) {
        ...
        SHUT_LOGD(
            "No hack in shader converter to define overloaded function with "
            "int\n");
    }
    ...
    if (GetEnvVarInt("LIBGL_BEGINEND", &env_begin_end, 0)) {
        ...
        case 2:
            globals4es.beginend = 1;
            SHUT_LOGD(
                "Try to merge subsequent glBegin/glEnd blocks, even if there "
    ...
    if (env_forcenpot == 0 &&
        ...
        if (hardext.npot == 3) {
            ...
            SHUT_LOGD(
                "Forcing NPOT support by disabling MIPMAP support for NPOT "
                "textures\n");
        } else {
            SHUT_LOGD(
                "WARNING, No Limited or Full NPOT support in hardware, Forcing "
                "NPOT has no effect!\n");
        }
    }
    ...
    if (globals4es.maxbatch == 0) {
        ...
        SHUT_LOGD(
            "Trying to batch subsequent glDrawXXXX of size between %d and %d "
            "vertices\n",
    ...
    if (hardext.esversion == 1)
        ...
        case 3:
            SHUT_LOGD(
                "Trying to use VBO (special glLockArrays case for idtech3 "
                "engine)\n");
            break;
        default:
            globals4es.usevbo = 1;
    ...
    if (globals4es.blitfullscreen)
        SHUT_LOGD(
            "Hack to trigger a SwapBuffers when a Full Framebuffer Blit on "
            "default FBO is done\n");
    ...
    env(LIBGL_NOCLEAN, globals4es.noclean,
        "Don't clean Context when destroyed");
    ...
    if (hardext.prgbin_n > 0 && !globals4es.notexarray) {
        env(LIBGL_NOPSA, globals4es.nopsa,
            "Don't use PrecompiledShaderArchive");
