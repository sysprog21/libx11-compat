...
void push_hit()
{
    ...
    if (glstate->selectbuf.hit) {
        if (!glstate->selectbuf.overflow) {
            // Normalize zmin/zmax
            if ((glstate->selectbuf.zmaxoverall -
                 glstate->selectbuf.zminoverall) != 0.0f) {
                glstate->selectbuf.zmin =
                    (glstate->selectbuf.zmin - glstate->selectbuf.zminoverall) /
                    (glstate->selectbuf.zmaxoverall -
                     glstate->selectbuf.zminoverall);
...
{
    /* Return True is the segment is fully inside viewscreen or cross the
     * viewscreen Viewscreen is (-1,-1)(+1,+1) False if completly outside
     */
...
GLboolean select_triangle_in_viewscreen(const GLfloat *a,
                                        const GLfloat *b,
                                        const GLfloat *c)
{
    /* Return True is the triangle is in the viewscreen, or completly include,
     * or include the viewscreen
     */
    // Check if any segment intersect the viewscreen (include test if any point
    // is inside the viewscreen)
...
void select_glDrawArrays(const vertexattrib_t *vtx,
                         GLenum mode,
                         GLuint first,
                         GLuint count)
{
    ...
    if (glstate->selectbuf.buffer == NULL)
        return;
    GLfloat *vert =
        copy_gl_array(vtx->pointer, vtx->type, vtx->size, vtx->stride, GL_FLOAT,
                      4, 0, count + first, NULL);
...
#define FOUND()                     \
    ...
    for (int i = first; i < count + first; i++) {
        ...
        ZMinMax(&glstate->selectbuf.zminoverall,
                &glstate->selectbuf.zmaxoverall, vert + i * 4);
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        case GL_LINES:
            if (i % 2 == 1) {
                if (select_segment_in_viewscreen(vert2 + (i - 1) * 4,
                                                 vert2 + i * 4)) {
...
void select_glDrawElements(const vertexattrib_t *vtx,
                           GLenum mode,
                           GLuint count,
                           GLenum type,
                           GLvoid *indices)
{
    ...
    GLushort *sind =
        (GLushort *) ((type == GL_UNSIGNED_SHORT) ? indices : NULL);
    ...
    max++;
    GLfloat *vert = copy_gl_array(vtx->pointer, vtx->type, vtx->size,
                                  vtx->stride, GL_FLOAT, 4, 0, max, NULL);
    ...
    if (sind) {
        for (int i = 0; i < count; i++) {
            ...
            case GL_LINES:
                ...
                break;
            case GL_LINE_STRIP:
            case GL_LINE_LOOP:  // FIXME: the last "loop" segment is missing
                                // here
                if (i > 0) {
                    if (select_segment_in_viewscreen(vert + sind[(i - 1)] * 4,
                                                     vert + sind[i] * 4)) {
