...
const char *const *fpe_VertexShader(shaderconv_need_t *need, fpe_state_t *state)
{
    ...
    if (comments) {
        sprintf(buff,
                "// ** Vertex Shader **\n// ligthting=%d (twosided=%d, "
                "separate=%d, color_material=%d)\n// secondary=%d, "
                "planes=%s\n// point=%d%s\n",
    ...
    if (lighting) {
        ...
        if (!(color_material &&
              (state->cm_front_mode == FPE_CM_DIFFUSE ||
        ...
        for (int i = 0; i < hardext.maxlights; i++) {
            if (state->light & (1 << i)) {
                ...
                sprintf(buff,
                        "uniform _gl4es_LightProducts "
                        "_gl4es_FrontLightProduct_%d;\n",
                        i);
                ShadAppend(buff);
                headers++;
    ...
    if (!lighting) {
        ...
        if (twosided) {
            ...
            sprintf(bm_diffuse, "%s",
                    (color_material &&
                     (state->cm_back_mode == FPE_CM_DIFFUSE ||
                      state->cm_back_mode == FPE_CM_AMBIENTDIFFUSE))
                        ? "gl_Color.xyz * _gl4es_LightSource_"
        ...
        for (int i = 0; i < hardext.maxlights; i++) {
            if (state->light & (1 << i)) {
                if (comments) {
                    sprintf(buff,
                            "// light %d on, light_direction=%d, "
                            "light_cutoff180=%d\n",
                            i, (state->light_direction >> i & 1),
                            (state->light_cutoff180 >> i & 1));
                    ShadAppend(buff);
                }
                // enabled light i att depend on light position w
                if ((state->light_direction >> i & 1) ==
                    ...
                    sprintf(buff,
                            "VP = _gl4es_LightSource_%d.position.xyz - "
                            "vertex.xyz;\n",
                            i);
                    ShadAppend(buff);
                ...
                if ((state->light_cutoff180 >> i & 1) == 0) {
                    ...
                    ShadAppend(buff);
                    sprintf(buff,
                            "if(spot<_gl4es_LightSource_%d.spotCosCutoff) "
                            "spot=0.0; else spot=pow(spot, "
                ...
                if (twosided) {
                    if (color_material &&
                        ...
                        sprintf(buff,
                                "back_aa = %s.xyz * "
                                "_gl4es_LightSource_%d.ambient.xyz;\n",
                                bm_ambient, i);
                        ShadAppend(buff);
                    } else {
                        sprintf(buff,
                                "back_aa = "
                                "_gl4es_BackLightProduct_%d.ambient.xyz;\n",
                                i);
                        ShadAppend(buff);
                ...
                ShadAppend(buff);
                sprintf(buff,
                        "dd = (nVP>0.)?(nVP * %s%d.diffuse.xyz):vec3(0.);\n",
                        fm_diffuse, i);
                ShadAppend(buff);
                need_lightproduct[0][i] = 1;
                if (twosided) {
        ...
        if (state->light_separate) {
            ...
            if (twosided) {
                ShadAppend(
                    "SecBackColor.rgb = clamp(SecBackColor.rgb, 0., 1.);\n");
            }
        }
        if (is_default && need) {
        ...
        if (t) {
                ...
                for (int j = 0; j < ntc; j++) {
                    if (tg[j] == FPE_TG_NORMALMAP) {
                        ...
                        if (!spheremap) {
                            ...
                            ShadAppend(
                                "vec3 tmpsphere = "
                                "reflect(normalize(vertex.xyz), normal);\n");
                            ShadAppend("tmpsphere.z+=1.0;\n");
                            if (j == 0 && tg[j + 1] == FPE_TG_SPHEREMAP)
                                sprintf(buff,
                                        "tmp_tcoor.xy = "
                                        "tmpsphere.xy*(0.5*inversesqrt(dot("
                                        "tmpsphere, tmpsphere))) + vec2(0.5);");
                            else
                                ShadAppend(
                                    "tmpsphere.xy = "
                                    "tmpsphere.xy*(0.5*inversesqrt(dot("
                        ...
                        sprintf(buff,
                                "tmp_tcoor.%c=dot(gl_Vertex, "
                                "_gl4es_ObjectPlane%c_%d);\n",
            ...
            if (t == FPE_TEX_STRM) {
                sprintf(buff, "_gl4es_TexCoord_%d = %s.%s / %s.q;\n", i,
                        text_tmp, texxyzsize[t - 1], text_tmp);
            } else {
            ...
            if (adjust) {
                ...
                sprintf(buff, "_gl4es_TexCoord_%d.xy *= _gl4es_TexAdjust_%d;\n",
                        i, i);  // to avoid error on Cube map... but will that
                                // work anyway?
                ShadAppend(buff);
            }
        }
    ...
    if (point) {
        ...
        sprintf(buff,
                "gl_PointSize = "
                "clamp(gl_Point.size*inversesqrt(gl_Point."
                                      ...
                                      fpe_state_t *state)
{
    ...
    if (comments) {
        sprintf(buff,
                "// ** Fragment Shader **\n// lighting=%d, alpha=%d, "
                "secondary=%d, planes=%s, texturing=%d point=%d\n",
                lighting, alpha_test, secondary, fpe_binary(planes, 6),
                texturing, point);
        ShadAppend(buff);
        headers += gl4es_countline(buff);
    }
    ...
    if (texturing && (!point || pointsprite)) {
        ...
        for (int i = 0; i < hardext.maxtex; i++) {
            ...
            if (t) {
                ...
                if (comments) {
                    sprintf(buff,
                            "// Texture %d active: %X, texenv=%X, format=%X\n",
                ...
                case FPE_DECAL:
                    sprintf(buff,
                            "fColor.rgb = mix(fColor.rgb, texColor%d.rgb, "
                            "texColor%d.a);\n",
                            i, i);
                    ShadAppend(buff);
                    needclamp = 0;
                ...
                case FPE_BLEND:
                    ...
                    sprintf(buff,
                            ...
                            i);
                    shad = gl4es_inplace_insert(gl4es_getline(shad, headers),
                                                buff, shad, &shad_cap);
                    headers += gl4es_countline(buff);
                    needclamp = 0;
                    if (texformat != FPE_TEX_ALPHA) {
                    ...
                    case FPE_TEX_DEPTH:
                        sprintf(buff,
                                "fColor.a = mix(fColor.a, "
                                "_gl4es_TextureEnvColor_%d.a, texColor%d.a);\n",
                                i, i);
                        ShadAppend(buff);
                        break;
                    default:
                ...
                case FPE_COMBINE4: {
                    ...
                    for (int j = 0; j < 4; j++) {
                        if (src_a[j] == FPE_SRC_CONSTANT ||
                            src_r[j] == FPE_SRC_CONSTANT)
                            constant = 1;
                    }
                    if (comments) {
                        sprintf(buff,
                                " //  Combine RGB: fct=%d, Src/Op: 0=%d/%d "
                                "1=%d/%d 2=%d/%d 3=%d/%d\n",
                                combine_rgb, src_r[0], op_r[0], src_r[1],
                                op_r[1], src_r[2], op_r[2], src_r[3], op_r[3]);
                        ShadAppend(buff);
                        sprintf(buff,
                                " //  Combine Alpha: fct=%d, Src/Op: 0=%d/%d "
                                "1=%d/%d 2=%d/%d 3=%d/%d\n",
                                combine_alpha, src_a[0], op_a[0], src_a[1],
                                op_a[1], src_a[2], op_a[2], src_a[3], op_a[3]);
                        ShadAppend(buff);
                    }
                    if (constant) {
                        ...
                        sprintf(
                            ...
                            i);
                        shad =
                            gl4es_inplace_insert(gl4es_getline(shad, headers),
                                                 buff, shad, &shad_cap);
                        headers += gl4es_countline(buff);
                    }
                    for (int j = 0; j < 4; j++) {
                        if (src_r[j] == src_a[j] &&
                            op_r[j] == FPE_OP_SRCCOLOR &&
                            op_a[j] == FPE_OP_ALPHA) {
                            sprintf(buff, "Arg%d = %s;\n", j,
                                    fpe_texenvSrc(src_r[j], i, twosided));
                            ShadAppend(buff);
                        } else if (src_r[j] == src_a[j] &&
                                   op_r[j] == FPE_OP_MINUSCOLOR &&
                            ...
                            if (op_r[j] != -1)
                                ...
                                case FPE_OP_MINUSCOLOR:
                                    sprintf(
                                        buff,
                                        "Arg%d.rgb = vec3(1.) - %s.rgb;\n", j,
                                        fpe_texenvSrc(src_r[j], i, twosided));
                                    ShadAppend(buff);
                                    break;
                                case FPE_OP_ALPHA:
                                    sprintf(
                                        buff, "Arg%d.rgb = vec3(%s.a);\n", j,
                                        fpe_texenvSrc(src_r[j], i, twosided));
                                    ShadAppend(buff);
                                    break;
                                case FPE_OP_MINUSALPHA:
                                    sprintf(
                                        buff, "Arg%d.rgb = vec3(1. - %s.a);\n",
                                        j,
                                        fpe_texenvSrc(src_r[j], i, twosided));
                                    ShadAppend(buff);
                                    break;
                    ...
                    if (combine_rgb != FPE_CR_DOT3_RGBA &&
                        ...
                        case FPE_CR_INTERPOLATE:
                            ShadAppend(
                                "fColor = Arg0*Arg2 + Arg1*(vec4(1.)-Arg2);\n");
                            break;
                        ...
                        case FPE_CR_MOD_ADD_SIGNED:
                            ShadAppend(
                                "fColor = Arg0*Arg2 + Arg1 - vec4(0.5);\n");
                            break;
                        case FPE_CR_MOD_SUB:
                            ShadAppend("fColor = Arg0*Arg2 - Arg1;\n");
                            break;
                        ...
                        case FPE_CR_DOT3_RGB:
                            ShadAppend(
                                "fColor.rgb = "
                                "vec3(4.*((Arg0.r-0.5)*(Arg1.r-0.5)+(Arg0.g-0."
                                "5)*(Arg1.g-0.5)+(Arg0.b-0.5)*(Arg1.b-0.5)));"
                                "\n");
                            break;
                        case FPE_CR_DOT3_RGBA:
                            ShadAppend(
                                "fColor = "
                                "vec4(4.*((Arg0.r-0.5)*(Arg1.r-0.5)+(Arg0.g-0."
                                "5)*(Arg1.g-0.5)+(Arg0.b-0.5)*(Arg1.b-0.5)));"
                                "\n");
                            break;
                        case FPE_CR_MOD_ADD:
                            ShadAppend(
                                "fColor.rgb = Arg0.rgb*Arg2.rgb + Arg1.rgb;\n");
                            break;
                        case FPE_CR_MOD_ADD_SIGNED:
                            ShadAppend(
                                "fColor.rgb = Arg0.rgb*Arg2.rgb + Arg1.rgb - "
    ...
    if (fog) {
        ...
        case FPE_FOG_EXP:
            sprintf(buff,
                    "%s float FogF = clamp(exp(-gl_Fog.density * fog_c), 0., "
                    "1.);\n",
                    fogp);
            break;
        case FPE_FOG_EXP2:
    ...
    if (shaderblend) {
        if (comments) {
            sprintf(buff, "//Blend: src=%d/%d, dst=%d/%d, eq=%d/%d\n",
                    ...
                    state->blenddstrgb, state->blenddstalpha, state->blendeqrgb,
                    state->blendeqalpha);
            ShadAppend(buff);
        }
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if (blendrgb == blendalpha) {
                ...
                case FPE_BLEND_ONE_MINUS_CONSTANT_COLOR:
                    sprintf(buff,
                            " %s.rgb = vec3(1.0)-_gl4es_BlendColor.rgb;\n",
                            blend);
                    break;
                case FPE_BLEND_CONSTANT_ALPHA:
                ...
                case FPE_BLEND_ONE_MINUS_CONSTANT_ALPHA:
                    sprintf(buff,
                            " %s.rgb = vec3(1.0 - _gl4es_BlendColor.a);\n",
                            blend);
                    break;
                case FPE_BLEND_SRC_ALPHA_SATURATE:
        ...
        if (state->blendeqrgb == state->blendeqalpha) {
            ...
            case FPE_BLENDEQ_FUNC_ADD:
                sprintf(buff,
                        "%s.rgb = srcblend.rgb*%s.rgb + dstblend.rgb*%s.rgb;\n",
            ...
            case FPE_BLENDEQ_FUNC_REVERSE_SUBTRACT:
                sprintf(buff,
                        "%s.rgb = dstblend.rgb*%s.rgb - srcblend.rgb*%s.rgb;\n",
                                          ...
                                          int default_fragment)
{
    ...
    if (default_fragment)
        printf("fpe_CustomVertexShader(%p, %p, %d)\n%s\ncolor=%d\n", initial,
               state, default_fragment, initial, color);
                                            ...
                                            fpe_state_t *state)
{
    ...
    if (alpha_test || planes || shaderblend) {
        ...
        if (is_fragcolor) {
            int l_main =
                gl4es_getline_for(
                    shad, gl4es_prev_str(shad, strstr(shad, "_gl4es_main"))) -
                1;
    ...
    if (strstr(shad, "_gl4es_main")) {
        ...
        if (planes) {
            ...
            for (int i = 0; i < hardext.maxplanes; i++) {
                if ((planes >> i) & 1) {
                    sprintf(buff, "%smin(0., clippedvertex_%d)", k ? "+" : "",
                            i);
                    ShadAppend(buff);
                    k = 1;
                }
        ...
        if (alpha_test) {
            if (alpha_test && alpha_func > FPE_NEVER) {
                shad =
                    gl4es_inplace_insert(gl4es_getline(shad, headline),
            ...
            if (alpha_func == FPE_ALWAYS) {
                ...
                // FPE_LESS FPE_EQUAL FPE_LEQUAL FPE_GREATER FPE_NOTEQUAL
                // FPE_GEQUAL but need to negate the operator
                const char *alpha_test_op[] = {">=", "!=", ">",
                                               "<=", "==", "<"};
                sprintf(buff,
        ...
        if (shaderblend) {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                int blendrgb = i ? state->blenddstrgb : state->blendsrcrgb;
                int blendalpha =
                    i ? state->blenddstalpha : state->blendsrcalpha;
                if (blendrgb == blendalpha) {
                    int need_vec4 = 0;
                    switch (blendrgb) {
                    case FPE_BLEND_ZERO:
