...
renderlist_t *arrays_to_renderlist(renderlist_t *list,
                                   GLenum mode,
                                   GLsizei skip,
                                   GLsizei count)
{
...
#undef OP
        ...
        if (!globals4es.novaocache && glstate->vao != glstate->defaultvao) {
            ...
            *glstate->vao->shared_arrays =
                2;  // already shared between glstate & list
#define G2(AA, A, B)                                                   \
            ...
            if (glstate->vao->shared_arrays) {
                if (glstate->vao->vertexattrib[ATT_COLOR].size == GL_BGRA)
                    glstate->vao->color.ptr = copy_gl_pointer_color_bgra(
                        ...
                        glstate->vao->vertexattrib[ATT_COLOR].stride, 4, 0,
                        count);
                else
                ...
                if (glstate->vao->vertexattrib[ATT_COLOR].size == GL_BGRA)
                    list->color = copy_gl_pointer_color_bgra(
                        glstate->vao->vertexattrib[ATT_COLOR].pointer,
                        glstate->vao->vertexattrib[ATT_COLOR].stride, 4, skip,
                        count);
                else
            ...
            if (glstate->vao->shared_arrays) {
                if (glstate->vao->vertexattrib[ATT_SECONDARY].size == GL_BGRA)
                    glstate->vao->secondary.ptr = copy_gl_pointer_color_bgra(
                        ...
                        glstate->vao->vertexattrib[ATT_SECONDARY].stride, 4, 0,
                        count);
                else
                ...
                if (glstate->vao->vertexattrib[ATT_SECONDARY].size == GL_BGRA)
                    list->secondary = copy_gl_pointer_color_bgra(
                        glstate->vao->vertexattrib[ATT_SECONDARY].pointer,
                        glstate->vao->vertexattrib[ATT_SECONDARY].stride, 4,
                        skip, count);
                else
            ...
            if (glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + i].enabled) {
                if (glstate->vao->shared_arrays) {
                    glstate->vao->tex[i].ptr = copy_gl_pointer_tex(
                        &glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + i], 4,
                        0, count);
...
static renderlist_t *arrays_add_renderlist(renderlist_t *a,
                                           GLenum mode,
                                           GLsizei skip,
                                           GLsizei count,
                                           GLushort *indices,
                                           int ilen_b)
{
    ...
    // append all draw elements of b in a check the final indice size of a and b
    int ilen_a = a->ilen;
    ...
    unsharedindices_renderlist(
        a, ((ilen_a) ? ilen_a : a->len) + ((ilen_b) ? ilen_b : len_b));
    // append arrays
    if (glstate->vao->shared_arrays) {
        if (a->vert)
        ...
        if (a->color)
            memcpy(a->color + a->len * 4, glstate->vao->color.ptr + skip * 4,
                   len_b * 4 * sizeof(GLfloat));
        if (a->secondary)
            memcpy(a->secondary + a->len * 4,
                   glstate->vao->secondary.ptr + skip * 4,
                   len_b * 4 * sizeof(GLfloat));
        if (a->fogcoord)
        ...
        for (int i = 0; i < a->maxtex; i++)
            if (a->tex[i])
                memcpy(a->tex[i] + a->len * 4,
                       glstate->vao->tex[i].ptr + skip * 4,
                       len_b * 4 * sizeof(GLfloat));
    } else {
        if (a->vert)
            copy_gl_pointer_tex_noalloc(a->vert + a->len * 4,
                                        &glstate->vao->vertexattrib[ATT_VERTEX],
                                        4, skip, count);
        if (a->normal)
            copy_gl_pointer_raw_noalloc(a->normal + a->len * 3,
                                        &glstate->vao->vertexattrib[ATT_NORMAL],
                                        3, skip, count);
        if (a->color) {
            if (glstate->vao->vertexattrib[ATT_COLOR].size == GL_BGRA)
        ...
        if (a->secondary)
            if (glstate->vao->vertexattrib[ATT_SECONDARY].size == GL_BGRA)
                copy_gl_pointer_color_bgra_noalloc(
                    a->secondary + a->len * 4,
                    glstate->vao->vertexattrib[ATT_SECONDARY].pointer,
                    glstate->vao->vertexattrib[ATT_SECONDARY].stride, 4, skip,
                    count);
            else
        ...
        for (int aa = 0; aa < hardext.maxtex; aa++) {
            if (glstate->enable.texture[aa]) {
                if ((hardext.esversion == 1) &&
                    ((glstate->enable.texgen_s[aa] ||
                      glstate->enable.texgen_t[aa] ||
                      glstate->enable.texgen_r[aa] ||
                      glstate->enable.texgen_q[aa])))
                    return true;
                if ((!glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + aa]
                          .enabled) &&
                    !(mode == GL_POINT && glstate->texture.pscoordreplace[aa]))
                    return true;
                if ((glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + aa]
                         .enabled) &&
                    (glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + aa].size ==
                     1))
                    return true;
            }
        }
...
GLuint len_indices(const GLushort *sindices,
...
static void glDrawElementsCommon(GLenum mode,
                                 GLint first,
                                 GLsizei count,
                                 GLuint len,
...
#define client_state(A, B, C)                           \
    if ((glstate->vao->vertexattrib[A].enabled !=       \
        ...
        (hardext.esversion != 1)) {                     \
        C if (glstate->vao->vertexattrib[A].enabled)    \
    ...
    if (glstate->render_mode == GL_SELECT) {
        ...
        if (!sindices && !iindices)
            select_glDrawArrays(&glstate->vao->vertexattrib[ATT_VERTEX], mode,
                                first, count);
        else
            select_glDrawElements(
                &glstate->vao->vertexattrib[ATT_VERTEX], mode, count,
...
#define GetP(A) (&glstate->vao->vertexattrib[A])
            // secondary color and color sizef != 4 are "intercepted" and draw
            // using a list, unless using ES>1.1
            ...
            for (int aa = 0; aa < hardext.maxtex; aa++) {
                ...
                if (itarget >= 0) {
                    ...
                    if (p->enabled) {
                        ...
                        gles_glMultiTexCoord4f(
                            GL_TEXTURE0 + aa, glstate->texcoord[aa][0],
                            glstate->texcoord[aa][1], glstate->texcoord[aa][2],
...
#undef GetP
        ...
        if (hardext.esversion > 1 && globals4es.usevbo == 2 &&
            ...
            // can now browse all enabled VA, and put the corresponding data in
            // a VBO warning, with the use of first Checking only Vertex Attrib
            // for now! TODO: check all va, and take care of interleaved ones...
...
void APIENTRY_GL4ES gl4es_glDrawRangeElements(GLenum mode,
                                              GLuint start,
                                              GLuint end,
                                              GLsizei count,
                                              GLenum type,
        ...
        if (globals4es.mergelist && list->stage >= STAGE_DRAW &&
            ...
            if (list->vert) {
                glstate->list.active = arrays_add_renderlist(
                    list, mode, start, end + 1, sindices, count);
...
void APIENTRY_GL4ES gl4es_glDrawElements(GLenum mode,
                                         GLsizei count,
                                         GLenum type,
                                         const GLvoid *indices)
{
    ...
    // TODO: split for count > 65535? special check for QUADS and TRIANGLES that
    // need multiple of 4 or 3 vertex...
    count = adjust_vertices(mode, count);
    ...
    if (compiling) {
        ...
        if (globals4es.mergelist && list->stage >= STAGE_DRAW &&
            ...
            list = NewDrawStage(list, mode);
            glstate->list.active = arrays_add_renderlist(
                list, mode, min, max + 1, sindices, count);
            NewStage(glstate->list.active, STAGE_POSTDRAW);
            return;
        }
...
void APIENTRY_GL4ES gl4es_glDrawArrays(GLenum mode, GLint first, GLsizei count)
{
    ...
    if (glstate->list.active) {
        ...
        if (globals4es.mergelist && list->stage >= STAGE_DRAW &&
            ...
            if (list->vert) {
                glstate->list.active = arrays_add_renderlist(
                    list, mode, first, count + first, NULL, 0);
                NewStage(glstate->list.active, STAGE_POSTDRAW);
                return;
            }
        }
...
void APIENTRY_GL4ES gl4es_glMultiDrawArrays(GLenum mode,
                                            const GLint *firsts,
    ...
    for (int i = 0; i < primcount; i++) {
        ...
        if (compiling) {
            if (globals4es.mergelist &&
                glstate->list.active->stage >= STAGE_DRAW &&
            ...
            glstate->list.active = arrays_to_renderlist(
                glstate->list.active, mode, first, count + first);
            NewStage(glstate->list.active, STAGE_POSTDRAW);
        ...
        if (intercept) {
            if (list) {
                ...
                if (globals4es.mergelist && list->stage >= STAGE_DRAW &&
                    is_list_compatible(list) && !list->use_glstate) {
                    list = NewDrawStage(list, mode);
                    list = arrays_add_renderlist(list, mode, first,
                                                 count + first, NULL, 0);
            ...
            if (mode == GL_QUADS) {
                ...
                if ((indcnt < realcount) || (indfirst != realfirst)) {
                    ...
                    indfirst = realfirst;
                    GLushort *p = indices;
                    for (int k = 0, j = indfirst; k + 3 < indcnt;
                         k += 4, j += 4) {
                        *(p++) = j + 0;
                        *(p++) = j + 1;
                        *(p++) = j + 2;
                                              ...
                                              GLsizei primcount)
{
    ...
    for (int i = 0; i < primcount; i++) {
        ...
        bool need_free = !((type == GL_UNSIGNED_SHORT) ||
                           (!compiling && !intercept &&
                            type == GL_UNSIGNED_INT && hardext.elementuint));
        if (need_free) {
            GLvoid *src;
            if (glstate->vao->elements) {
                src = (void *) (char *) glstate->vao->elements->data +
                      (uintptr_t) indices[i];
            } else {
                src = (GLvoid *) indices[i];
            }
            sindices = copy_gl_array(src, type, 1, 0, GL_UNSIGNED_SHORT, 1, 0,
                                     count, NULL);
            old_index = wantBufferIndex(0);
        } else {
            if (type == GL_UNSIGNED_INT) {
                if (glstate->vao->elements) {
                    iindices = (void *) (char *) glstate->vao->elements->data +
                               (uintptr_t) indices[i];
                } else {
                ...
                if (glstate->vao->elements) {
                    sindices = (void *) (char *) glstate->vao->elements->data +
                               (uintptr_t) indices[i];
                } else {
...
void APIENTRY_GL4ES
gl4es_glMultiDrawElementsBaseVertex(GLenum mode,
                                    GLsizei *counts,
                                    GLenum type,
                                    const void *const *indices,
                                    GLsizei primcount,
...
void APIENTRY_GL4ES gl4es_glDrawElementsBaseVertex(GLenum mode,
                                                   GLsizei count,
                                                   GLenum type,
                                                   const void *indices,
                                                   GLint basevertex)
{
    ...
    if (basevertex == 0)
        ...
        if (!compiling) {
            if (!intercept && glstate->list.pending &&
                count > MAX_BATCH)  // too large and will not intercept, stop
                                    // the BATCH
                gl4es_flush();
            else if ((!intercept && !glstate->list.pending &&
                      count < MIN_BATCH) ||
...
void APIENTRY_GL4ES gl4es_glDrawRangeElementsBaseVertex(GLenum mode,
                                                        GLuint start,
                                                        GLuint end,
                                                        GLsizei count,
                                                        GLenum type,
...
void APIENTRY_GL4ES gl4es_glDrawArraysInstanced(GLenum mode,
                                                GLint first,
                                                GLsizei count,
                                                GLsizei primcount)
{
...
void APIENTRY_GL4ES gl4es_glDrawElementsInstanced(GLenum mode,
                                                  GLsizei count,
                                                  GLenum type,
                                                  const void *indices,
                                                  GLsizei primcount)
...
void APIENTRY_GL4ES gl4es_glDrawElementsInstancedBaseVertex(GLenum mode,
                                                            GLsizei count,
                                                            GLenum type,
                                                            const void *indices,
                                                            GLsizei primcount,
                                                            GLint basevertex)
{
    DBG(printf("glDrawElementsInstanceBaseVertex(%s, %d, %s, %p, %d, %d), "
