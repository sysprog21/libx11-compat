...
void APIENTRY_GL4ES gl4es_glLightModelfv(GLenum pname, const GLfloat *params)
{
    ...
    if (glstate->list.active)
        if ((glstate->list.compiling)) {
            ...
            glstate->list.active->lightmodel =
                (GLfloat *) malloc(4 * sizeof(GLfloat));
            int sz = 4;
            if (pname == GL_LIGHT_MODEL_TWO_SIDE ||
                pname == GL_LIGHT_MODEL_COLOR_CONTROL ||
                pname == GL_LIGHT_MODEL_LOCAL_VIEWER)
                sz = 1;
            memcpy(glstate->list.active->lightmodel, params,
                   sz * sizeof(GLfloat));
            noerrorShim();
            return;
        } else
...
void APIENTRY_GL4ES gl4es_glLightfv(GLenum light,
                                    GLenum pname,
                                    const GLfloat *params)
{
    ...
    case GL_POSITION:
        ...
        if (memcmp(glstate->light.lights[nl].position, tmp,
                   4 * sizeof(GLfloat)) == 0)
            return;
    ...
    case GL_SPOT_DIRECTION:
        ...
        if (memcmp(glstate->light.lights[nl].spotDirection, tmp,
                   3 * sizeof(GLfloat)) == 0)
            return;
        memcpy(glstate->light.lights[nl].spotDirection, tmp,
               3 * sizeof(GLfloat));
        break;
                                       ...
                                       const GLfloat *params)
{
    if (glstate->list.active)
        if (glstate->list.begin) {
            // if a glMaterialfv is called inside a glBegin/glEnd block then use
            // rlMaterial to store in current list the material wanted as if the
            // material was asked before the glBegin() It's not real behaviour,
    ...
    case GL_AMBIENT:
        if (face == GL_FRONT_AND_BACK || face == GL_FRONT)
            memcpy(glstate->material.front.ambient, params,
                   4 * sizeof(GLfloat));
    ...
    case GL_DIFFUSE:
        if (face == GL_FRONT_AND_BACK || face == GL_FRONT)
            memcpy(glstate->material.front.diffuse, params,
                   4 * sizeof(GLfloat));
        if (face == GL_FRONT_AND_BACK || face == GL_BACK)
    ...
    case GL_SPECULAR:
        if (face == GL_FRONT_AND_BACK || face == GL_FRONT)
            memcpy(glstate->material.front.specular, params,
                   4 * sizeof(GLfloat));
        if (face == GL_FRONT_AND_BACK || face == GL_BACK)
            memcpy(glstate->material.back.specular, params,
                   4 * sizeof(GLfloat));
        break;
    case GL_EMISSION:
        if (face == GL_FRONT_AND_BACK || face == GL_FRONT)
            memcpy(glstate->material.front.emission, params,
                   4 * sizeof(GLfloat));
        if (face == GL_FRONT_AND_BACK || face == GL_BACK)
            memcpy(glstate->material.back.emission, params,
                   4 * sizeof(GLfloat));
        break;
