...
#define FAIL(str)                    \
    ...
    *error_msg = strdup(str);        \
    return
void generateVariablePre(sCurStatus *curStatusPtr,
                         int vertex,
                         char **error_msg,
                         sVariable *varPtr)
{
...
void generateInstruction(sCurStatus *curStatusPtr,
                         int vertex,
                         char **error_msg,
...
#define PUSH_DESTLEN(i)                                                        \
    for (dstSwizLen = 0;                                                       \
         (dstSwizLen < 4) && (SWIZ(i, dstSwizLen) != SWIZ_NONE); ++dstSwizLen) \
        ;                                                                      \
    switch (dstSwizLen) {                                                      \
    case 1:                                                                    \
        APPEND_OUTPUT("(", 1)                                                  \
...
void generateVariablePst(sCurStatus *curStatusPtr,
                         int vertex,
                         char **error_msg,
                         sVariable *varPtr)
{
