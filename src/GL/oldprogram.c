...
void APIENTRY_GL4ES gl4es_glProgramStringARB(GLenum target,
                                             GLenum format,
                                             GLsizei len,
                                             const GLvoid *string)
{
    ...
    if (!old) {
        // no ARB program is bound to the selected target
    ...
    if (!old->shader) {
        DBG(printf("Error, no shader attached but glProgramStringARB(...) "
                   "called\n");)
        errorShim(GL_INVALID_OPERATION);
        return;
    }
    ...
    if (!old->shader->converted) {
        ...
        if (glstate->glsl->error_msg)
            free(glstate->glsl->error_msg);
        glstate->glsl->error_msg =
            strdup("Error with GLSL->GLSL:ES conversion");
        glstate->glsl->error_ptr = 0;
        return;
    }
...
void APIENTRY_GL4ES gl4es_glBindProgramARB(GLenum target, GLuint program)
{
    ...
    if (program) {
        ...
        if (k == kh_end(oldprograms)) {
            // if program as not be generated it's fine, create a new one
            // on-the-fly
            int ret;
    ...
    case GL_VERTEX_PROGRAM_ARB:
        if (program) {
            ...
            if (!old->type) {
                ...
                old->prog_local_params = (float *) calloc(
                    MAX_VTX_PROG_LOC_PARAMS * 4, sizeof(float));
    ...
    case GL_FRAGMENT_PROGRAM_ARB:
        if (program) {
            ...
            if (!old->type) {
                ...
                old->prog_local_params = (float *) calloc(
                    MAX_FRG_PROG_LOC_PARAMS * 4, sizeof(float));
...
void APIENTRY_GL4ES gl4es_glDeleteProgramsARB(GLsizei n, const GLuint *programs)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (k != kh_end(oldprograms)) {
            ...
            // drop the binding if this program is currently in use, else the
            // stale pointer/ids would reference freed memory later
                                                      ...
                                                      const GLdouble *params)
{
    DBG(printf("glProgramEnvParameter4dvARB(%s, %u, %p[%f/%f/%f/%f])\n",
               PrintEnum(target), index, params, params[0], params[1],
               params[2], params[3]);)
    float *f = NULL;
    switch (target) {
    case GL_VERTEX_PROGRAM_ARB:
...
void APIENTRY_GL4ES gl4es_glGetProgramivARB(GLenum target,
                                            GLenum pname,
                                            GLint *params)
{
    ...
    case GL_MAX_PROGRAM_NATIVE_TEX_INDIRECTIONS_ARB:
        *params = 8;
        break;
    /* // bounded program stats... case GL_PROGRAM_NATIVE_INSTRUCTIONS_ARB: case
     * GL_PROGRAM_INSTRUCTIONS_ARB: case GL_PROGRAM_NATIVE_TEMPORARIES_ARB: case
     * GL_PROGRAM_TEMPORARIES_ARB: case GL_PROGRAM_NATIVE_PARAMETERS_ARB: case
     * GL_PROGRAM_PARAMETERS_ARB: case GL_PROGRAM_NATIVE_ATTRIBS_ARB: case
     * GL_PROGRAM_ATTRIBS_ARB: case GL_PROGRAM_NATIVE_ADDRESS_REGISTERS_ARB:
     * case GL_PROGRAM_ADDRESS_REGISTERS_ARB: case
     * GL_PROGRAM_NATIVE_ALU_INSTRUCTIONS_ARB: case
     * GL_PROGRAM_ALU_INSTRUCTIONS_ARB: case
     * GL_PROGRAM_NATIVE_TEX_INSTRUCTIONS_ARB: case
     * GL_PROGRAM_TEX_INSTRUCTIONS_ARB: case
     * GL_PROGRAM_NATIVE_TEX_INDIRECTIONS_ARB: case
...
void APIENTRY_GL4ES gl4es_glGetProgramStringARB(GLenum target,
                                                GLenum pname,
                                                GLvoid *string)
{
