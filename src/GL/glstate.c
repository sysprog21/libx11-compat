...
static void free_texture(gltexture_t *tex)
{
    ...
    if (tex->data)
        free(tex->data);
    // renderbuffer linked to this texture will be freed by the
    // free_renderbuffer function.
    free(tex);
}
...
void *NewGLState(void *shared_glstate, int es2only)
{
    ...
    if (shared_glstate) {
        ...
        /* actual_tex2d caches which real GLES texture is bound per TMU. That is
         * a property of the real GLES context, not of the shared object tables,
         * so it must NOT be shared: libx11-compat backs each GLXContext with
         * its own EGL context, and a shared cache would let one context skip a
         * real bind the other never issued (mesa sharedtex renders untextured).
         * Kept per-glstate below; the logical texture objects (texture.list)
         * stay shared.
         *
         * Same class, still shared below because no current workload drives
         * shared GLX contexts through them: glsl->program (current program),
         * fbo.old (recycled real FBO ids), queries active state. They track
         * real per-EGL-context bindings too, so a shared-context
         * shader/FBO/query client would need the same per-glstate split applied
         * here. Left as is until a workload exercises it; the fix pattern is
         * this actual_tex2d change.
         */
        glstate->texture.list = copy_state->texture.list;
        glstate->glsl = copy_state->glsl;
        // gleshard is not shared (at least not the VA)
    ...
    // actual_tex2d: per-glstate (real GLES binding cache), never shared
    ...
    memcpy(&glstate->material.back, &glstate->material.front,
           sizeof(material_t));
    // Fog
    ...
    for (int i = 0; i < hardext.maxtex; i++) {
        ...
        glstate->texenv[i].env.op2_rgb = glstate->texenv[i].env.op0_alpha =
            glstate->texenv[i].env.op1_alpha =
...
void DeleteGLState(void *oldstate)
{
    ...
    free(state->actual_tex2d);  // per-glstate, always owned
...
#undef free_matrix
    ...
    // TODO: delete the "immediate" stuff and bitmap texture? scratch buffer
    if (state->scratch)
        free(state->scratch);
    // merger buffers
