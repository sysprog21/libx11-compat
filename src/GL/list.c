...
{
    // return true if renderlist contains only rendering command, no state
    // changes
    if (list->calls.len)
        return false;
    if (list->matrix_op)
...
bool islistscompatible_renderlist(renderlist_t *a, renderlist_t *b)
{
    ...
    if ((a->set_texture == b->set_texture) &&
        ((a->texture != b->texture) ||
         (a->target_texture != b->target_texture)))
        return false;
    if (!a->set_texture && b->set_texture)
        return false;
...
void renderlist_lineloop_lines(GLushort *ind,
                               int len,
                               GLushort *indices,
                               int count)
{
...
void renderlist_linestrip_lines(GLushort *ind,
                                int len,
                                GLushort *indices,
                                int count)
{
...
void renderlist_trianglestrip_triangles(GLushort *ind,
                                        int len,
                                        GLushort *indices,
                                        int count)
{
...
void renderlist_trianglefan_triangles(GLushort *ind,
                                      int len,
                                      GLushort *indices,
                                      int count)
{
...
void renderlist_quads_triangles(GLushort *ind,
                                int len,
                                GLushort *indices,
                                int count)
{
...
#define copy_a_indices    \
    ...
    case GL_LINE_LOOP:
        alloc_a_indices;
        renderlist_lineloop_lines(a->indices, a->indices ? a->ilen : a->len,
                                  newind, 0);
        a->mode = GL_LINES;
        copy_a_indices;
        break;
    ...
    default:
        if (!a->indices) {
            ...
            if (a->indice_cap < capindices) {
                a->indices = (GLushort *) realloc(
                    a->indices, capindices * sizeof(GLushort));
                a->indice_cap = capindices;
            }
        }
        break;
...
void doadd_renderlist(renderlist_t *a,
                      GLenum mode,
                      GLushort *indices,
                      int count,
                      int size_to_add)
{
    ...
    case GL_LINE_LOOP:
        ...
        break;
    case GL_LINE_STRIP:
        renderlist_linestrip_lines(indices, count, a->indices + a->ilen,
                                   a->len);
        break;
    case GL_QUAD_STRIP:
    case GL_TRIANGLE_STRIP:
    ...
    case GL_TRIANGLE_FAN:
        ...
        break;
    case GL_QUADS:
        renderlist_quads_triangles(indices, count, a->indices + a->ilen,
                                   a->len);
        break;
    default:
...
void append_renderlist(renderlist_t *a, renderlist_t *b)
{
    ...
    unsharedindices_renderlist(
        a, ((ilen_a) ? ilen_a : a->len) + ((ilen_b) ? ilen_b : b->len));
    // append arrays
    if (a->vert)
    ...
    if (a->fogcoord)
        memcpy(a->fogcoord + a->len * 1, b->fogcoord,
               b->len * 1 * sizeof(GLfloat));
    for (int i = 0; i < a->maxtex; i++)
        if (a->tex[i])
            memcpy(a->tex[i] + a->len * 4, b->tex[i],
                   b->len * 4 * sizeof(GLfloat));
    // indices
    if (!a->mode_inits)
...
renderlist_t *append_calllist(renderlist_t *list, renderlist_t *a)
{
    ...
    while (list->next)
        list = list->next;
    while (a) {
        if (ispurerender_renderlist(a) &&
            islistscompatible_renderlist(list, a)) {
            // append list!
...
#define PROCESS(W, T, C)                                                \
    if (list->W) {                                                      \
        ...
        kh_foreach_value(a->W, m, k = kh_put(W, list->W, C, &ret);      \
            ...
            PROCESS(light, renderlight_t,
                    m->pname | ((m->which - GL_LIGHT0) << 16));
            PROCESS(texgen, rendertexgen_t,
                    m->pname | ((m->coord - GL_S) << 16));
...
void free_renderlist(renderlist_t *list)
{
    ...
    do {
        ...
        if (list->light) {
            renderlight_t *m;
            kh_foreach_value(list->light, m, free(m);)
    ...
    if (list->use_glstate) {
        if (list->len >= glstate->merger_cap) {
            ...
            for (int a = 2; a < list->maxtex; a++)
                realloc_sublist(glstate->merger_tex[a - 2], 4,
                                glstate->merger_cap);
            if (list->vert)
...
void resize_indices_renderlist(renderlist_t *list, int n)
{
    ...
    if (list->use_glstate) {
        ...
        list->indices = (GLushort *) realloc(
            list->indices, list->indice_cap * sizeof(GLushort));
    }
}
...
void adjust_renderlist(renderlist_t *list)
{
    ...
    if (hardext.esversion == 1)  // no more adjustment needed for ES2
        for (int a = 0; a < list->maxtex; a++) {
            ...
            if ((list->tex[a]) && (itarget == ENABLED_TEXTURE_RECTANGLE) &&
                (bound)) {
                tex_coord_rect_arb(list->tex[a], list->tex_stride[a] >> 2,
                                   list->len, bound->width, bound->height);
            }
        }
}
