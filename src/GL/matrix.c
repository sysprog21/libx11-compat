...
static int update_current_identity(int I)
{
    ...
    case GL_TEXTURE:
        return glstate->texture_matrix[glstate->texture.active]->identity =
                   (I) ? 1
                       : is_identity(
                             TOP(texture_matrix[glstate->texture.active]));
    default:
        if (glstate->matrix_mode >= GL_MATRIX0_ARB &&
            glstate->matrix_mode < GL_MATRIX0_ARB + MAX_ARB_MATRIX)
            return glstate->arb_matrix[glstate->matrix_mode - GL_MATRIX0_ARB]
                       ->identity =
                       (I) ? 1
                           : is_identity(TOP(arb_matrix[glstate->matrix_mode -
                                                        GL_MATRIX0_ARB]));
        return 0;
    }
}
...
void APIENTRY_GL4ES gl4es_glMatrixMode(GLenum mode)
{
    ...
    if (!((mode == GL_MODELVIEW) || (mode == GL_PROJECTION) ||
          (mode == GL_TEXTURE) ||
          (mode >= GL_MATRIX0_ARB &&
           mode < (GL_MATRIX0_ARB + MAX_ARB_MATRIX)))) {
...
void APIENTRY_GL4ES gl4es_glPopMatrix(void)
{
    ...
    if (glstate->list.active && !(glstate->list.compiling) &&
        ...
        glstate->modelview_matrix->top > 0) {
...
void APIENTRY_GL4ES gl4es_glLoadMatrixf(const GLfloat *m)
{
    ...
    if (send_to_hardware()) {
        ...
        if (id)
            gles_glLoadIdentity();  // in case the driver as some special
                                    // optimisations
        else
...
void APIENTRY_GL4ES gl4es_glRotatef(GLfloat angle,
                                    GLfloat x,
                                    GLfloat y,
                                    GLfloat z)
{
...
void APIENTRY_GL4ES gl4es_glOrthof(GLfloat left,
                                   GLfloat right,
                                   GLfloat bottom,
                                   GLfloat top,
                                   GLfloat nearVal,
...
void APIENTRY_GL4ES gl4es_glFrustumf(GLfloat left,
                                     GLfloat right,
                                     GLfloat bottom,
                                     GLfloat top,
                                     GLfloat nearVal,
