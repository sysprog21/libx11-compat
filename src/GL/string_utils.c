...
char *gl4es_inplace_replace(char *pBuffer,
                            int *size,
                            const char *S,
                            const char *D)
{
    ...
    pBuffer = gl4es_resize_if_needed(
        pBuffer, size, (lD - lS) * gl4es_count_string(pBuffer, S));
    char *p = pBuffer;
    while ((p = strstr(p, S))) {
        // found an occurrence of S check if good to replace, strchr also found
        // '\0' :)
...
char *gl4es_inplace_insert(char *pBuffer,
                           const char *S,
                           char *master,
                           int *size)
{
...
int gl4es_count_string(const char *pBuffer, const char *S)
{
    ...
    int n = 0;
    while ((p = strstr(p, S))) {
        // found an occurrence of S check if good to count, strchr also found
        // '\0' :)
        if (strchr(AllSeparators, p[lS]) != NULL &&
            (p == pBuffer || strchr(AllSeparators, p[-1]) != NULL))
            n++;
...
char *gl4es_resize_if_needed(char *pBuffer, int *size, int addsize)
{
    ...
    if (newsize > *size) {
        newsize += 100;
        char *reloc = (char *) realloc(pBuffer, newsize);
        if (reloc) {
            p = reloc;
            *size = newsize;
        }
...
char *gl4es_next_blank(char *pBuffer)
{
    if (!pBuffer)
        return NULL;
    while (*pBuffer && !gl4es_is_blank(*pBuffer))
        ++pBuffer;
    return pBuffer;
}
...
char *gl4es_inplace_replace_simple(char *pBuffer,
                                   int *size,
                                   const char *S,
                                   const char *D)
{
    ...
    char *p = pBuffer;
    while ((p = strstr(p, S))) {
        // found an occurrence of S move out rest of string
        memmove(p + lD, p + lS, strlen(p) - lS + 1);
        // replace
        memcpy(p, D, strlen(D));
        // next
