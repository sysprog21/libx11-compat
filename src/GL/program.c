...
void APIENTRY_GL4ES gl4es_glBindAttribLocation(GLuint program,
                                               GLuint index,
                                               const GLchar *name)
{
...
void APIENTRY_GL4ES gl4es_glDeleteProgram(GLuint program)
{
    ...
    if (gles_glDeleteProgram) {
        ...
        noerrorShim();
    // TODO: check GL ERROR to not clean in case of error? clean attached
    // shaders
...
void APIENTRY_GL4ES gl4es_glGetActiveAttrib(GLuint program,
                                            GLuint index,
                                            GLsizei bufSize,
                                            GLsizei *length,
                                            GLint *size,
...
void APIENTRY_GL4ES gl4es_glGetAttachedShaders(GLuint program,
                                               GLsizei maxCount,
                                               GLsizei *count,
                                               GLuint *shaders)
{
...
void APIENTRY_GL4ES gl4es_glGetActiveUniform(GLuint program,
                                             GLuint index,
                                             GLsizei bufSize,
                                             GLsizei *length,
                                             GLint *size,
...
void APIENTRY_GL4ES gl4es_glGetProgramInfoLog(GLuint program,
                                              GLsizei maxLength,
                                              GLsizei *length,
                                              GLchar *infoLog)
{
...
void APIENTRY_GL4ES gl4es_glGetProgramiv(GLuint program,
                                         GLenum pname,
                                         GLint *params)
{
    ...
    case GL_ACTIVE_UNIFORM_MAX_LENGTH: {
        ...
        kh_foreach_value(
            glprogram->uniform, m,
            if (l < strlen(m->name) + 1) l = strlen(m->name) + 1;) *params = l;
    } break;
...
static void fill_program(program_t *glprogram)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        DBG(if (e2 == GL_NO_ERROR))
        {
            ...
            if (id != -1) {
                for (int j = 0; j < size; j++) {
                    ...
                    gluniform->type = type;
                    gluniform->cache_offs =
                        uniform_cache + j * uniformsize(type);
        ...
        DBG(else printf(
                "LIBGL: Warning, getting Uniform #%d info failed with %s\n", i,
                PrintEnum(e2));)
    }
    free(name);
    // reset uniform cache
    ...
    // Maybe Sampler uniform should not be initialized to 0, but to -1, to be
    ...
    if (glprogram->uniform) {
        ...
        kh_foreach(glprogram->uniform, k, m,
                   if (m->type == GL_SAMPLER_2D || m->type == GL_SAMPLER_CUBE)
                       memset((char *) glprogram->cache.cache + m->cache_offs,
                              0xff, m->cache_size);)
    }

    // Grab all Attrib
    gles_glGetProgramiv(glprogram->id, GL_ACTIVE_ATTRIBUTES, &n);
    gles_glGetProgramiv(glprogram->id, GL_ACTIVE_ATTRIBUTE_MAX_LENGTH,
                        &maxsize);
    ...
    for (int i = 0; i < n; i++) {
        ...
        DBG(if (e2 == GL_NO_ERROR))
        {
            gles_glGetActiveAttrib(glprogram->id, i, maxsize, NULL, &size,
                                   &type, name);
        ...
        DBG(else printf(
                "LIBGL: Warning, getting Attrib #%d info failed with %s\n", i,
                PrintEnum(e2));)
    }
    free(name);
}

int gl4es_useProgramBinary(GLuint program,
                           int length,
                           GLenum format,
                           const void *binary)
{
...
int gl4es_getProgramBinary(GLuint program,
                           int *length,
                           GLenum *format,
                           void **binary)
{
...
void APIENTRY_GL4ES gl4es_glGetProgramBinary(GLuint program,
                                             GLsizei bufSize,
                                             GLsizei *length,
                                             GLenum *binaryFormat,
                                             void *binary)
{
    ...
    CHECK_PROGRAM(void, program)
    LOAD_GLES_OES(glGetProgramBinary);
    gles_glGetProgramBinary(glprogram->id, bufSize, length, binaryFormat,
                            binary);
    errorGL();
}

void APIENTRY_GL4ES gl4es_glProgramBinary(GLuint program,
                                          GLenum binaryFormat,
                                          const void *binary,
                                          GLsizei length)
{
...
// GL_ARB_shader_objects some are not simple alias but wrappers
...
GLvoid APIENTRY_GL4ES gl4es_glGetInfoLog(GLhandleARB obj,
                                         GLsizei maxLength,
                                         GLsizei *length,
                                         GLcharARB *infoLog)
{
