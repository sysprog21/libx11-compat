...
GLuint APIENTRY_GL4ES gl4es_glCreateShader(GLenum shaderType)
{
    ...
    if (k == kh_end(shaders)) {
        ...
        glshader = kh_value(shaders, k) =
            (shader_t *) calloc(1, sizeof(shader_t));
    } else {
        glshader = kh_value(shaders, k);
    }
    glshader->id = shader;
...
void APIENTRY_GL4ES gl4es_glCompileShader(GLuint shader)
{
    ...
    if (gles_glCompileShader) {
        ...
        if (globals4es.logshader) {
            ...
            if (status != GL_TRUE) {
                printf(
                    "LIBGL: Error while compiling shader %d. Original source "
                    "is:\n%s\n=======\n",
...
void APIENTRY_GL4ES gl4es_glShaderSource(GLuint shader,
                                         GLsizei count,
                                         ...
                                         const GLint *length)
{
    ...
    CHECK_SHADER(void, shader)
    // get the size of the shader sources and than concatenate in a single
    // string
    int l = 0;
    ...
    if (gles_glShaderSource) {
        ...
        // #version 100)
        if (glstate->glsl->es2 &&
            !strncmp(glshader->source, "#version 100", 12))
...
void redoShader(GLuint shader, shaderconv_need_t *need)
{
    ...
    gles_glShaderSource(
        shader, 1,
        (const GLchar *const *) ((glshader->converted) ? (&glshader->converted)
                                                       : (&glshader->source)),
        NULL);
    // recompile...
...
void APIENTRY_GL4ES gl4es_glGetShaderSource(GLuint shader,
                                            GLsizei bufSize,
                                            GLsizei *length,
                                            GLchar *source)
{
...
void APIENTRY_GL4ES gl4es_glGetShaderInfoLog(GLuint shader,
                                             GLsizei maxLength,
                                             GLsizei *length,
                                             GLchar *infoLog)
{
...
void APIENTRY_GL4ES gl4es_glGetShaderiv(GLuint shader,
                                        GLenum pname,
                                        GLint *params)
{
    DBG(printf("glGetShaderiv(%d, %s, %p)\n", shader, PrintEnum(pname),
               params);)
    // find shader
    CHECK_SHADER(void, shader)
    LOAD_GLES2(glGetShaderiv);
...
void APIENTRY_GL4ES gl4es_glShaderBinary(GLsizei count,
