...
{
    sVariable *var = (sVariable *) calloc(
        1, sizeof(sVariable));  // malloc *should* be fine here
...
void freeStatus(sCurStatus *curStatus)
{
    if (curStatus->valueType == TYPE_INST_DECL) {
        ...
        char *strPtr;
        while ((strPtr = (char *) popArray(
                    (sArray *) &curStatus->curValue.newVar))) {
            free(strPtr);
        }
    ...
    sVariable *varPtr;
    while (
        (varPtr = (sVariable *) popArray((sArray *) &curStatus->variables))) {
        deleteVariable(&varPtr);
    }
    sInstruction *instPtr;
    while ((instPtr = (sInstruction *) popArray(
...
int appendString(sCurStatus *curStatusPtr, const char *str, size_t strLen)
{
    ...
    if (curStatusPtr->outLeft < strLen) {
        ...
        while (curStatusPtr->outLeft < strLen) {
            ...
            if (!resize((void **) &curStatusPtr->outputString,
                        &curStatusPtr->outCap, sizeof(char))) {
                return 1;
            }
        }
    ...
    ARBCONV_DBG_HEAVY(
        ARBCONV_DBG(char *dup = malloc((strLen + 1) * sizeof(char));
                    memcpy(dup, str, strLen); dup[strLen] = '\0';
                    ...
                    free(dup); fflush(stdout);) if (curStatusPtr->outputEnd !=
