...
void rebind_real_buff_arrays(int old_buffer, int new_buffer)
{
    for (int j = 0; j < hardext.maxvattrib; j++) {
        if (glstate->vao->vertexattrib[j].real_buffer == old_buffer) {
            ...
            /* if(!new_buffer) glstate->vao->vertexattrib[j].real_pointer = 0;
             */
        }
    }
}
                                       ...
                                       GLenum usage)
{
    ...
    for (int i = 0; i < hardext.maxvattrib; ++i) {
        ...
        if (v->buffer == buff) {
            ...
            // do not update real_pointer, as it's the relative start in the
            // buffer
        }
    }
    noerrorShim();
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (t) {  // don't allow to remove default one
                ...
                if (k != kh_end(list)) {
                    buff = kh_value(list, k);
                    if (buff->real_buffer) {
                        rebind_real_buff_arrays(buff->real_buffer,
                                                0);  // unbind
                                             ...
                                             GLvoid *data)
{
    ...
    if (buff == NULL)
        return;  // Should generate an error!
    if (offset < 0 || size < 0 || offset > buff->size ||
...
void *APIENTRY_GL4ES gl4es_glMapBufferRange(GLenum target,
                                            GLintptr offset,
                                            GLsizeiptr length,
                                            GLbitfield access)
{
                                              ...
                                              GLsizeiptr size)
{
    ...
    if (readOffset < 0 || writeOffset < 0 || size < 0 ||
        readOffset > readbuff->size || size > readbuff->size - readOffset ||
        writeOffset > writebuff->size || size > writebuff->size - writeOffset) {
        errorShim(GL_INVALID_VALUE);
        return;
    }
