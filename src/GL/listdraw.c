        ...
        if (i)
            for (int j = i - 1; j < i; ++j) {
                ...
                if (r->real_base < t->real_base + t->real_size) {
                    base = t->vbo_base + (r->real_base - t->real_base);
                    basebase = t->vbo_basebase;
                    break;
                }
            }
        r->vbo_base = base;
...
void listActiveVBO(renderlist_t *list, save_vbo_t *saved)
{
    if (list->vert) {
        ...
        saved[ATT_VERTEX].buffer =
            glstate->vao->vertexattrib[ATT_VERTEX].buffer;
    ...
    if (list->normal) {
        ...
        saved[ATT_NORMAL].buffer =
            glstate->vao->vertexattrib[ATT_NORMAL].buffer;
...
int fill_lineIndices(modeinit_t *modes,
                     int length,
                     GLenum mode,
                     GLushort *indices,
                     GLushort *ind_line)
...
#define ind(a) indices ? indices[a] : (a)
    ...
    for (int m = 0; m < length; m++) {
        ...
        case GL_TRIANGLES:
            ...
            case GL_QUADS:
                if (len > 3) {
                    // 4 lines per quads, but dest may already be a triangles
                    // list...
                    if (len == 4) {
                        // just 1 Quad
                        for (; i < 4; i++) {
    ...
    do {
        ...
        if (list->raster_op) {
            if (list->raster_op == 1) {
                ...
                gl4es_glPixelTransferf(list->raster_op & 0xFFFF,
                                       list->raster_xyz[0]);
            }
        }
        if (list->raster) {
        ...
        if (list->bitmaps) {
            for (int i = 0; i < list->bitmaps->count; i++) {
                ...
                gl4es_glBitmap(l->width, l->height, l->xorig, l->yorig,
                               l->xmove, l->ymove, l->bitmap);
            }
        }
        ...
        if (list->linestipple_op) {
            gl4es_glLineStipple(list->linestipple_factor,
                                list->linestipple_pattern);
        }
                    ...
                    if (indices) {
                        for (int i = 0; i < list->ilen; i++)
                            for (int j = 0; j < 4; j++) {
                                ...
                                list->final_colors[k] =
                                    list->color[k] + list->secondary[k];
                            }
                    } else {
                        for (int i = 0; i < list->len * 4; i++)
                            list->final_colors[i] =
                                list->color[i] + list->secondary[i];
                    }
                }
                gles_glColorPointer(4, GL_FLOAT, 0, list->final_colors);
            } else {
                gles_glColorPointer(4, GL_FLOAT, list->color_stride,
                                    list->color);
            }
        } else {
        ...
        if (hardext.esversion > 1) {
            ...
            if ((glstate->fog.coord_src == GL_FOG_COORD) && (list->fogcoord)) {
                ...
                fpe_glFogCoordPointer(GL_FLOAT, list->fogcoord_stride,
                                      list->fogcoord);
            } else {
    ...
    if (cur_tex != A) {                               \
        ...
        if (stipple) {
            ...
            list->tex[stipple_tmu] = gen_stipple_tex_coords(
                list->vert, list->indices,
                list->mode_inits ? list->mode_inits : &tmp, list->vert_stride,
                list->mode_inits ? list->mode_init_len : 1,
                (list->use_glstate) ? (list->vert + 8 + stipple_tmu * 4)
                                    : NULL);
        }
#define RS(A, len)                                                \
        ...
        if (hardext.esversion == 1) {
            for (int a = 0; a < hardext.maxtex; a++) {
                if (glstate->enable.texture[a] ||
                    ...
                    needclean[a] = 0;
                    use_texgen[a] = 0;
                    if ((glstate->enable.texgen_s[a] ||
                         glstate->enable.texgen_t[a] ||
                         glstate->enable.texgen_r[a] ||
                         glstate->enable.texgen_q[a])) {
                        TEXTURE(a);
                        RS(a, list->len);
                        gen_tex_coords(
                            list->vert, list->normal, &glstate->texgened[a],
                    ...
                    if ((list->tex[a] || (use_texgen[a] && !needclean[a])) &&
                        ((!(globals4es.texmat ||
                            glstate->texture_matrix[a]->identity)) ||
                         (bound->adjust))) {
                        if (!use_texgen[a]) {
                            RS(a, list->len);
                            if (list->tex_stride[a]) {
                                ...
                                int stride = list->tex_stride[a] >>
                                             2;  // stride need to be a multiple
                                                 // of 4 (i.e. sizeof(GLfloat))
                        ...
                        if (!(globals4es.texmat ||
                              glstate->texture_matrix[a]->identity))
                            tex_coord_matrix(glstate->texgened[a], list->len,
                                             getTexMat(a));
                        if (bound->adjust) {
                            tex_coord_npot(glstate->texgened[a], list->len,
                                           bound->width, bound->height,
                                           bound->nwidth, bound->nheight);
                        }
                    }
                }
                if ((list->tex[a] ||
                    ...
                    if (glstate->gleshard->vertexattrib[ATT_MULTITEXCOORD0 + a]
                            .enabled ||
                        (hardext.esversion != 1)) {  // optim for ES1.1 to avoid
                                                     // useless texture switch
                        TEXTURE(a);
...
#undef TEXTURE
        ...
        if (indices) {
            if (glstate->render_mode == GL_SELECT) {
                ...
                vtx.size = 4;
                vtx.stride = 0;
                select_glDrawElements(&vtx, list->mode, list->ilen,
                ...
                if (glstate->polygon_mode == GL_LINE &&
                    ...
                    if (!list->ind_lines) {
                        GLushort *ind_line = list->ind_lines =
                            (GLushort *) malloc(sizeof(GLushort) * ilen * 4 +
                                                2);
                        modeinit_t tmp;
                        tmp.mode_init = list->mode_init;
                        tmp.ilen = list->ilen;
                        int k = fill_lineIndices(
                    ...
                    if (list->instanceCount == 1)
                        ...
                        for (glstate->instanceID = 0;
                             ...
                             ++glstate->instanceID)
                            gles_glDrawElements(mode, list->ilen,
                                                GL_UNSIGNED_SHORT,
                                                vbo_indices ? NULL : indices);
