              ...
              char **versionString)
{
    ...
    khash_t(alldefine) *alldefines =
        kh_init(alldefine);  // will conatin all defines, even the one without
                             // int inside
    ...
    while (tok.type != TK_NULL) {
        ...
        if (tok.type != TK_NULL) {
            ...
            case 100:
                ...
                break;
            case 120:
                status = 100;  // continue comment, what ever follow (NewLine or
                               // anything else)
                break;
            ...
            case 620:
                if (tok.type == TK_SPACE) {
                    ...
                    defval = tok.integer;
                    status =
                        630;  // check if end of line (so it's a simple define)
                } else if (tok.type == TK_NEWLINE) {
                    {
                        int l = p - oldp;
                        if (l > MAXSTR - 1) {
            ...
            case 630:
                if (tok.type == TK_SPACE) {
                    ...
                    status = 0;  // ok, define added to collection, left the
                                 // line as-is anyway
                } else {
                    status = 399;  // fallback
                }
            ...
            case 710:
                if (tok.type == TK_SPACE) {
                    ...
                    if (!indefined)
                        status = 0;  // ok... no handling of #defined for now,
                                     // so just write through the line
                } /* else {
            ...
            if (notok)
                ...
                if (!incomment && !indefined && tok.type == TK_TEXT) {
                    ...
                    if (k != kh_end(defines)) {
                        ...
                        sprintf(tok.str, "%d",
                                v);  // overide define with defined value
                    }
                }
                int l = strlen(tok.str);
                if (sz + l >= cap) {
