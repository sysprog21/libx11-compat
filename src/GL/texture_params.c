 ...
 *
 * Have to check is ARB_RECTANGLE is used Apply texture matrix if not identity
                        ...
                        vertexattrib_t *ptr)
{
    ...
    if (changes) {
        ...
        copy_gl_pointer_tex_noalloc(glstate->helper_tex[texunit], ptr, 4, 0,
                                    len);
        // Normalize if needed
        if (itarget == ENABLED_TEXTURE_RECTANGLE)
            tex_coord_rect_arb(glstate->helper_tex[texunit], 4, len,
                               bound->width, bound->height);
        // Apply transformation matrix if any
        if (!(globals4es.texmat || glstate->texture_matrix[texunit]->identity))
            tex_coord_matrix(glstate->helper_tex[texunit], len,
                             getTexMat(texunit));
        // NPOT adjust
        if (bound->adjust)
    ...
    case GL_MIRRORED_REPEAT_OES:
        if (globals4es.defaultwrap == 2 && hardext.npot < 3 && !texture->valid)
            wrap = GL_CLAMP_TO_EDGE;
                 ...
                 texture->npot) {
            // should "upgrade" the texture to POT size... printf("Warning,
            // REPEAT / MIRRORED_REPEAT on NPOT texture\n");
            wrap = GL_CLAMP_TO_EDGE;  // repeat is not support on NPOT with
                                      // limited_npot
        }
        break;
    }
...
void APIENTRY_GL4ES gl4es_glTexParameterfv(GLenum target,
                                           GLenum pname,
...
void APIENTRY_GL4ES gl4es_glTexParameteriv(GLenum target,
                                           GLenum pname,
                                           const GLint *params)
{
    GLfloat fparams[4];
                ...
                for (a = 0; a < MAX_TEX; a++) {
                    ...
                    for (int j = 0; j < ENABLED_TEXTURE_LAST; j++)
                        if (tex == glstate->texture.bound[a][j]) {
                            glstate->texture.bound[a][j] =
                                glstate->texture.zero;
                            found = 1;
                        }
...
void APIENTRY_GL4ES gl4es_glGetTexLevelParameterfv(GLenum target,
                                                   GLint level,
                                                   GLenum pname,
                                                   GLfloat *params)
{
    ...
    if (!getSamplerParameterfv(&bound->sampler, pname, params)) {
        ...
        case GL_TEXTURE_WIDTH:
            if (rtarget == GL_PROXY_TEXTURE_2D)
                ...
                (*params) =
                    nlevel((bound) ? bound->width : hardext.maxsize, level);
        ...
        case GL_TEXTURE_HEIGHT:
            if (rtarget == GL_PROXY_TEXTURE_2D)
                ...
                (*params) =
                    nlevel((bound) ? bound->height : hardext.maxsize, level);
                if (level && !(bound->mipmap_auto || bound->mipmap_need))
                    (*params) = 0;  // Mipmap level not loaded
            }
            break;
        ...
        case GL_TEXTURE_COMPRESSED_IMAGE_SIZE:
            if ((bound->orig_internal == GL_COMPRESSED_RGB) ||
                (bound->orig_internal == GL_COMPRESSED_RGBA)) {
                int w = nlevel((bound->width >> level),
                               2);  // DXT works on 4x4 blocks...
                int h = nlevel((bound->height >> level), 2);
                w <<= 2;
                h <<= 2;
                if (bound->orig_internal == GL_COMPRESSED_RGB ||
                    bound->wanted_internal ==
                        GL_COMPRESSED_RGBA_S3TC_DXT1_EXT)  // DXT1, 64bits (i.e.
                                                           // size=8) for a 4x4
                                                           // block
                    (*params) = (w * h) / 2;
        ...
        case GL_TEXTURE_INTENSITY_SIZE:
            if (bound)
                (*params) =
                    32;  // is it correct ? GLES doesn't store Intensity...
                         // Shall I return 0 instead? Or fake it and return 8?
            else
                (*params) = 0;
            break;
...
void APIENTRY_GL4ES gl4es_glPixelStorei(GLenum pname, GLint param)
{
    ...
    case GL_PACK_SWAP_BYTES:
        // Fake... TODO? glstate->texture.unpack_lsb_first = param;
        return;
    ...
    for (int i = 0; i < glstate->bound_changed; i++) {
        ...
        if (tgt != ENABLED_CUBE_MAP) {  // CUBE MAP are immediately bound
            if (t != glstate->actual_tex2d[i]) {
                ...
                DBG(printf("Binding %s/%s[%d]:Texture[%u] (sampler[%d]=%p)\n",
                           PrintEnum(to_target(tgt)), PrintEnum(target), i, t,
                           i, glstate->samplers.sampler[i]);)
                gles_glBindTexture(GL_TEXTURE_2D, t);
        ...
        if (drawing) {
            ...
            if (tex->mipmap_need && !tex->mipmap_done) {
                if (!tex->mipmap_auto) {
                    // should check if glGenerateMipmap exist, and fall back to
                    // no mipmap if not
