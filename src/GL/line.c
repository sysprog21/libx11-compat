...
GLfloat *gen_stipple_tex_coords(GLfloat *vert,
                                GLushort *sindices,
                                modeinit_t *modes,
                                int stride,
                                int length,
    ...
    for (int k = 0; k < length; k++) {
        ...
        if (mode == GL_LINES ||
            ...
            for (; i < modes[k].ilen; i += 2) {
                ...
                DBG(printf("%f->%f (%f,%f -> %f,%f)\t", oldlen, len, x1, y1, x2,
                           y2);)
                if (sindices)
                    texPos =
                        tex +
                        texstride *
                            sindices[i +
                                     0];  // it gets written 2*, but that should
                                          // be ok, it's the same value
                ...
                // (well, last segment, the "loop" one, will look strange, but I
                // will not add a vertex for that)
            if (sindices)
                vertPos = vert + stride * sindices[i];
            vector_matrix(vertPos, mvp, v);
            x2 = (v[0] / v[3]) * w;
