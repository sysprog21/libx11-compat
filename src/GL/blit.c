                             ...
                             GLint mode)
{
    ...
    if (drawtexok) {
        ...
        gles_glTexParameteriv(GL_TEXTURE_2D, GL_TEXTURE_CROP_RECT_OES,
                              sourceRect);
        ...
        // TODO: do something with width / height of ViewPort? then draw it
        ...
        GLfloat w2 =
            2.0f / (customvp ? vpwidth : glstate->raster.viewport.width);
        GLfloat h2 =
            2.0f / (customvp ? vpheight : glstate->raster.viewport.height);
        ...
        for (int a = 1; a < hardext.maxtex; a++)
            if (glstate->gleshard->vertexattrib[ATT_MULTITEXCOORD0 + a]
                    .enabled) {
...
/* Delete any blit shaders/programs already created, then free the state. The
 * struct is memset to 0, so a field for an object not yet created is 0, which
 * gles_glDeleteShader/gles_glDeleteProgram silently ignore, so this is safe at
 * every partial-initialization failure point.
 */
static void free_blit_gles2(void)
{
    if (!glstate->blit)
        return;
    LOAD_GLES2(glDeleteShader);
    LOAD_GLES2(glDeleteProgram);
    gles_glDeleteShader(glstate->blit->pixelshader);
    gles_glDeleteShader(glstate->blit->pixelshader_alpha);
    gles_glDeleteShader(glstate->blit->vertexshader);
    gles_glDeleteShader(glstate->blit->vertexshader_alpha);
    gles_glDeleteProgram(glstate->blit->program);
    gles_glDeleteProgram(glstate->blit->program_alpha);
    free(glstate->blit);
    glstate->blit = NULL;
}
                             ...
                             GLint mode)
{
    ...
    if (!glstate->blit) {
        ...
        gles_glGetShaderiv(glstate->blit->pixelshader, GL_COMPILE_STATUS,
                           &success);
        if (!success) {
        ...
        glstate->blit->pixelshader_alpha =
            gles_glCreateShader(GL_FRAGMENT_SHADER);
        gles_glShaderSource(glstate->blit->pixelshader_alpha, 1,
                            (const char **) src, NULL);
        ...
        if (!success) {
            LOAD_GLES(glGetShaderInfoLog);
            char log[400];
            gles_glGetShaderInfoLog(glstate->blit->pixelshader_alpha, 399, NULL,
                                    log);
        ...
        if (!success) {
            LOAD_GLES(glGetShaderInfoLog);
            char log[400];
            gles_glGetShaderInfoLog(glstate->blit->vertexshader, 399, NULL,
                                    log);
        ...
        glstate->blit->vertexshader_alpha =
            gles_glCreateShader(GL_VERTEX_SHADER);
        ...
        if (!success) {
            LOAD_GLES(glGetShaderInfoLog);
            char log[400];
            gles_glGetShaderInfoLog(glstate->blit->vertexshader_alpha, 399,
                                    NULL, log);
        ...
        gles_glAttachShader(glstate->blit->program,
                            glstate->blit->vertexshader);
        ...
        gles_glUniform1i(
            gles_glGetUniformLocation(glstate->blit->program, "uTex"), 0);
        ...
        gles_glGetProgramiv(glstate->blit->program_alpha, GL_LINK_STATUS,
                            &success);
        if (!success) {
            SHUT_LOGE("Failed to link blit program.\n");
            /* program is bound (glUseProgram above); restore before deleting.
             */
