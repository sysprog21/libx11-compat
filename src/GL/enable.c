...
static void proxy_glEnable(GLenum cap,
                           bool enable,
...
#define clientGO_proxyFPE(constant, name) \
    ...
    case GL_BLEND:
        ...
        proxy_GOFPE(GL_LIGHTING, lighting,
                    glstate->fpe_state->lighting = enable);
        ...
        // cannot use clientGO_proxyFPE here, has the ClientArray are really
        // enabled / disabled elsewhere in fact (inside glDraw or list_draw)
    ...
    case GL_TEXTURE_2D:
        if (glstate->list.pending &&
            ((glstate->enable.texture[glstate->texture.active] >>
             ...
             1) != enable)
            gl4es_flush();
        if (enable == ((glstate->enable.texture[glstate->texture.active] >>
                        ENABLED_TEX2D) &
                       1))
            return;  // no change
        if (enable)
...
{
    DBG(printf("glIsEnabed(%s), list.begin=%d, list.compiling=%d, "
               "list.pending=%d\n",
    ...
    if (glstate->list.active && !glstate->list.pending)
        gl4es_flush();
    GLboolean gles_glIsEnabled(
        glIsEnabled_ARG_EXPAND);  // LOAD_GLES(glIsEnabled);
    noerrorShim();
    switch (cap) {
