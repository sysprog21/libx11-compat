...
void readfboBegin()
{
    if (glstate->fbo.fbo_read == glstate->fbo.fbo_draw)
        return;
    DBG(printf(
            "readfboBegin, fbo status read=%u, draw=%u, main=%u, current=%u\n",
    ...
    if (glstate->fbo.framebufferlist)
        for (int i = 0; i < n; i++) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (t) {
                    k = kh_get(framebufferlist_t, glstate->fbo.framebufferlist,
                               t);
                    if (k != kh_end(glstate->fbo.framebufferlist)) {
                        ...
                        for (int j = 0; j < MAX_DRAW_BUFFERS; ++j) {
                            if (fb->color[j] &&
                                fb->t_color[j] != GL_RENDERBUFFER) {
                                gltexture_t *tex = gl4es_getTexture(
                                    fb->t_color[j], fb->color[j]);
                                if (tex) {
                                    tex->binded_fbo = 0;
                        ...
                        if (fb->depth && fb->t_depth != GL_RENDERBUFFER) {
                            gltexture_t *tex =
                                gl4es_getTexture(fb->t_depth, fb->depth);
                            if (tex) {
                                tex->binded_fbo = 0;
                                tex->binded_attachment = 0;
                                tex->renderdepth = 0;
                        ...
                        if (fb->stencil && fb->t_stencil != GL_RENDERBUFFER) {
                            gltexture_t *tex =
                                gl4es_getTexture(fb->t_stencil, fb->stencil);
                            if (tex) {
                                tex->binded_fbo = 0;
                                tex->binded_attachment = 0;
                                tex->renderstencil = 0;
                        ...
                        free(fb);
                        kh_del(framebufferlist_t, glstate->fbo.framebufferlist,
                               k);
                    }
                }
            }
...
void SetAttachment(glframebuffer_t *fb,
                   GLenum attachment,
                   GLenum atttarget,
                   GLuint att,
                   int level)
                                                 ...
                                                 GLint level)
{
        ...
        if (!tex) {
            ...
            if (globals4es.fbtexscale > 0.0f || tex->shrink || tex->useratio ||
                ...
                LOGD("%s texture for FBO\n",
                     (tex->useratio) ? "going back to npot size pot'ed"
                ...
                gles_glTexImage2D(GL_TEXTURE_2D, 0, tex->format, tex->nwidth,
                                  tex->nheight, 0, tex->format, tex->type,
                                  NULL);
                if (oldtex != tex->glname)
            ...
            if (tex->format == GL_BGRA &&
                (globals4es.nobgra || !hardext.bgra8888)) {
                if (need_change < 2)
                    need_change += 2;
                tex->format = GL_RGBA;
            }
            if (need_change) {
                ...
                if (need_change & 1) {
                    ...
                    tex->adjustxy[0] = (float) tex->width / tex->nwidth;
                    tex->adjustxy[1] = (float) tex->height / tex->nheight;
                    tex->adjust = (tex->width != tex->nwidth ||
                                   tex->height != tex->nheight);
                }
                int oldactive = glstate->texture.active;
                if (oldactive)
                    gles_glActiveTexture(GL_TEXTURE0);
            ...
            DBG(printf("found texture, glname=%u, size=%ix%i(%ix%i), "
                       "format/type=%s/%s\n",
                       texture, tex->width, tex->height, tex->nwidth,
                       tex->nheight, PrintEnum(tex->format),
                       PrintEnum(tex->type));)
        }
    }
    ...
    if (old_attachment) {
        gltexture_t *old =
            gl4es_getTexture(old_attachment_type, old_attachment);
        if (old) {
    ...
    SetAttachment(fb, attachment, textarget, tex ? tex->texture : texture,
                  level);
        ...
        if (!tex->actual.min_filter || !minmag_npot(tex->actual.min_filter) ||
            ...
            if (hardext.npot < 2) {
                tex->sampler.min_filter =
                    minmag_forcenpot(tex->sampler.min_filter);
                tex->adjust = 0;
                tex->mipmap_need = 0;
                tex->mipmap_auto = 0;
            }
    ...
    if (attachment == GL_DEPTH_ATTACHMENT /*&& hardext.depthtex==0*/) {
        ...
        if (hardext.depthtex && (tex || !texture)) {
            // depth texture supported! check if texture needs to be re-created
            // as true depth texture
            if (tex && !(tex->format == GL_DEPTH_COMPONENT ||
                         tex->format == GL_DEPTH_STENCIL)) {
                tex->format = GL_DEPTH_COMPONENT;
                if (tex->type != GL_UNSIGNED_INT &&
                    tex->type != GL_UNSIGNED_SHORT && tex->type != GL_FLOAT)
                    tex->type =
            ...
            gles_glFramebufferTexture2D(ntarget, attachment, GL_TEXTURE_2D,
                                        texture, 0);
        } else {
            // let's create a renderbuffer and attach it instead of the
            // (presumably) depth texture
            if (tex && !tex->renderdepth) {
                ...
                gl4es_glRenderbufferStorage(
                    GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, twidth, theight);
    ...
    if (attachment == GL_STENCIL_ATTACHMENT /*&& hardext.depthtex==0*/) {
        ...
        // DEPTH+DEPTH_STENCIL extension of GL_STENCIL_INDEX8 with STENCIL
        // extension and having DEPTH+DEPTH_STENCIL extension doesn't grant
        // STENCIL8 texture extension!
        if ((tex || !texture) && (hardext.stenciltex ||
                                  (hardext.depthtex && hardext.depthstencil))) {
            // depth texture supported, so are stencil one then! check if
            // texture needs to be re-created as true depth texture
            if (tex && !(tex->format == GL_STENCIL_INDEX8 ||
                         tex->format == GL_DEPTH_STENCIL)) {
                if (tex->format == GL_DEPTH_ATTACHMENT) {
                    // TODO: need to create a new texture, as the depth one is
                    // probably used
                    ...
                    if (oldactive)
                        gles_glActiveTexture(GL_TEXTURE0);
                    gltexture_t *bound =
                        glstate->texture.bound[0 /*glstate->texture.active*/]
                                              [ENABLED_TEX2D];
                    GLuint oldtex = bound->glname;
                    ...
                    gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                                         GL_CLAMP_TO_EDGE);
                    tex->format = (hardext.stenciltex) ? GL_STENCIL_INDEX8
                                                       : GL_DEPTH_STENCIL;
                    tex->type = (hardext.stenciltex) ? GL_UNSIGNED_BYTE
                                                     : GL_UNSIGNED_INT_24_8;
                    gl4es_glTexImage2D(GL_TEXTURE_2D, 0, tex->format, nwidth,
                                       nheight, 0, tex->format, tex->type,
                                       NULL);
                    if (oldtex != tex->glname)
                        gles_glBindTexture(GL_TEXTURE_2D, oldtex);
                    if (oldactive)
                    ...
                    gles_glTexImage2D(GL_TEXTURE_2D, 0, tex->format,
                                      tex->nwidth, tex->nheight, 0, tex->format,
                                      tex->type, NULL);
                    if (oldtex != tex->glname)
                        gles_glBindTexture(GL_TEXTURE_2D, oldtex);
                    if (oldactive)
            ...
            if (tex && !tex->renderstencil) {
                ...
                gl4es_glRenderbufferStorage(GL_RENDERBUFFER, GL_STENCIL_INDEX8,
                                            twidth, theight);
                gl4es_glBindRenderbuffer(GL_RENDERBUFFER, 0);
            }
    ...
    if (attachment == GL_DEPTH_STENCIL_ATTACHMENT /*&& hardext.depthtex==0*/) {
        ...
        if (hardext.depthstencil /*&& !(hardext.depthtex && tex)*/) {
            if (hardext.depthtex && (tex || !texture)) {
                ...
                gles_glFramebufferTexture2D(ntarget, GL_DEPTH_ATTACHMENT,
                ...
                if (tex && !tex->renderdepth) {
                    ...
                    gl4es_glBindRenderbuffer(GL_RENDERBUFFER, tex->renderdepth);
                    gl4es_glRenderbufferStorage(
                        GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, twidth, theight);
                    gl4es_glBindRenderbuffer(GL_RENDERBUFFER, 0);
                }
                errorGL();
                gl4es_glFramebufferRenderbuffer(ntarget, GL_DEPTH_ATTACHMENT,
            ...
            if (hardext.depthtex && (tex || !texture)) {
                // depth texture supported! check if texture needs to be
                // re-created ad true depth texture
                if (tex && tex->format != GL_DEPTH_COMPONENT) {
                    tex->format = GL_DEPTH_COMPONENT;
                    if (tex->type != GL_UNSIGNED_INT &&
                        tex->type != GL_UNSIGNED_SHORT && tex->type != GL_FLOAT)
                        tex->type = (hardext.depth24) ? GL_UNSIGNED_INT
                                                      : GL_UNSIGNED_SHORT;
                    tex->fpe_format = FPE_TEX_DEPTH;
                    int oldactive = glstate->texture.active;
                    if (oldactive)
    ...
    if (level != 0) {
        ...
        if ((scrap_width != twidth) || (scrap_height != theight)) {
            ...
            gles_glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, scrap_width,
                              scrap_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
            if (oldtex != scrap_tex)
                gles_glBindTexture(GL_TEXTURE_2D, oldtex);
        }
        texture = scrap_tex;
        ...
        if (rend->renderbuffer) {
            ...
            if (!rend->secondarytexture) {
                ...
                gl4es_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                                      GL_LINEAR);
                gl4es_glTexImage2D(GL_TEXTURE_2D, 0, format, width, height, 0,
            ...
            gl4es_glFramebufferTexture2D(target, attachment, GL_TEXTURE_2D, 0,
                                         0);
        }
        return;
    }
    ...
    if (glstate->fbo.renderbufferlist)
        for (int i = 0; i < n; i++) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (t) {
                    k = kh_get(renderbufferlist_t,
                               glstate->fbo.renderbufferlist, t);
                    if (k != kh_end(glstate->fbo.renderbufferlist)) {
                        ...
                        if (rend->secondarybuffer)
                            gles_glDeleteRenderbuffers(1,
                                                       &rend->secondarybuffer);
                        if (rend->secondarytexture)
                            gl4es_glDeleteTextures(1, &rend->secondarytexture);
                        free(rend);
                        kh_del(renderbufferlist_t,
                               glstate->fbo.renderbufferlist, k);
                    }
                }
            }
        }
                                                ...
                                                GLsizei height)
{
    ...
    height = (hardext.npot > 0 && !globals4es.potframebuffer) ? height
                                                              : npot(height);
    ...
    if (rend->secondarybuffer) {
        if (use_secondarybuffer) {
            ...
            gles_glRenderbufferStorage(target, GL_STENCIL_INDEX8, width,
                                       height);
    ...
    if (rend->secondarytexture) {
        ...
        gltexture_t *tex =
            gl4es_getTexture(GL_TEXTURE_2D, rend->secondarytexture);
        LOAD_GLES(glActiveTexture);
        LOAD_GLES(glBindTexture);
        LOAD_GLES(glTexImage2D);
        int oldactive = glstate->texture.active;
...
void APIENTRY_GL4ES
gl4es_glRenderbufferStorageMultisample(GLenum target,
                                       GLsizei samples,
                                       GLenum internalformat,
                                       GLsizei width,
                                       GLsizei height)
...
void APIENTRY_GL4ES
gl4es_glGetFramebufferAttachmentParameteriv(GLenum target,
                                            GLenum attachment,
                                            GLenum pname,
                                            GLint *params)
{
    ...
    // more stuff can be done hack to return DEPTH size
    if (attachment == GL_DEPTH_ATTACHMENT &&
        ...
        if (hardext.depthtex == 0) {
            ...
            *params = GetAttachment(fb, attachment);
            // TODO: Check all this?!
            if (*params)
                *params = 16;  // Depth buffer is 16 on GLES. No check for 24
                               // bits here...
            return;
        }
        // check if it's depth/stencil 16/8, in that case, lie by spoofing 24/8
        // or some FNA game will fail
        int depth, stencil;
        GLenum ntarget = ReadDraw_Push(target);
...
void createMainFBO(int width, int height)
{
    ...
    glstate->fbo.mainfbo_nwidth = width =
        hardext.npot > 0 ? width : npot(width);
    ...
    gles_glTexImage2D(GL_TEXTURE_2D, 0,
                      globals4es.fbo_noalpha ? GL_RGB : GL_RGBA, width, height,
                      0, globals4es.fbo_noalpha ? GL_RGB : GL_RGBA,
    ...
    gles_glRenderbufferStorage(GL_RENDERBUFFER, GL_STENCIL_INDEX8, width,
                               height);
...
void APIENTRY_GL4ES gl4es_glBlitFramebuffer(GLint srcX0,
                                            GLint srcY0,
                                            GLint srcX1,
                                            GLint srcY1,
                                            GLint dstX0,
                                            ...
                                            GLenum filter)
{
    // mask will be ignored filter will be taken only for ReadFBO has no Texture
    // attached (so readpixel is used)
    DBG(printf("glBlitFramebuffer(%d, %d, %d, %d,  %d, %d, %d, %d,  0x%04X, "
               "%s) fbo_read=%d, fbo_draw=%d\n",
               srcX0, srcY0, srcX1, srcY1, dstX0, dstY0, dstX1, dstY1, mask,
               PrintEnum(filter), glstate->fbo.fbo_read->id,
               glstate->fbo.fbo_draw->id);)

    if ((mask & GL_COLOR_BUFFER_BIT) == 0)
        return;  // cannot copy DEPTH or STENCIL data on GLES, only
                 // COLOR_BUFFER...
    ...
    GLuint texture =
        (glstate->fbo.fbo_read->id == 0 && glstate->fbo.mainfbo_fbo)
...
void APIENTRY_GL4ES gl4es_glClearBufferiv(GLenum buffer,
                                          GLint drawbuffer,
                                          const GLint *value)
{
    noerrorShim();
    ...
    case GL_COLOR:
        ...
        if (!(attch >= GL_COLOR_ATTACHMENT0 &&
            ...
            // how to convert the value? Most FB will be 8bits / component for
            // now...
            gl4es_glClearColor(value[0] / 127.0f, value[1] / 127.0f,
                               value[2] / 127.0f, value[3] / 127.0f);
            gl4es_glClear(GL_COLOR_BUFFER_BIT);
            gl4es_glClearColor(oldclear[0], oldclear[1], oldclear[2],
                               oldclear[3]);
...
void APIENTRY_GL4ES gl4es_glClearBufferuiv(GLenum buffer,
                                           GLint drawbuffer,
                                           const GLuint *value)
{
    noerrorShim();
...
void APIENTRY_GL4ES gl4es_glClearBufferfv(GLenum buffer,
                                          GLint drawbuffer,
                                          const GLfloat *value)
{
    noerrorShim();
...
void APIENTRY_GL4ES gl4es_glClearBufferfi(GLenum buffer,
                                          GLint drawbuffer,
                                          GLfloat depth,
                                          GLint stencil)
{
...
void APIENTRY_GL4ES gl4es_glColorMaskIndexed(GLuint framebuffer,
                                             GLboolean red,
                                             GLboolean green,
                                             GLboolean blue,
                                             GLboolean alpha)
...
void gl4es_saveCurrentFBO()
{
    ...
    if (framebuffer) {
        ...
        if (hardext.vendor & VEND_ARM)
            gl4es_glFinish();  // MALI seems to need a flush commandbefore
                               // unbinding the Framebuffer here
        gles_glBindFramebuffer(GL_FRAMEBUFFER, 0);
    }
}
