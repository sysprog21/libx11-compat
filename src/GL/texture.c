...
// expand non-power-of-two sizes TODO: what does this do to repeating textures?
int npot(int n)
{
    if (n == 0)
        return 0;
...
static void *swizzle_texture(GLsizei width,
                             GLsizei height,
                             GLenum *format,
                             GLenum *type,
                             GLenum intermediaryformat,
                             ...
                             gltexture_t *bound)
{
    ...
    if (*format != intermediaryformat || intermediaryformat != internalformat) {
        ...
        if ((*type) == GL_HALF_FLOAT)
            (*type) = GL_HALF_FLOAT_OES;  // the define is different between GL
                                          // and GLES...
        switch (*format) {
        case GL_R:
        case GL_RED:
        ...
        case GL_BGRA:
            if (hardext.bgra8888 &&
                ...
                convert = 1;
                if (hardext.bgra8888 && (*type == GL_UNSIGNED_INT_8_8_8_8)) {
        ...
        if (convert) {
            ...
            *format = dest_format;
            ...
            if (raster_need_transform())
                if (!pixel_transform(data, &pixels, width, height, *format,
                                     *type, glstate->raster.raster_scale,
...
static int get_shrinklevel(int width, int height, int level)
{
    ...
    case 8:  // advertise *4 max texture size, but >2048 are shrinked to 2048
        if ((mipwidth > hardext.maxsize * 2) ||
            (mipheight > hardext.maxsize * 2)) {
            shrink = 2;
        } else if ((mipwidth > hardext.maxsize) ||
                   (mipheight > hardext.maxsize)) {
            shrink = 1;
        }
        break;
...
void APIENTRY_GL4ES gl4es_glTexImage2D(GLenum target,
                                       GLint level,
                                       ...
                                       const GLvoid *data)
{
    DBG(printf("glTexImage2D on target=%s with unpack_row_length(%i), "
               "size(%i,%i) and skip(%i,%i), format(internal)=%s(%s), type=%s, "
               "data=%p, level=%i (mipmap_need=%i, mipmap_auto=%i, "
               "base_level=%i, max_level=%i) => texture=%u (streamed=%i), "
               "glstate->list.compiling=%d\n",
               PrintEnum(target), glstate->texture.unpack_row_length, width,
               height, glstate->texture.unpack_skip_pixels,
               ...
               glstate->texture
                   .bound[glstate->texture.active][what_target(target)]
                   ->mipmap_need,
               glstate->texture
                   .bound[glstate->texture.active][what_target(target)]
                   ->mipmap_auto,
    ...
    // Special case when resizing an attached to FBO texture, that is attached
    // to depth and/or stencil => resizing is specific then
    if (bound->binded_fbo &&
        ...
        if (data != NULL)
            printf(
                "LIBGL: Warning, Depth/stencil texture resized and with "
                "data\n");
        // get new size...
        ...
        if (bound->binded_attachment == GL_DEPTH_ATTACHMENT ||
            ...
            if (bound->renderdepth) {
                ...
                gles_glTexImage2D(GL_TEXTURE_2D, 0, bound->format,
                                  bound->nwidth, bound->nheight, 0,
    ...
    if (level == 0 || !bound->valid) {
        bound->wanted_internal =
            internalformat;  // save it before transformation
    }
    GLenum new_format =
        swizzle_internalformat((GLenum *) &internalformat, format, type);
    if (level == 0 || !bound->valid) {
    ...
    if (datab) {
        ...
        GLvoid *old = pixels;
        pixels =
            (GLvoid *) swizzle_texture(width, height, &format, &type,
            ...
            case 11:
                if ((mipwidth > 1) && (mipheight > 1)) {
                    ...
                    if (ratiox == 0.5f && ratioy == 0.5f &&
                        npot(width) == width && npot(height) == height) {
                        // prefer the fast and clean way first
                        pixel_halfscale(pixels, &out, width, height, format,
                                        type);
                    } else {
                        bound->ratiox = ratiox;
                        bound->ratioy = ratioy;
                        bound->useratio = 1;
                        pixel_scale(pixels, &out, width, height, newwidth,
                                    newheight, format, type);
                    }
            ...
            default:
                ...
                while (shrink) {
                    ...
                    if (toshrink == 1) {
                        ...
                        pixel_quarterscale(pixels, &out, width, height, format,
                                           type);
                        if (!bound->valid) {
                            bound->ratiox *= 0.25f;
                            bound->ratioy *= 0.25f;
        ...
        if (!bound->streamed)
            swizzle_texture(width, height, &format, &type, internalformat,
                            new_format, NULL,
            ...
            case 11:
                if ((mipwidth > 1) && (mipheight > 1)) {
                    ...
                    if (globals4es.texshrink == 11 &&
                        newwidth > hardext.maxsize)
                        newwidth =
                            hardext.maxsize;  // in case of some rounding error
                    if (globals4es.texshrink == 11 &&
                        newheight > hardext.maxsize)
                        newheight =
                            hardext.maxsize;  // in case of some rounding error
    ...
    /* TODO: GL_INVALID_VALUE is generated if border is not 0.
     * GL_INVALID_OPERATION is generated if type is GL_UNSIGNED_SHORT_5_6_5 and
     * format is not GL_RGB.
     *
     * GL_INVALID_OPERATION is generated if type is one of
     * GL_UNSIGNED_SHORT_4_4_4_4, or GL_UNSIGNED_SHORT_5_5_5_1 and format is not
     * GL_RGBA.
     */
    ...
    int limitednpot = 0;
    {
        ...
        bound->npot = (nheight != height ||
                       nwidth != width);  // hardware that fully support NPOT
                                          // doesn't care anyway
        if (bound->npot) {
...
#undef MIN_SIZE
        ...
        if (bound->npot) {
            if (limitednpot && rtarget == GL_TEXTURE_2D) {
                bound->sampler.wrap_t = bound->sampler.wrap_s =
                    GL_CLAMP_TO_EDGE;
            } else if (!wrap_npot(bound->sampler.wrap_s) ||
                       !wrap_npot(bound->sampler.wrap_t)) {
                // resize to npot boundaries (not ideal if the wrap value is
                // change after upload of the texture)
                ...
                GLvoid *out = pixels;
                if (pixels)
                    pixel_scale(pixels, &out, width, height, nwidth, nheight,
                                format, type);
                if (out != pixels && pixels != datab)
                    free(pixels);
                pixels = out;
        ...
        if (level == 0) {
            ...
            if (target == GL_TEXTURE_RECTANGLE_ARB && hardext.esversion == 2) {
                bound->adjust =
                    0;  // because this test is used in a lot of places
                bound->adjustxy[0] = 1.0f / width;
                bound->adjustxy[1] = 1.0f / height;
            } else {
                // TEXTURE_RECTANGLE could be mutualize with npot texture
                // probably
        ...
        if (!(globals4es.texstream && bound->streamed)) {
            if ((target != GL_TEXTURE_RECTANGLE_ARB) &&
                ...
                if ((itarget != ENABLED_CUBE_MAP &&
                    ...
                    gl4es_glTexParameteri(
                        ...
                        bound->sampler.min_filter);  // forcing min_filter to be
                                                     // recomputed
                }
            }
            ...
            if (height != nheight || width != nwidth) {
                errorGL();
                gles_glTexImage2D(rtarget, level, format, nwidth, nheight,
                ...
                if (level == 0 && (width == 1 && height == 1 && pixels)) {
                    ...
                    // also, don't keep the fact we have resized, the
                    // non-adjusted coordinates will work (as the texture is
                    // enlarged)
                    if (width == 1) {
                        gles_glTexSubImage2D(rtarget, level, 1, 0, width,
                                             height, format, type, pixels);
                        nwidth = 1;
                    }
                    if (height == 1) {
                        gles_glTexSubImage2D(rtarget, level, 0, 1, width,
                                             height, format, type, pixels);
                        nheight = 1;
                    }
                    if (width == 1 &&
                        height ==
                            1) {  // create a manual mipmap just in case_state
                        gles_glTexSubImage2D(rtarget, level, 1, 1, width,
                                             height, format, type, pixels);
                        gles_glTexImage2D(rtarget, 1, format, 1, 1, 0, format,
                                          type, pixels);
                    }
                }
#endif
            ...
            if (bound->base_level == level &&
                ...
                while (leveln) {
                    ...
                    if (!pot && pixels)
                        gles_glTexSubImage2D(rtarget, leveln, 0, 0, nw, nh,
                                             format, type, ndata);
                }
                if (ndata != pixels)
                    free(ndata);
            ...
            if (((bound->max_level == level && (level || bound->mipmap_need)) ||
                 ...
                 (globals4es.automipmap == 5 && level &&
                  !bound->mipmap_done)) &&
                !(bound->max_level == bound->base_level &&
                  bound->max_level == 0)) {
                ...
                while (nw != 1 || nh != 1) {
                    ...
                    gles_glTexImage2D(rtarget, leveln, format, nw, nh, border,
                                      format, type, (pot) ? ndata : NULL);
                    if (!pot && pixels)
                        gles_glTexSubImage2D(rtarget, leveln, 0, 0, nww, nhh,
                                             format, type, ndata);
                }
                if (ndata != pixels)
                    free(ndata);
    ...
    if ((target == GL_TEXTURE_2D) && globals4es.texcopydata &&
        ...
        if (datab) {
            if (!pixel_convert(pixels, &bound->data, width, height, format,
                               type, GL_RGBA, GL_UNSIGNED_BYTE, 0,
                               glstate->texture.unpack_align))
                printf(
                    "LIBGL: Error on pixel_convert when TEXCOPY in "
                    "glTexImage2D\n");
        }
    }
    if (pixels != datab) {
...
void APIENTRY_GL4ES gl4es_glTexSubImage2D(GLenum target,
                                          GLint level,
                                          GLint xoffset,
                                          GLint yoffset,
                                          GLsizei width,
                                          ...
                                          const GLvoid *data)
{
    ...
    DBG(printf(
            ...
            "data=%p(vao=%p)\n",
            PrintEnum(target), glstate->texture.unpack_row_length, width,
            height, xoffset, yoffset, glstate->texture.unpack_skip_pixels,
            glstate->texture.unpack_skip_rows, PrintEnum(format),
            PrintEnum(type), level,
            glstate->texture.bound[glstate->texture.active][itarget]
                ->base_level,
        ...
        if (!pixel_convert(old, &pixels, width, height, format, type,
            ...
            if (bound->inter_format != bound->format ||
                ...
                if (!pixel_convert(pixels, &pix2, width, height, format, type,
                                   ...
                                   glstate->texture.unpack_align)) {
                    printf(
                        "LIBGL: Error in pixel_convert while "
                        "glTexSubImage2D\n");
                }
    ...
    if (bound->shrink || bound->useratio) {
        ...
        if (bound->useratio) {
            ...
            int newwidth = width * bound->ratiox;
            int newheight = height * bound->ratioy;
            pixel_scale(pixels, &old, width, height, newwidth, newheight,
                        format, type);
            width = newwidth;
            height = newheight;
            ...
            while (shrink) {
                ...
                if (toshrink == 1)
                    ...
                    pixel_quarterscale(pixels, &old, width, height, format,
                                       type);
                if (old != pixels && pixels != datab)
                    free(pixels);
                pixels = old;
    ...
    if (globals4es.texstream && bound->streamed) {
        ...
        if (bound->base_level == level &&
            !(bound->max_level == level && level == 0)) {
            int leveln = level, nw = width, nh = height, xx = xoffset,
                yy = yoffset;
            void *ndata = pixels;
            while (leveln) {
                if (pixels) {
                ...
                yy <<= 1;
                --leveln;
                gles_glTexSubImage2D(rtarget, leveln, xx, yy, nw, nh, format,
                                     type, ndata);
            }
            if (ndata != pixels)
                free(ndata);
    ...
    if ((target == GL_TEXTURE_2D) && globals4es.texcopydata &&
        ((globals4es.texstream && !bound->streamed) || !globals4es.texstream)) {
        GLvoid *tmp =
            (char *) bound->data + (yoffset * bound->width + xoffset) * 4;
        if (!pixel_convert(pixels, &tmp, width, height, format, type, GL_RGBA,
                           GL_UNSIGNED_BYTE, bound->width,
                           glstate->texture.unpack_align))
            printf(
                "LIBGL: Error on pixel_convert while TEXCOPY in "
                "glTexSubImage2D\n");
    }
...
// 1d stubs
void APIENTRY_GL4ES gl4es_glTexImage1D(GLenum target,
                                       GLint level,
                                       GLint internalFormat,
                                       GLsizei width,
                                       GLint border,
...
void APIENTRY_GL4ES gl4es_glTexSubImage1D(GLenum target,
                                          GLint level,
                                          GLint xoffset,
                                          GLsizei width,
                                          GLenum format,
...
void APIENTRY_GL4ES gl4es_glTexStorage1D(GLenum target,
                                         GLsizei levels,
                                         GLenum internalformat,
                                         GLsizei width)
{
...
void APIENTRY_GL4ES gl4es_glTexStorage2D(GLenum target,
                                         GLsizei levels,
                                         GLenum internalformat,
                                         GLsizei width,
                                         GLsizei height)
{
    ...
    DBG(printf("glTexStorage2D(%s, %d, %s, %d, %d)\n", PrintEnum(target),
               levels, PrintEnum(internalformat), width, height);)
    if (!levels) {
        noerrorShim();
        return;
    }
