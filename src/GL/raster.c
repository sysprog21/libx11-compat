...
void APIENTRY_GL4ES gl4es_glViewport(GLint x,
                                     GLint y,
                                     GLsizei width,
                                     GLsizei height)
{
...
void APIENTRY_GL4ES gl4es_glScissor(GLint x,
                                    GLint y,
                                    GLsizei width,
                                    GLsizei height)
{
...
void APIENTRY_GL4ES gl4es_glPixelTransferf(GLenum pname, GLfloat param)
{
    if (glstate->list.active)
        if (glstate->list.compiling) {
            NewStage(glstate->list.active, STAGE_RASTER);
            rlRasterOp(glstate->list.active, pname | 0x10000, param, 0.0f,
                       0.0f);
            noerrorShim();
            return;
        } else
    ...
    case GL_MAP_COLOR:
        ...
        break;
        /* default: printf("LIBGL: stubbed glPixelTransferf(%04x, %f)\n", pname,
         * param);
         */
        // the other...
    }
...
void raster_to_texture(rasterlist_t *r)
{
    ...
    if (r->texture == 0) {
        ...
        gl4es_glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                              GL_CLAMP_TO_EDGE);
        gl4es_glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                              GL_CLAMP_TO_EDGE);
        ...
        gl4es_glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL,
                              0);  // ... if not needed
        gl4es_glTexImage2D(
            GL_TEXTURE_2D, 0, GL_RGBA, glstate->raster.raster_nwidth,
            glstate->raster.raster_nheight, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    } else {
        gl4es_glBindTexture(GL_TEXTURE_2D, r->texture);
    }
...
void bitmap_flush()
{
    ...
    if (glstate->raster.bm_tnwidth < glstate->raster.bm_width ||
        ...
        gl4es_glTexImage2D(
            GL_TEXTURE_2D, 0, GL_RGBA, glstate->raster.bm_tnwidth,
            glstate->raster.bm_tnheight, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    }
    ...
    if (sx == 0 && sy == 0 && ex == glstate->raster.bm_width &&
        ...
        gl4es_glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, glstate->raster.bm_width,
                              glstate->raster.bm_height, GL_RGBA,
...
void APIENTRY_GL4ES gl4es_glBitmap(GLsizei width,
                                   GLsizei height,
                                   GLfloat xorig,
                                   GLfloat yorig,
                                   GLfloat xmove,
...
{
    // TODO: shouldn't be drawn if the raster pos is outside the viewport? TODO:
    // negative width/height mirrors bitmap?
    noerrorShim();
    FLUSH_BEGINEND;
    if (glstate->list.active) {
        ...
        if (list->bitmaps->count == list->bitmaps->cap) {
            ...
            list->bitmaps->list = (bitmap_list_t *) realloc(
                list->bitmaps->list,
                list->bitmaps->cap * sizeof(bitmap_list_t));
        }
    ...
    if (glstate->raster.bm_alloc <
        ...
        glstate->raster.bm_alloc = glstate->raster.viewport.width *
                                   glstate->raster.viewport.height * 4;
    ...
    // finished, move...
    if ((glstate->raster.rPos.x + xmove) >= 0 &&
        (glstate->raster.rPos.y + ymove) >= 0) {
        glstate->raster.rPos.x += xmove;
        glstate->raster.rPos.y += ymove;
    }
...
void APIENTRY_GL4ES gl4es_glDrawPixels(GLsizei width,
                                       GLsizei height,
                                       GLenum format,
                                       GLenum type,
                                       const GLvoid *data)
{
    ...
    if (pixtrans) {
        for (int y = 0; y < height; y++) {
            to = glstate->raster.data +
                 4 * (GLint) (y * glstate->raster.raster_width);
            from = pixels +
                   4 * (glstate->texture.unpack_skip_pixels +
...
void APIENTRY_GL4ES gl4es_glPixelMapfv(GLenum map,
                                       GLsizei mapsize,
...
void APIENTRY_GL4ES gl4es_glPixelMapuiv(GLenum map,
                                        GLsizei mapsize,
                                        const GLuint *values)
{
    if (mapsize > MAX_MAP_SIZE) {
...
void APIENTRY_GL4ES gl4es_glPixelMapusv(GLenum map,
                                        GLsizei mapsize,
