...
#define clone_gl_pointer(t, s, n)                                       \
    ...
    t.type = type;                                                      \
    t.stride = stride;                                                  \
    t.pointer = (void *) ((char *) pointer +                            \
                          (uintptr_t) ((glstate->vao->vertex)           \
...
void APIENTRY_GL4ES gl4es_glVertexPointer(GLint size,
                                          GLenum type,
                                          GLsizei stride,
...
void APIENTRY_GL4ES gl4es_glColorPointer(GLint size,
                                         GLenum type,
                                         GLsizei stride,
                                         const GLvoid *pointer)
{
...
void APIENTRY_GL4ES gl4es_glNormalPointer(GLenum type,
                                          GLsizei stride,
                                          const GLvoid *pointer)
{
...
void APIENTRY_GL4ES gl4es_glTexCoordPointer(GLint size,
                                            GLenum type,
                                            GLsizei stride,
                                            const GLvoid *pointer)
{
...
void APIENTRY_GL4ES gl4es_glSecondaryColorPointer(GLint size,
                                                  GLenum type,
                                                  GLsizei stride,
                                                  const GLvoid *pointer)
{
...
void APIENTRY_GL4ES gl4es_glFogCoordPointer(GLenum type,
                                            GLsizei stride,
                                            const GLvoid *pointer)
{
...
void APIENTRY_GL4ES gl4es_glInterleavedArrays(GLenum format,
                                              GLsizei stride,
                                              const GLvoid *pointer)
{
...
void APIENTRY_GL4ES gl4es_glNormal3fv(GLfloat *v)
{
    if (glstate->list.active) {
        if (glstate->list.active->stage != STAGE_DRAW) {
            if (glstate->list.compiling && glstate->list.active) {
                memcpy(glstate->list.active->lastNormal, v,
                       3 * sizeof(GLfloat));
            } else if (glstate->list.pending &&
                       glstate->list.active->stage == STAGE_POSTDRAW) {
                memcpy(glstate->list.active->post_normals, v,
                       3 * sizeof(GLfloat));
                glstate->list.active->post_normal = 1;
                return;
            }
            ...
            if (!glstate->list.pending)
                return gl4es_glNormal3f(v[0], v[1],
                                        v[2]);  // this will put the call on the
                                                // stack in the current list
        } else {
...
void APIENTRY_GL4ES gl4es_glColor4f(GLfloat red,
                                    GLfloat green,
                                    GLfloat blue,
                                    GLfloat alpha)
{
...
void APIENTRY_GL4ES gl4es_glColor4fv(GLfloat *v)
{
    if (glstate->list.active) {
        if (glstate->list.active->stage != STAGE_DRAW) {
            if (glstate->list.compiling ||
                glstate->list.active->stage < STAGE_DRAW) {
                memcpy(glstate->list.active->lastColors, v,
                       4 * sizeof(GLfloat));
                glstate->list.active->lastColorsSet = 1;
            } else if (glstate->list.pending &&
                       glstate->list.active->stage == STAGE_POSTDRAW) {
                memcpy(glstate->list.active->post_colors, v,
                       4 * sizeof(GLfloat));
                glstate->list.active->post_color = 1;
                return;
            }
...
void APIENTRY_GL4ES gl4es_glTexCoord4f(GLfloat s,
                                       GLfloat t,
                                       GLfloat r,
                                       GLfloat q)
{
    if (glstate->list.active) {
        if (glstate->list.pending)
            ...
            // test if called between glBegin / glEnd but Texture is not active
            // and not using a program. In that case, ignore the call
            if (hardext.esversion == 1 || glstate->glsl->program ||
                (glstate->list.begin &&
                 (glstate->list.compiling || glstate->enable.texture[0])))
                rlMultiTexCoord4f(glstate->list.active, GL_TEXTURE0, s, t, r,
                                  q);
        }
    }
    noerrorShimNoPurge();
...
void APIENTRY_GL4ES gl4es_glMultiTexCoord4f(GLenum target,
                                            GLfloat s,
                                            GLfloat t,
                                            GLfloat r,
                                            GLfloat q)
...
void ToBuffer(int first, int count)
{
    ...
    uintptr_t master =
        (uintptr_t) glstate->vao->vertexattrib[ATT_VERTEX].pointer;
    ...
    gl4es_scratch_vertex(
        total);  // alloc if needed and bind scratch vertex buffer
...
void APIENTRY_GL4ES gl4es_glNewList(GLuint list, GLenum mode)
{
    ...
    // TODO: if glstate->list.active is already defined, we probably need to
    // clean up here
    glstate->list.active = alloc_renderlist();
...
void APIENTRY_GL4ES gl4es_glCallLists(GLsizei n,
                                      GLenum type,
                                      const GLvoid *lists)
{
...
void APIENTRY_GL4ES gl4es_glIndexPointer(GLenum type,
                                         GLsizei stride,
                                         const GLvoid *pointer)
{
...
void APIENTRY_GL4ES gl4es_glColorMask(GLboolean red,
                                      GLboolean green,
                                      GLboolean blue,
                                      GLboolean alpha)
{
...
{
    // TODO: test valid clamp values? Not to be executed in list
...
void gl4es_scratch_indices(int alloc)
{
    ...
    if (glstate->scratch_indices_size < alloc) {
        gles_glBufferData(GL_ELEMENT_ARRAY_BUFFER, alloc, NULL,
                          GL_DYNAMIC_DRAW);
