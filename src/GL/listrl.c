...
static inline void rlVertexCommon(renderlist_t *list, int idx, int l)
{
    if (list->use_glstate) {
        ...
        if (list->fogcoord)
            memcpy(list->fogcoord + idx, glstate->fogcoord,
                   sizeof(GLfloat) * 1);
    } else {
        if (!list->vert)
            ...
            resize_renderlist(list);
        if (list->normal)
            memcpy(list->normal + (l * 3), list->lastNormal,
                   sizeof(GLfloat) * 3);
        if (list->fogcoord)
            memcpy(list->fogcoord + (l * 1), glstate->fogcoord,
                   sizeof(GLfloat) * 1);
    }
    // common part
    if (list->color)
    ...
    if (list->secondary)
        memcpy(list->secondary + (l * 4), glstate->secondary,
               sizeof(GLfloat) * 4);
    if (list->tex[0])
    ...
    for (int a = 2; a < list->maxtex; a++)
        if (list->tex[a])
            memcpy(list->tex[a] + (l * 4), glstate->texcoord[a],
                   sizeof(GLfloat) * 4);
}

void FASTMATH
rlVertex4f(renderlist_t *list, GLfloat x, GLfloat y, GLfloat z, GLfloat w)
{
...
void rlEnd(renderlist_t *list)
{
    ...
    if (list->indices && list->merger_mode && list->len - list->cur_istart) {
        ...
        resize_indices_renderlist(
            list, indices_getindicesize(list->merger_mode, len));
        ...
        case GL_TRIANGLE_STRIP:
            ...
            for (int i = istart + 3; i < list->len; i++) {
                ...
                list->indices[list->ilen++] =
                    istart + (ivert - ((ivert % 2) ? 1 : 2));
                list->indices[list->ilen++] =
                    istart + (ivert - ((ivert % 2) ? 2 : 1));
                list->indices[list->ilen++] = istart + (ivert++);
            }
            break;
        case GL_QUADS:
    ...
    if (list->mode_inits)
        list_add_modeinit(
            list, list->merger_mode ? list->merger_mode : list->mode_init);
    if (list->color)
...
void FASTMATH
rlColor4f(renderlist_t *list, GLfloat r, GLfloat g, GLfloat b, GLfloat a)
{
    rlColorCommon(list);
...
void rlMaterialfv(renderlist_t *list,
                  GLenum face,
                  GLenum pname,
...
void rlLightfv(renderlist_t *list,
               GLenum which,
               GLenum pname,
               const GLfloat *params)
{
...
void rlTexGenfv(renderlist_t *list,
                GLenum coord,
                GLenum pname,
                const GLfloat *params)
{
...
void FASTMATH rlMultiTexCoord4f(renderlist_t *list,
                                GLenum target,
                                GLfloat s,
                                GLfloat t,
                                GLfloat r,
...
void rlTexEnvfv(renderlist_t *list,
                GLenum target,
                GLenum pname,
                const GLfloat *params)
{
...
void rlTexEnviv(renderlist_t *list,
                GLenum target,
                GLenum pname,
                const GLint *params)
{
