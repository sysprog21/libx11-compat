...
static inline bool remap_pixel(const GLvoid *src,
                               GLvoid *dst,
...
#define write_each(amod, vmod, hmod)                                \
    ...
    // this pixel stores our intermediate color it will be RGBA and normalized
    // to between (0.0 - 1.0f)
    pixel_t pixel;
    ...
    type_case(GL_DOUBLE, GLdouble, read_each(, , ))
        type_case(GL_FLOAT, GLfloat, read_each(, , ))
            type_case(GL_HALF_FLOAT_OES, halffloat_t, read_each(, , float_h2f))
                type_case(GL_BYTE, GLbyte,
                          read_each(, / 128.0f, )) case GL_INT8_REV:
        type_case(GL_UNSIGNED_BYTE, GLubyte, read_each(, / 255.0f, )) type_case(
            GL_UNSIGNED_SHORT, GLushort,
            read_each(, / 65535.0f, )) type_case(GL_INT8, GLubyte,
                                                 read_each(max_a -, / 255.0f, ))
            type_case(GL_UNSIGNED_SHORT_5_6_5_REV, GLushort,
                ...
                type_case(GL_UNSIGNED_SHORT_1_5_5_5_REV, GLushort,
                    ...
                    type_case(
                        ...
                        s = (const GLushort[]) {((v) & 0x0f), ((v >> 4) & 0x0f),
                                                ((v >> 8) & 0x0f),
                                                ((v >> 12) & 0x0f)};
                        read_each(, / 15.0f, );)
    ...
    if ((dst_color->red == dst_color->green) &&
        ...
        GLfloat aa = (pixel.r + pixel.g + pixel.b) /
                     3.0f;  //*TODO* find a better formula. real luminance is
    ...
    switch (dst_type) {
        type_case(GL_FLOAT, GLfloat, write_each(, , ))
            type_case(GL_HALF_FLOAT_OES, halffloat_t, write_each(, , float_f2h))
                type_case(GL_BYTE, GLbyte, write_each(, *127.0f, ))
                    type_case(GL_UNSIGNED_BYTE, GLubyte, write_each(, *255.0, ))
                        type_case(GL_UNSIGNED_SHORT, GLushort,
                                  write_each(, *65535.0f, ))
                            type_case(GL_UNSIGNED_INT, GLuint,
                                      write_each(, *(float) 0xffffffff, ))
                                type_case(GL_INT8_REV, GLubyte,
                                          write_each(, *255.0, ))
            ...
            type_case(GL_UNSIGNED_SHORT_5_6_5, GLushort, GLfloat color[4];
                ...
                type_case(
                    GL_UNSIGNED_SHORT_5_5_5_1, GLushort, GLfloat color[4];
                    color[dst_color->red] = pixel.r;
                    color[dst_color->green] = pixel.g;
                    color[dst_color->blue] = pixel.b;
...
static inline bool transform_pixel(const GLvoid *src,
                                   GLvoid *dst,
                                   const colorlayout_t *src_color,
                                   GLenum src_type,
                                   const GLfloat *scale,
...
#define transformf(pix, number)               \
    ...
    type_case(GL_DOUBLE, GLdouble, read_each(, , ))
        ...
        type_case(GL_UNSIGNED_BYTE, GLubyte, read_each(, / 255.0f, )) type_case(
            GL_UNSIGNED_SHORT, GLushort, read_each(, / 65535.0f, ))
...
static inline bool half_pixel(const GLvoid *src0,
                              const GLvoid *src1,
                              const GLvoid *src2,
                              const GLvoid *src3,
                              GLvoid *dst,
...
static inline bool quarter_pixel(const GLvoid *src[16],
                                 GLvoid *dst,
                                 const colorlayout_t *src_color,
                                 GLenum src_type)
{
...
#define write_each(amod, vmod, hmod)                                \
    ...
    pixel.r = (pix[0].r + pix[1].r + pix[2].r + pix[3].r + pix[4].r + pix[5].r +
               pix[6].r + pix[7].r + pix[8].r + pix[9].r + pix[10].r +
               pix[11].r + pix[12].r + pix[13].r + pix[14].r + pix[15].r) *
              0.0625f;
    pixel.g = (pix[0].g + pix[1].g + pix[2].g + pix[3].g + pix[4].g + pix[5].g +
               pix[6].g + pix[7].g + pix[8].g + pix[9].g + pix[10].g +
               pix[11].g + pix[12].g + pix[13].g + pix[14].g + pix[15].g) *
              0.0625f;
    pixel.b = (pix[0].b + pix[1].b + pix[2].b + pix[3].b + pix[4].b + pix[5].b +
               pix[6].b + pix[7].b + pix[8].b + pix[9].b + pix[10].b +
               pix[11].b + pix[12].b + pix[13].b + pix[14].b + pix[15].b) *
              0.0625f;
    pixel.a = (pix[0].a + pix[1].a + pix[2].a + pix[3].a + pix[4].a + pix[5].a +
               pix[6].a + pix[7].a + pix[8].a + pix[9].a + pix[10].a +
               pix[11].a + pix[12].a + pix[13].a + pix[14].a + pix[15].a) *
              0.0625f;
...
bool pixel_convert(const GLvoid *src,
                   GLvoid **dst,
                   GLuint width,
                   GLuint height,
                   GLenum src_format,
                   ...
                   GLuint align)
{
    ...
    GLuint dst_width =
        dst_width2 - (width * pixel_sizeof(dst_format, dst_type));
    GLuint src_width =
    ...
    if ((src_type == dst_type) && (dst_format == src_format)) {
        ...
        if (stride)  // for in-place conversion
            for (int yy = 0; yy < height; yy++)
                memcpy((char *) (*dst) + yy * dst_width2,
                       (char *) src + yy * src_width, src_width);
        else
    ...
    // fast optimized loop for common conversion cases first... TODO: Rewrite
    // that with some Macro, it's obviously doable to simplify the reading (and
    // writing) of all this simple BGRA <-> RGBA / UNSIGNED_BYTE
    ...
    if (src_format == GL_COLOR_INDEX) {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                idx =
                    (((*((GLubyte *) src_pos)) << glstate->raster.index_shift) +
                     glstate->raster.index_offset);
                if (glstate->raster.map_i2i_size - 1)
                    idx =
                        glstate->raster
                            .map_i2i[idx & (glstate->raster.map_i2i_size - 1)];
                tmp[0] = glstate->raster
                             .map_i2r[idx & (glstate->raster.map_i2r_size - 1)];
                tmp[1] = glstate->raster
                             .map_i2g[idx & (glstate->raster.map_i2g_size - 1)];
                tmp[2] = glstate->raster
                             .map_i2b[idx & (glstate->raster.map_i2b_size - 1)];
                tmp[3] = glstate->raster
                             .map_i2a[idx & (glstate->raster.map_i2a_size - 1)];
                remap_pixel((const GLvoid *) tmp, (GLvoid *) dst_pos, src_color,
                            GL_UNSIGNED_BYTE, dst_color, dst_type);
                src_pos += src_stride;
                dst_pos += dst_stride;
            }
            dst_pos += dst_width;
...
bool pixel_transform(const GLvoid *src,
                     GLvoid **dst,
                     GLuint width,
                     GLuint height,
                     GLenum src_format,
                     GLenum src_type,
                     const GLfloat *scales,
                     const GLfloat *bias)
{
    const colorlayout_t *src_color;
    GLuint pixels = width * height;
...
bool pixel_scale(const GLvoid *old,
                 GLvoid **new,
                 GLuint width,
                 GLuint height,
                 GLuint new_width,
...
bool pixel_halfscale(const GLvoid *old,
                     GLvoid **new,
                     GLuint width,
                     GLuint height,
                     GLenum format,
                     GLenum type)
{
    ...
    for (int y = 0; y < new_height; y++) {
        for (int x = 0; x < new_width; x++) {
            ...
            half_pixel((GLvoid *) pix0, (GLvoid *) pix1, (GLvoid *) pix2,
                       (GLvoid *) pix3, (GLvoid *) pos, src_color, type);
            pos += pixel_size;
        }
    }
    *new = dst;
                      ...
                      GLenum type)
{
    ...
    for (int y = 0; y < new_height; y++) {
        for (int x = 0; x < new_width; x++) {
            ...
            pix3 = src + ((x * mx + dx) + (y * my + dy) * width) * pixel_size;
            half_pixel((GLvoid *) pix0, (GLvoid *) pix1, (GLvoid *) pix2,
                       (GLvoid *) pix3, (GLvoid *) tmp, src_color, type);
            *((GLushort *) pos) = (((GLushort) tmp[0]) & 0xf0) << 8 |
                                  (((GLushort) tmp[1]) & 0xf0) << 4 |
                                  (((GLushort) tmp[2]) & 0xf0) |
                                  (((GLushort) tmp[3]) >> 4);
            pos += dest_size;
        }
...
bool pixel_quarterscale(const GLvoid *old,
                        GLvoid **new,
                        GLuint width,
                        GLuint height,
                        GLenum format,
    ...
    if (!src_color->type) {
        ...
        for (int y = 0; y < new_height; y++) {
            for (int x = 0; x < new_width; x++) {
                ...
                // no smart downsize here, the pixel is probably not RGB anyway
                memcpy((void *) pos, (const void *) pix[0], pixel_size);
                pos += pixel_size;
            }
        }
        *new = dst;
        ...
        for (int x = 0; x < new_width; x++) {
            for (int dx = 0; dx < 4; dx++) {
                for (int dy = 0; dy < 4; dy++) {
                    pix[dx + dy * 4] =
                        src + ((x * 4 + dxs[dx]) + (y * 4 + dys[dy]) * width) *
                                  pixel_size;
                }
            }
            quarter_pixel((const GLvoid **) pix, (GLvoid *) pos, src_color,
                          type);
            pos += pixel_size;
        }
    }
...
bool pixel_doublescale(const GLvoid *old,
                       GLvoid **new,
                       GLuint width,
                       GLuint height,
                       GLenum format,
                       GLenum type)
{
    ...
    for (int y = 0; y + 1 < new_height; y += 2) {
        for (int x = 0; x + 1 < new_width; x += 2) {
            ...
            memcpy((void *) pos, (void *) pix0, pixel_size);
            memcpy((void *) (pos + new_width * pixel_size), (void *) pix0,
                   pixel_size);
            pos += pixel_size;
            memcpy((void *) pos, (void *) pix0, pixel_size);
            memcpy((void *) (pos + new_width * pixel_size), (void *) pix0,
                  ...
                  GLuint align)
{
    ...
    if (format == GL_RGB && type == GL_UNSIGNED_BYTE) {
        ...
        if (!pixel_convert(pixels, (GLvoid **) &src, width, height, format,
                           type, GL_RGB, GL_UNSIGNED_BYTE, 0, align)) {
            return false;
        }
    }
