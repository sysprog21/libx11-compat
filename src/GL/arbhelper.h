...
// #define DEBUG ARBCONV_DBG - general ArbConverter debug logs
...
typedef enum _eVariableType {
    ...
    VARTYPE_TEXTURE,    // Not available in vertex programs, fixed name in
                        // fragment programs
    VARTYPE_TEXTARGET,  // Not available in vertex programs, fixed name in
                        // fragment programs
    VARTYPE_UNK
} eVariableType;
...
#define TOKEN2STR(t)                                                          \
    (ENUMVALUE2STR(t, TOK_, NULL) ENUMVALUE2STR(                              \
        t, TOK_, WHITESPACE) ENUMVALUE2STR(t, TOK_, SIGN)                     \
         ENUMVALUE2STR(t, TOK_, INTEGER) ENUMVALUE2STR(                       \
             t, TOK_, FLOATCONST) ENUMVALUE2STR(t, TOK_, IDENTIFIER)          \
             ENUMVALUE2STR(t, TOK_, POINT) ENUMVALUE2STR(                     \
                 t, TOK_, UPTO) ENUMVALUE2STR(t, TOK_, COMMA)                 \
                 ENUMVALUE2STR(t, TOK_, EQUALS) ENUMVALUE2STR(                \
                     t, TOK_, LSQBRACKET) ENUMVALUE2STR(t, TOK_, RSQBRACKET)  \
                     ENUMVALUE2STR(t, TOK_, LBRACE) ENUMVALUE2STR(            \
                         t, TOK_, RBRACE) ENUMVALUE2STR(t, TOK_, END_OF_INST) \
                         ENUMVALUE2STR(t, TOK_, LINE_COMMENT) ENUMVALUE2STR(  \
                             t, TOK_, NEWLINE) ENUMVALUE2STR(t, TOK_, END)    \
