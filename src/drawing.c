...
void drawWindowDataToScreen()
{
    ...
    for (i = 0; i < GET_WINDOW_STRUCT(SCREEN_WINDOW)->children.length; i++) {
        ...
        /* A window can enter Mapped state without ever drawing, so its
         * backing texture is still NULL. Triggering getWindowRenderer
         * here lazily allocates and background-fills the texture so
         * the present pipeline never silently skips a mapped window. */
        if (!child->sdlTexture) {
        ...
        /* Clamp the readback rect to all three of the X11 logical size,
         * the backing texture size, and the SDL window surface size.
         * Window surface and X11 logical w/h can diverge during resize
         * and on high-DPI setups; an unclamped read would overflow
         * winSurface->pixels. */
        int texW = 0, texH = 0;
...
SDL_Renderer *getWindowRenderer(Window window)
{
    ...
    if (!renderer) {
        /* All windows draw on the SCREEN renderer with a per-window
         * backing texture as the render target. Mapped top-level
         * windows used to own a separate SDL_Renderer attached to
         * their SDL_Window, which forced every XCopyArea between two
         * top-level windows (and every Pixmap->Window copy) into a
         * cross-renderer readback. Unifying on SCREEN renderer keeps
         * those copies in renderer space; presentation to the actual
         * SDL_Window happens in drawWindowDataToScreen. */
     ...
     * We allocate the SDL surface/texture lazily, so do the equivalent
     * one-shot fill here to avoid presenting uninitialized memory. */
    if (justCreatedRenderer) {
        ...
        /* Direct color push above bypassed the (gc, generation) cache;
         * drop the cache so the next applySdlDrawState re-pushes state
         * instead of trusting a stale entry. */
        invalidateSdlDrawStateCache();
    }
                                  ...
                                  const SDL_Rect *source)
{
    ...
    /* Clip the requested rect to the viewport. SDL_RenderReadPixels
     * fails outright if the rect leaves the render target, so an X11
     * caller asking for a drawable-relative area that runs off the edge
     * would otherwise lose all pixels instead of getting the in-bounds
     * portion zero-padded. */
    SDL_Rect viewport;
    SDL_RenderGetViewport(renderer, &viewport);
    SDL_Rect viewBounds = {0, 0, viewport.w, viewport.h};
    SDL_Rect clipped;
...
#define APPLY_OPERATION_TO_SURFACE(surface, operation, startX, startY, w, h) \
    {                                                                        \
        ...
        int y, i;                                                            \
        for (y = startY; y < h; y++) {                                       \
            pixelPointer = (Uint32 *) surface->pixels +                      \
                           y * (surface->pitch / bytesPerPixel) + startX;    \
            for (i = 0; i < w; i++)                                          \
                operation                                                    \
        }                                                                    \
...
void clipSurface(SDL_Surface *surface,
                 SDL_Surface *clipSurface,
                 int originX,
                 int originY)
{
    LOCK_SURFACE(clipSurface);
    Uint32 *clipPixel = clipSurface->pixels;
    APPLY_OPERATION_TO_SURFACE(
        surface, { pixelPointer[i] &= 0xFFFFFF00 | *clipPixel++; }, originX,
        originY, clipSurface->w, clipSurface->h);
    UNLOCK_SURFACE(clipSurface);
}

void colorClipped(SDL_Surface *surface,
                  SDL_Surface *clipSurface,
                  int originX,
                  int originY,
                  long color)
{
    ...
    APPLY_OPERATION_TO_SURFACE(
        surface,
        {
            if (*clipPixel++ & 0x000000FF && 0) {
                pixelPointer[i] = color;
                LOG("clipping with fg color\n");
            }
        },
               ...
               unsigned long plane)
{
    ...
    /* SDL_Rect and SDL_CreateTexture take signed ints, and the pixel
     * scratch buffer is width * height * sizeof(Uint32) bytes. Reject
     * dimensions that would overflow either signed int or size_t before
    ...
    SDL_Rect srcRect = {src_x, src_y, (int) width, (int) height};
    ...
    SDL_Rect destRect = {dest_x, dest_y, (int) width, (int) height};
    int clipCount = getGcClipIterationCount(gc);
    int rcCopy = 0;
    for (int clip = 0; clip < clipCount; clip++) {
...
int XDrawPoint(Display *display, Drawable d, GC gc, int x, int y)
{
    ...
    GraphicContext *gContext = GET_GC(gc);
    applySdlDrawState(renderer, gc, SDL_BLENDMODE_NONE, gContext->foreground);
    int clipCount = getGcClipIterationCount(gc);
    for (int clip = 0; clip < clipCount; clip++) {
        if (!setGcClipForIteration(renderer, gc, clip))
            continue;
                  ...
                  int nsegments)
{
    ...
    applySdlDrawState(renderer, gc, SDL_BLENDMODE_BLEND,
                      GET_GC(gc)->foreground);
    /* XSegment specifies disjoint segments, so we cannot use
     * SDL_RenderDrawLines (which connects them). One call per segment. */
    int clipCount = getGcClipIterationCount(gc);
    for (int clip = 0; clip < clipCount; clip++) {
        if (!setGcClipForIteration(renderer, gc, clip))
            continue;
               ...
               int mode)
{
    ...
    /* The X protocol allows arbitrarily large point counts. Stack-allocating
     * an SDL_Point[npoints] for untrusted input is a stack-overflow path,
     * so fall back to the heap once the request exceeds the small inline
     * budget. */
    enum { POINT_INLINE_BUDGET = 128 };
    SDL_Point inlinePoints[POINT_INLINE_BUDGET];
    SDL_Point *heapPoints = NULL;
               ...
               Bool exposures)
{
    ...
    SDL_Rect clearRect = {clearX, clearY, clearWidth, clearHeight};
        ...
        if (background &&
            ...
            for (int tileY = clearRect.y; tileY < clearRect.y + clearRect.h;
                ...
                for (int tileX = clearRect.x; tileX < clearRect.x + clearRect.w;
                     tileX += textureWidth) {
                    SDL_Rect dest = {tileX, tileY, textureWidth, textureHeight};
    ...
    if (exposures) {
        SDL_Rect exposeRect = {clearX, clearY, clearWidth, clearHeight};
              ...
              int dest_y)
{
    ...
    if (IS_TYPE(dest, WINDOW)) {
        ...
        /* Fast path: source is a Pixmap (already an SDL_Texture) and
         * the destination is drawing on the same renderer that owns the
         * pixmap texture. Skip the readback-and-upload dance and let
         * SDL_RenderCopy stay in renderer space. */
        if (IS_TYPE(src, PIXMAP)) {
            ...
            if (pixmapTexture && destRenderer == pixmapRenderer) {
                SDL_Rect fastSrc = {src_x, src_y, (int) width, (int) height};
                SDL_Rect fastDest = {dest_x, dest_y, (int) width, (int) height};
                    ...
                    int nrectangles)
{
    ...
    if (gContext->fillStyle == FillSolid) {
        LOG("Fill_style is %s\n", "FillSolid");
        if (gContext->function == GXinvert) {
            /* Software-side invert. SDL_Renderer has no raster-op
             * equivalent, so read the affected pixels back, invert each
             * RGB byte, and blit them in place. */
            int clipCount = getGcClipIterationCount(gc);
            for (int clip = 0; clip < clipCount; clip++) {
                if (!setGcClipForIteration(renderer, gc, clip))
                    continue;
