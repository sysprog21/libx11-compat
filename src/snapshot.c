...
int snapshotRequestResizeAndWait(int width, int height)
{
    ...
    /* Resize triggers a full reflow in Motif/Xt clients (ViolaWWW re-renders
     * every visible paragraph), so the main thread may not drain SDL events for
     * noticeably longer than a plain snapshot. 15s is generous enough to cover
     * that without masking a real stuck-thread hang.
     */
    deadline.tv_sec += 15;
    pthread_mutex_lock(&snapshotMutex);
    int wait_rc = 0;
...
int snapshotRequestAndWait(const char *path)
{
    ...
    if (!copy)
        return -1;
    pthread_mutex_lock(&snapshotMutex);
    /* Lazy registration inside the lock: two concurrent first callers would
     * otherwise each get their own type id from SDL, leaking one id and pushing
     * events that no one is registered to handle (gemini-flagged race).
     */
    if (snapshotEventType == (Uint32) -1) {
        snapshotEventType = SDL_RegisterEvents(1);
        if (snapshotEventType == (Uint32) -1) {
     ...
     * indefinitely. The save itself is single-ms for a typical window, but the
     * main thread may not drain the SDL_USEREVENT for a while if Motif is
     * mid-reflow (e.g. right after a resize, or during the initial-expose burst
     * of a freshly mapped XmMainWindow). 15s covers that without masking a
     * genuine stuck-thread hang.
     */
    struct timespec deadline;
    clock_gettime(CLOCK_REALTIME, &deadline);
    deadline.tv_sec += 15;
