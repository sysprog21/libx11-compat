 ...
 * from the dynamic symbol table so a system libX11.so.6 loaded into
 * the same process via SDL2's transitive dependency chain cannot
 * interpose on them. Without this, libX11.so.6 and libX11-compat.so
 * share storage for _XInitDisplayLock_fn et al., libX11.so.6's
 * XInitThreads (or any implicit init) writes a libX11.so.6 function
 * address into the shared slot, libX11-compat's XOpenDisplay invokes
 * it, and Display->lock_fns ends up with libX11.so.6's vtable,
 * crashing the next LockDisplay against a mismatched struct layout.
 */
