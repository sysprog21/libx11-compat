...
/* Query -[NSWindow inLiveResize] on the captured resize window.
 *
 * Returns 1 while the modal resize loop is running, 0 when it has ended or when
 * no window is captured (so a missing/failed capture disarms rather than
 * sticking armed). BOOL is a signed char in the objc ABI; cast the IMP
 * accordingly.
 */
...
/* Balance the retain taken in liveResizeWillStart and drop the window reference
 * so inLiveResize is never queried on a stale handle. Idempotent. Kept separate
 * from the observer removal so a failed arm (window retained, observer never
 * created) still releases the window.
 */
static void releaseLiveResizeWindow(void)
{
    if (!liveResizeWindow)
        return;
    ((void (*)(void *, void *)) objc_msgSend)(liveResizeWindow,
...
static void disarmLiveResizeObserver(void)
{
    if (liveResizeObserverRunning) {
         ...
         * so CF does not dereference a freed observer after we return; the
         * deferred disarm releases the window then.
         */
    ...
    /* Drag is over (or an arm failed): release the window even when no observer
     * was created, so its retain is always balanced and the pointer never
     * dangles into the next drag.
     */
    releaseLiveResizeWindow();
...
static void liveResizeWillStart(void *self, void *_cmd, void *notification)
{
    ...
    /* Balance a prior capture first: a second WillStart before the matching
     * DidEnd/disarm (AppKit can deliver these out of order) would otherwise
     * overwrite a still-retained window and leak that retain.
     */
    releaseLiveResizeWindow();
    ...
    /* Retain the NSWindow for the duration of the drag. The notification only
     * lends us the object; if the app destroys the native window by a path
     * other than XCloseDisplay (a direct -[NSWindow close]/release), an
     * unretained pointer would dangle and the next tick's inLiveResize message
     * would land on freed memory. The matching release is in
     * releaseLiveResizeWindow, reached from disarmLiveResizeObserver, the
     * single point that clears liveResizeWindow.
     */
    if (liveResizeWindow)
        ((void (*)(void *, void *)) objc_msgSend)(liveResizeWindow,
...
static void liveResizeDidEnd(void *self, void *_cmd, void *notification)
{
    ...
    /* Disarm unconditionally so the retained window is released even when the
     * observer is already gone (a failed arm leaves the window retained with no
     * observer; disarmLiveResizeObserver releases it regardless). Only present
     * a final reflowed frame when an observer was actually armed for this drag;
     * if it was already finalized there is nothing left to repaint.
     */
    int hadObserver = liveResizeObserver ? 1 : 0;
    disarmLiveResizeObserver();
    if (hadObserver) {
        /* One last present outside the modal loop so the final geometry lands
         * on fully reflowed content 1:1 (the observer is now removed, so
         * nothing else will repaint this frame). forceReflow bypasses the
         * settle wait so the final size always gets a real reflow even if the
         * drag ended mid-move.
         */
        libx11CompatPresentDuringLiveResizeEx(1);
    }
...
void libx11CompatConfigureLiveResizeLayer(void *nsWindow)
{
     ...
     * the pinned drawable lands at the wrong spot on Retina.
     *
     * Returns a CGFloat (double on 64-bit) by value.
     */
