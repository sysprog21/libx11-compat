...
python3 "$repo/scripts/compare-motif-reference.py" \\
    ...
    --allow-diff {q(args.allow_diff)} \\
    --allow-diff-mae-ceiling {q(args.allow_diff_mae_ceiling)} \\
    --allow-diff-changed-ceiling {q(args.allow_diff_changed_ceiling)} \\
    --top {q(args.top)}
"""
...
def main():
    ...
    parser.add_argument(
        "--mae-threshold",
        type=float,
        default=float(parse_env_default("MOTIF_DIFF_MAE_THRESHOLD", "0.11")),
    )
    parser.add_argument(
        "--changed-threshold",
        type=float,
        default=float(parse_env_default("MOTIF_DIFF_CHANGED_THRESHOLD", "0.32")),
    )
    parser.add_argument(
        "--allow-diff",
        default=parse_env_default("MOTIF_DIFF_ALLOW_DIFF", "ColorSel,Ext18List"),
        help="capture-name substrings reported as expected-diff (known bitmap"
        " font parity gaps) that do not fail the suite",
    )
    parser.add_argument(
        "--allow-diff-mae-ceiling",
        type=float,
        default=float(parse_env_default("MOTIF_DIFF_ALLOW_MAE_CEILING", "0.20")),
    )
    parser.add_argument(
    ...
    if args.compare_location is None:
        ...
        if env_compare_location:
            if env_compare_location not in ("remote", "local"):
                parser.error("MOTIF_DIFF_COMPARE_LOCATION must be 'remote' or 'local'")
